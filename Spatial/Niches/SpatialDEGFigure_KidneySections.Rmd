...
date: "2023-05-18"
Description: 
#   Detection of differentially expressed genes for different timepoints
...
rm(list=ls())
setwd("/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V09062023/KidneyRegions/")
...
Outdir =c("/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V09062023/KidneyRegions/")
...
Pyelonephritis_Infection<-subset(Pyelonephritis_Infection, subset= (orig.ident == "PyeloD0_1" |orig.ident == "PyeloD1_2"|orig.ident == "PyeloD3_1"|orig.ident == "PyeloD5_2"|orig.ident == "PyeloD7_1"|orig.ident == "PyeloD28_1"))

str(Pyelonephritis_Infection)

table(Pyelonephritis_Infection@meta.data$orig.ident)
...
```{r}
## plot the figure
Pyelonephritis_Infection$orig.ident<-ordered(factor(Pyelonephritis_Infection$orig.ident), levels=c("PyeloD0_1", "PyeloD1_2", "PyeloD3_1", "PyeloD5_2", "PyeloD7_1", "PyeloD28_1"))

Pyelonephritis_Infection@meta.data
Idents(Pyelonephritis_Infection)

## For all the regions:
Idents(Pyelonephritis_Infection) <- Pyelonephritis_Infection@meta.data$orig.ident

Idents(Pyelonephritis_Infection)
DimPlot(Pyelonephritis_Infection, reduction = "umap", group.by = "Annotation",split.by ="orig.ident",label = TRUE,label.size = 2)

Pyelonephritis_InfectionDimplotWithAll<- DimPlot(Pyelonephritis_Infection, reduction = "umap", group.by = "Annotation",label = TRUE,label.size = 6)+ ggtitle("Spatial UMAP")

ggsave("Pyelonephritis_InfectionDimplotWithAll.pdf", plot=Pyelonephritis_InfectionDimplotWithAll, height = 6, width = 8)

Pyelonephritis_InfectionDimplotWithAllWithoutLabel<- DimPlot(Pyelonephritis_Infection, reduction = "umap", group.by = "Annotation",label = FALSE) + ggtitle("Spatial UMAP")
Pyelonephritis_InfectionDimplotWithAllWithoutLabel
ggsave("Pyelonephritis_InfectionDimplotWithAllWithoutLabel.pdf", plot=Pyelonephritis_InfectionDimplotWithAllWithoutLabel, height = 6, width = 6)

Pyelonephritis_InfectionSpatialDimWithAllWithoutLabel<- 
  SpatialDimPlot(Pyelonephritis_Infection, images =selectedslices,crop = TRUE, label = TRUE,label.size = 3, ncol  = 6)
Pyelonephritis_InfectionSpatialDimWithAllWithoutLabel
ggsave("Pyelonephritis_InfectionSpatialDimWithAllWithoutLabel.pdf", plot=Pyelonephritis_InfectionSpatialDimWithAllWithoutLabel, height = 5, width = 30)

```

# redefine the default assay by the SCT using the scale data
```{r}
Pyelonephritis_Infection[["LOL"]] <- CreateAssayObject(counts = GetAssayData(Pyelonephritis_Infection,
                                                                                    assay="SCT", slot="data")) #Copy "SCT data" to "LOL counts" and "LOL data"
Pyelonephritis_Infection <- SetAssayData(Pyelonephritis_Infection, assay = "LOL", 
                                                slot = "scale.data", 
                                                new.data = GetAssayData(Pyelonephritis_Infection, assay="SCT",
                                                                        slot="scale.data")); gc() #"Copy "SCT scale.data" to "LOL scale.data"
DefaultAssay(Pyelonephritis_Infection) <- "LOL"
Pyelonephritis_Infection <- DietSeurat(Pyelonephritis_Infection, assays = "LOL", counts=T, data=T, scale.data=T)
Pyelonephritis_Infection <- RenameAssays(Pyelonephritis_Infection, LOL = 'SCT')
## set up default of Pyelonephritis_Infection
...
# Find the specific expressed genes in the infected regions: "Cortex","Medulla","Pelvis","Vascular","Processing Infection"
# We use the MAST to compare two regions in cluster, here we only concentrate on the infected regions 
```{r}
# identify markers that are different between 5_1 and controls
# the control combined the 5_1,10_1,2_1,10_0,2_0,5_0,5_2

Pyelonephritis_Infection@meta.data<- Pyelonephritis_Infection@meta.data %>%
  mutate(InfectionCombinedComparing = 
           case_when(Annotation %in% c("N1","N2","N3","N4","N5") ~ "Cortex",
                     Annotation %in% c("N6","N7") ~  "Medulla",
                     Annotation %in% c("N8","N10","N11") ~  "Pelvis",
                     Annotation %in% c("N9") ~  "Vascular",
                     #Annotation %in% c("5_1") ~ "Abscess LOC Cortical-Papillary",
                     #Annotation %in% c("10_1") ~ "Abscess Margin Cortical-Papillary",
                     #Annotation %in% c("2_1") ~"Abscess LOC Pelvis",
                    Annotation %in% c("N12","N13","N14","N15","N16","N17") ~ "Processing Infection" )) 

Idents(Pyelonephritis_Infection) <- Pyelonephritis_Infection@meta.data$InfectionCombinedComparing
# change the order or Idents
Idents(Pyelonephritis_Infection) <- ordered(factor(Idents(Pyelonephritis_Infection)), levels=c("Cortex","Medulla","Pelvis","Vascular","Processing Infection"))

factor(Idents(Pyelonephritis_Infection))
#levels(factor(Pyelonephritis_Infection@meta.data$InfectionCombinedComparing))
# make the identity as the abscess regions for all the spots
...
### InfectedCategories_Clustermarkers <- FindAllMarkers(Pyelonephritis_Infection,
###               min.pct = 0.25,test.use= "MAST") # only markers that are in 25% of cells

# save all biomarkers as a dataframe
### write.csv(InfectedCategories_Clustermarkers, "Top_Regionalmarkers_Mast.csv")
...
top_InfectedrmarkersGenesheatmap<- DoHeatmap(Pyelonephritis_Infection, features = top_InfectedrmarkersGenes, size = 2.5)  + scale_fill_gradientn(colors = c("lightblue", "white", "FireBrick")) +  theme(legend.position="bottom")
?DoHeatmap
...
top_InfectedrmarkersGeneDotPlot<- 
   DotPlot(Pyelonephritis_Infection, features = top_InfectedrmarkersGenes,cols = "Spectral", dot.scale = 4) +RotatedAxis() +theme(legend.text = element_text(size = 12), legend.title = element_text(size = 8))
  #DotPlot(Pyelonephritis_Infection, features = top_InfectedrmarkersGenes)+RotatedAxis() 

ggsave("top_InfectedrmarkersGeneDotPlot.pdf", plot =top_InfectedrmarkersGeneDotPlot, height = 4, width = 12 )
...
for (m in SectionClusters){
  ...
  if( m == "Cortex" | m == "Medulla" | m == "Pelvis" | m== "Processing Infection" | m== "Vascular"){
...
SectionDEGlistGOPlotWithInfectionDotPlot<- dotplot(SectionDEGlistGOPlotWithInfectionSimplifed,font.size = 12,label_format = 80,showCategory = 10) +RotatedAxis()
?dotplot
#low = "LightBlue", midpoint=0, mid="white", high = "FireBrick",
ggsave(paste0("SectionDEGlistGOPlotDotPlot_withinfection.pdf"), plot = SectionDEGlistGOPlotWithInfectionDotPlot, height = 14, width = 14 )
write.csv(SectionDEGlistGOPlotWithInfectionSimplifed, "SectionDEGlistWithInfectionGOPlotSimplifed.csv")

# considering the GO enrichment took a long time, we read the saved files

SectionDEGlistGOPlotWithInfectionSimplifed <- read.csv("SectionDEGlistWithInfectionGOPlotSimplifed.csv", header = T)
...
#saveRDS(acts, file="Pyelonephritis_Infection_Transcriptfactor.RDS")
...
ggsave("Pyelonephritis_InfectionHeatmap_Transcriptfactor_Progeny_Decouple.pdf", plot = IMMTFheatmap, width = 5, height = 5)
...
DefaultAssay(object = Pyelonephritis_Infection) <- "STC"
