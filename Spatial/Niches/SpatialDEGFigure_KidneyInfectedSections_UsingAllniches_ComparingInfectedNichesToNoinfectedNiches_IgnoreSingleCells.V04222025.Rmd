...
BioMarkerDir <- paste0(Outdir,"BiomarkersV2/")
...
# we detect the DEGs comparing between infected dates to non-infected date

# subset dates (day 1 vs control, day 3 vs control, day 5 vs control, day 7 vs control, day 28 vs control)
InfectedDates<-levels(Pyelonephritis_Infection$group)[2:6]
Pyelonephritis_Infection_subDEGsList <-list()
for (i in InfectedDates) {
  Pyelonephritis_Infection_sub <-
    subset(Pyelonephritis_Infection, subset = (group %in% c(i, "PyeloD0_1")))
  Idents(Pyelonephritis_Infection_sub) <- "group"
  # capture the DEGs from this group to the control
  Pyelonephritis_Infection_subDEGs <-
    FindAllMarkers(Pyelonephritis_Infection_sub,
                   min.pct = 0.3,
                   test.use = "MAST")
  Pyelonephritis_Infection_subDEGsList[[i]] <-
    Pyelonephritis_Infection_subDEGs %>% filter(cluster == i &
                                                  p_val_adj <= 0.05 & (avg_log2FC > 1 | avg_log2FC < -1))
}


names(Pyelonephritis_Infection_subDEGsList)

# We integrated the DEGs from the all the DEGs comparing between infected days to controls
SpatialAcrossDpiDEGs <- unique(c(Pyelonephritis_Infection_subDEGsList$PyeloD1_2$gene,Pyelonephritis_Infection_subDEGsList$PyeloD3_1$gene,Pyelonephritis_Infection_subDEGsList$PyeloD5_2$gene,Pyelonephritis_Infection_subDEGsList$PyeloD7_1$gene, Pyelonephritis_Infection_subDEGsList$PyeloD28_1$gene))
length(SpatialAcrossDpiDEGs)
...
# Please note that we did not profile the DEGs using the following datasets:
...
Deggene_list <- list(
  ...
  spatialdatedegs = SpatialAcrossDpiDEGs
)
ggvenn(
  Deggene_list,
...
FinalOverlappingGenes<- DEGsInfectedRegions %>% filter(gene %in% SpatialAcrossDpiDEGs)
...
```{r}
Top30FinalDEGs
# microarray, we should use day 1 vs day 0, day 3 vs day 0 , day7 vs day 0.
MicroDir = "/Users/XXW004/Documents/Projects/BrianBecknell/RishilTwoMice/"

# read the openxlsx (contains the sheets )
#install.packages("openxlsx")
library(openxlsx)
# list sheet names
MicroSheetNames <- getSheetNames(paste0(MicroDir,"Microarray HeN and HeOuJ Kidney DEGs (all genes).xlsx"))
MicroSheetNames
# read specfic sheet
MicroSheetNames[5:7]
Microdata_list<- lapply(MicroSheetNames, function(sheet){
  read.xlsx(paste0(MicroDir,"Microarray HeN and HeOuJ Kidney DEGs (all genes).xlsx"), sheet= sheet)
})
names(Microdata_list) <- MicroSheetNames
MicroSheetNames
MouseMicroArray1dpiDEGs<- Microdata_list$`HeOuJ - Day 1 vs Day 0`%>%  filter(adj.P.Val <=0.05 & (logFC >1 |logFC < -1)) 
MouseMicroArray3dpiDEGs<- Microdata_list$`HeOuJ - Day 3 vs Day 0`%>%  filter(adj.P.Val <=0.05 & (logFC >1 |logFC < -1)) 
MouseMicroArray7dpiDEGs<- Microdata_list$`HeOuJ - Day 7 vs Day 0`%>%  filter(adj.P.Val <=0.05 & (logFC >1 |logFC < -1)) 


# we can get the final DEGs
MouseMicroArrayDEGs <- unique(c(MouseMicroArray1dpiDEGs$mgi_symbol, MouseMicroArray3dpiDEGs$mgi_symbol, MouseMicroArray7dpiDEGs$mgi_symbol))
MouseMicroArrayDEGs
...
FinalOverlappingGenes
FinalOverlappingGenes %>% filter(gene %in% (c("Vcam1","Ccl7","Mmp8","Arg1","Mmp13","Pf4","Ccl3","Ccl4","Ccl5","Cxcl3","Xdh","Il6","Il1b","Il23","Cxcl1","Il1rn","Tnf","Ctss")))
...
Deggene_list <- list(
  ...
  microfinaldegs =MouseMicroArrayDEGs
)

TwoDatasetsOveralapingVenn<- ggvenn(
  Deggene_list,
  fill_color = c("skyblue", "orange"),
  stroke_size = 0.5,
  set_name_size = 5,
...
?ggvenn
ggsave("TwoDatasetsOveralapingVenn_IgnoreSinglecells.pdf",plot = TwoDatasetsOveralapingVenn, height = 3, width =3)
# the overlapping elements
FinalOverallapping<-  Reduce(intersect, Deggene_list)
length(FinalOverallapping)
write.csv(FinalOverallapping, file = "TwoDatasetFinalOverlappingGenes_IgnoreSinglecells.csv")
...
humanensembl <- biomaRt::useMart("ensembl", dataset = "hsapiens_gene_ensembl")
...
# HumanUrinegene_mapping
write.csv(HumanUrinegene_mapping, file = "HumanUrinegene_mapping.csv")
#HumanUrineProteins %>% nrow()
...
MouseHomo_HumanUrinegene[MouseHomo_HumanUrinegene %in% c("Vcam1","Ccl7","Mmp8","Arg1","Mmp13")]
...
FourDatasetsOveralapingVenn<- ggvenn(
...
ggsave( "FourDatasetsOveralapingVenn.pdf", plot = FourDatasetsOveralapingVenn,height = 4, width =6)
...
pdf("CandidateBioMarkers_AcrossDifferentDatasets_ignoreSingleCells.pdf", height = 5, width = 10)
upset(
  input_for_upset,
  set_size.show = TRUE,
...
# In Secreted ∩ Urinary ∩ DEGs but not secreted
CandiateDeggene_listNoSecreted<- list(
  Spatialdegs =FinalOverlappingGenes$gene,
  #spatialdatedegs = DEGsInfectedDays$gene,
  #Singlecelldegs =rownames(SingleCellOverallFinalDEGs),
  Microarraydegs =MouseMicroArrayDEGs,
...
intersect_sud_inurine_notsecreted <- setdiff(Reduce(intersect,CandiateDeggene_listNoSecreted), SecretedMousefinalGenes)
intersect_sud_inurine_notsecreted

# 
CandiateDeggene_listNoUrinary<- list(
  Spatialdegs =FinalOverlappingGenes$gene,
  #spatialdatedegs = DEGsInfectedDays$gene,
  #Singlecelldegs =rownames(SingleCellOverallFinalDEGs),
  Microarraydegs =MouseMicroArrayDEGs,
...
intersect_sud_insecreted_noturine <- setdiff(Reduce(intersect,CandiateDeggene_listNoUrinary), MouseHomo_HumanUrinegene)
intersect_sud_insecreted_noturine
...
AllCategories_Clustermarkers_infectedvsnoninfected %>% head()

SpatialNichesComparison <- AllCategories_Clustermarkers_infectedvsnoninfected %>% filter(cluster == "InfectedNiches") %>% dplyr::select(gene ,avg_log2FC,p_val_adj) %>%mutate(Group = "SpatialNiches") %>% rename("Gene"="gene","log2FoldChange"="avg_log2FC","Padj"="p_val_adj") 
SpatialNichesComparison %>% head()
## Spatial date comparision

# SpatialDateComparison <- AllCategories_Clustermarkers_infectedvsnoninfectedDate %>% filter(cluster == "InfectionDays")  %>% dplyr::select(gene ,avg_log2FC,p_val_adj) %>%mutate(Group = "SpatialDates") %>% rename("Gene"="gene","log2FoldChange"="avg_log2FC","Padj"="p_val_adj")

# seperate into different dates
SpatialDateComparisonD1 <- Pyelonephritis_Infection_subDEGsList$PyeloD1_2 %>% filter(cluster == "PyeloD1_2")  %>% dplyr::select(gene ,avg_log2FC,p_val_adj) %>%mutate(Group = "SpatialDay1") %>% rename("Gene"="gene","log2FoldChange"="avg_log2FC","Padj"="p_val_adj")
SpatialDateComparisonD3 <- Pyelonephritis_Infection_subDEGsList$PyeloD3_1 %>% filter(cluster == "PyeloD3_1")  %>% dplyr::select(gene ,avg_log2FC,p_val_adj) %>%mutate(Group = "SpatialDay3") %>% rename("Gene"="gene","log2FoldChange"="avg_log2FC","Padj"="p_val_adj")
SpatialDateComparisonD5 <- Pyelonephritis_Infection_subDEGsList$PyeloD5_2 %>% filter(cluster == "PyeloD5_2")  %>% dplyr::select(gene ,avg_log2FC,p_val_adj) %>%mutate(Group = "SpatialDay5") %>% rename("Gene"="gene","log2FoldChange"="avg_log2FC","Padj"="p_val_adj")
SpatialDateComparisonD7 <- Pyelonephritis_Infection_subDEGsList$PyeloD7_1 %>% filter(cluster == "PyeloD7_1")  %>% dplyr::select(gene ,avg_log2FC,p_val_adj) %>%mutate(Group = "SpatialDay7") %>% rename("Gene"="gene","log2FoldChange"="avg_log2FC","Padj"="p_val_adj")
SpatialDateComparisonD28 <- Pyelonephritis_Infection_subDEGsList$PyeloD28_1 %>% filter(cluster == "PyeloD28_1")  %>% dplyr::select(gene ,avg_log2FC,p_val_adj) %>%mutate(Group = "SpatialDay28") %>% rename("Gene"="gene","log2FoldChange"="avg_log2FC","Padj"="p_val_adj")
...
MicroArrayComparisonDay1 <- Microdata_list$`HeOuJ - Day 1 vs Day 0` %>% dplyr::select(mgi_symbol ,logFC,adj.P.Val) %>%mutate(Group = "MicroArray_D1") %>% rename("Gene"="mgi_symbol","log2FoldChange"="logFC","Padj"="adj.P.Val")

MicroArrayComparisonDay3 <- Microdata_list$`HeOuJ - Day 3 vs Day 0` %>% dplyr::select(mgi_symbol ,logFC,adj.P.Val) %>%mutate(Group = "MicroArray_D3") %>% rename("Gene"="mgi_symbol","log2FoldChange"="logFC","Padj"="adj.P.Val")

MicroArrayComparisonDay7 <- Microdata_list$`HeOuJ - Day 7 vs Day 0` %>% dplyr::select(mgi_symbol ,logFC,adj.P.Val) %>%mutate(Group = "MicroArray_D7") %>% rename("Gene"="mgi_symbol","log2FoldChange"="logFC","Padj"="adj.P.Val")

#MicroArrayComparison %>% head()

#FinalExpDEGs<- rbind(SpatialNichesComparison,SpatialDateComparison, MicroArrayComparison)

#FinalExpDEGsOriginal<- rbind(SpatialNichesComparison,SpatialDateComparison,SingleCellComparison, MicroArrayComparison)
FinalExpDEGsOriginal<- rbind(SpatialNichesComparison,SpatialDateComparisonD1,SpatialDateComparisonD3,SpatialDateComparisonD5,SpatialDateComparisonD7,SpatialDateComparisonD28, MicroArrayComparisonDay1,MicroArrayComparisonDay3, MicroArrayComparisonDay7 )
FinalExpDEGsOriginal
table(FinalExpDEGsOriginal$Group)
...
RequestedGenes <- FinalOverallapping
setwd(paste0(BioMarkerDir,"ConservedMarkers/"))

write.csv(RequestedGenes,file = paste0(PathwayName, ".csv"))
PathwayName <- "Biomarkers in Secreted not Urine"
RequestedGenes <- intersect_sud_insecreted_noturine
setwd(paste0(BioMarkerDir,"ConservedInSecretedNotUrine/"))
write.csv(RequestedGenes,file = paste0(PathwayName, ".csv"))

PathwayName <- "Biomarkers in Urine not Secreted"
RequestedGenes <- intersect_sud_inurine_notsecreted
setwd(paste0(BioMarkerDir,"ConservedInUrineNotSecreted/"))
write.csv(RequestedGenes,file = paste0(PathwayName, ".csv"))
#setwd(paste0(BioMarkerDir,"ConservedInUrineNotSecreted/"))
#
#PathwayName <- "Biomarkers in Secreted not Urine"
FinalExpDEGs<- FinalExpDEGsOriginal %>% filter(Gene %in% RequestedGenes)
#FinalExpDEGs<- FinalExpDEGsOriginal %>% filter(Gene %in% intersect_sud_inurine_notsecreted)
# FinalExpDEGs<- FinalExpDEGsOriginal %>% filter(Gene %in% intersect_sud_insecreted_noturine)
RequestedGenes
...
FinalExpDEGs$Group <- factor(FinalExpDEGs$Group, levels = c("SpatialNiches", "SpatialDay1","SpatialDay3","SpatialDay5","SpatialDay7","SpatialDay28", "MicroArray_D1","MicroArray_D3","MicroArray_D7"))  
levels(FinalExpDEGs$Group)
#FinalOverallappingOrdered<-c("Lcn2","Clu","Rnase6","Lyz1","S100a9","Il1r2","Tlr2","Ccl9","Ccl6","Smpdl3b","Crlf1","C3ar1","Cybb","Il1rn","Csf1r","Lgals3bp","Cd14","C3","B2m","Ly86","Vcam1","Fgl2","Hp","F10","F13a1","Apom","Cfp","Ctss","Ctsz","Mmp7","Prss8","Timp1","Igfals","Lmnb1","Anxa1","Anxa2","Apmap","Col5a1","Dcn","Emilin2","Fstl1","Flna","Itgb2","Itgb3","Lcp1","Lox","Msn","Pfn1","Tmsb4x","Arpc1b","Plek","Cltrn","Slc2a5","Tcn2","Clic1","Arhgdib","Golm1","Srgn","Aldh9a1","Cmbl","Gusb","Hgd","Napsa","Fam151a")

## we can customize the genes
RequestedGenes_sorted <- SpatialNichesComparison %>%  filter(Gene %in% RequestedGenes) %>% arrange(desc(log2FoldChange)) %>% pull(Gene)

# For conserved
RequestedGenes_sorted<- c("Il1r2", "Tlr2", "Lgals9", "Cybb", "S100a9", "Lcn2", "Ccl9", "Ccl6", "Pf4", "Il1rn", "Pdgfa", "Hp", "Plek", "Cd33", "C3ar1", "Lyz1", "Anxa1", "Csf1r", "Cd14", "Arhgdib", "C3", "B2m", "Ly86", "Unc13d", "Itgb2", "Vcam1", "Itgb3", "Nrp2", "Lcp1", "Olr1", "Mmp9", "Emilin2", "Timp1", "Mmp7", "Spp2", "Col5a1", "Adamts2", "Mmp2", "Lox", "Dcn", "Bgn", "Sparc", "Fbln5", "Ctsk", "F10", "F13a1", "F13b", "F7", "Fgl2", "Cfp", "Afm", "Slc2a5", "Slc12a3", "Folr1", "Tcn2", "Cltrn", "Cmbl", "Smpdl3b", "Hgd", "Napsa", "Sord", "Ddc", "Qsox1", "Enpp6", "Ddah1", "Prss8", "Aldh9a1", "Fah", "Ctss", "Gusb", "Ctsz", "Padi4", "Rnase6", "Apom", "Crlf1", "Igfals", "Ptprd", "Fstl1", "Egf", "Apmap", "Iqgap1", "Sh3bgrl3", "Fam151a", "Sprr2d", "Golm1", "Srgn", "Arpc1b", "Lgals3bp", "Marcks", "Flna", "Clu", "Msn", "Clic1", "Pfn1", "Tmsb4x", "Serpinf1", "Lmnb1","Anxa2")
length(RequestedGenes)
length(RequestedGenes_sorted)
setdiff(RequestedGenes, RequestedGenes_sorted)
FinalExpDEGs$Gene <- factor(FinalExpDEGs$Gene, levels = rev(RequestedGenes_sorted))
#FinalExpDEGs$Gene <- factor(FinalExpDEGs$Gene, levels = rev(FinalOverallapping_sorted))
...
ggsave(paste0(PathwayName, "_FC_HeatMap_inCollecDuct_TwoData.pdf"),plot =RequestedGenesHeatmap, height = 14, width =4.5 )
RequestedGenesHeatmap
...
ggsave(paste0(PathwayName, "_FC_DotPlot_inCollecDuct_DKD.pdf"),plot =RequestedGenesDotPlot, height = 12, width = 4 )
RequestedGenesDotPlot
...
# generate the spatial distribution of these conserved genes 
```{r}
#FinalOverallapping
ConservedBiomarkerDir<- c("/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V04222025/KidneyInfectedVsOther/BiomarkersV2/ConservedMarkers/")
setwd(ConservedBiomarkerDir)

ConservedInUrineNotSecretedDir<-paste0(BioMarkerDir,"ConservedInUrineNotSecreted/")
ConservedInSecretedNotUrineDir<- paste0(BioMarkerDir,"ConservedInSecretedNotUrine/")
setwd(paste0(BioMarkerDir,"ConservedInUrineNotSecreted/"))
setwd(paste0(BioMarkerDir,"ConservedInSecretedNotUrine/"))
FinalOverallappingDotPlot<- 
   DotPlot(Pyelonephritis_Infection, features = RequestedGenes_sorted,cols="RdBu") +
  theme(legend.text = element_text(size = 12), legend.title = element_text(size = 12))+RotatedAxis() + guides(color = guide_colorbar(title = 'AverExp'),
         size = guide_legend(title = "PercExp")) 
  #DotPlot(Pyelonephritis_Infection, features = top_InfectedrmarkersGenes)+RotatedAxis() 

ggsave("FinalOverallappingOrderedGeneDotPlot.pdf", plot =FinalOverallappingDotPlot, height = 5, width = 20 )
ggsave("FinalConservedInUrineNotSecretedDotPlot.pdf", plot =FinalOverallappingDotPlot, height = 5, width = 24 )
FinalUrineSpecificDotPlot<- 
   DotPlot(Pyelonephritis_Infection, features = intersect_sud_inurine_notsecreted,cols="RdBu") +
  theme(legend.text = element_text(size = 12), legend.title = element_text(size = 12))+RotatedAxis() + guides(color = guide_colorbar(title = 'AverExp'),
         size = guide_legend(title = "PercExp")) 
  #DotPlot(Pyelonephritis_Infection, features = top_InfectedrmarkersGenes)+RotatedAxis() 

ggsave("FinalUrineSpecificDotPlot.pdf", plot =FinalUrineSpecificDotPlot, height = 5, width = 30 )

intersect_sud_insecreted_noturine_top20<-intersect_sud_insecreted_noturine_sorted[1:20]
FinalSecretedSpecificTop20DotPlot<- 
   DotPlot(Pyelonephritis_Infection, features = intersect_sud_insecreted_noturine_top20,cols="RdBu") +
  theme(legend.text = element_text(size = 12), legend.title = element_text(size = 12))+RotatedAxis() + guides(color = guide_colorbar(title = 'AverExp'),
         size = guide_legend(title = "PercExp")) 
  #DotPlot(Pyelonephritis_Infection, features = top_InfectedrmarkersGenes)+RotatedAxis() 

ggsave("FinalSecretedSpecificTop20DotPlot.pdf", plot =FinalSecretedSpecificTop20DotPlot, height = 4, width = 6)

#for (j in FinalOverallappingOrdered) {
for (j in intersect_sud_insecreted_noturine_sorted) {
    skip_to_next <- FALSE
    ...
    # ggsave(
    #   filename = paste0(ConservedBiomarkerDir, "/", "Spatial_", j, ".pdf"),
    ...
    # 
     ggsave(
      filename = paste0(ConservedInSecretedNotUrineDir, "/", "Spatial_", j, ".pdf"),
      plot = GenesInGOtermMouseSpatialFeature,
      height = 3,
      width = 15
    )
