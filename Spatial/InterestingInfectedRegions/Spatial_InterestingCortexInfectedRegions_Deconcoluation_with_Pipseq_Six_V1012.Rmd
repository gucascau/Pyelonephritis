...
D7subcluster <- subset (Pyelonephritis_InfectionSubslidesD7,  (slice8_imagerow >110 & slice8_imagerow <200 )  & (slice8_imagecol < 330 & slice8_imagecol> 220))
...
InfectedRegionD7Spatial <-SpatialDimPlot(D7subcluster,label = TRUE, pt.size.factor = 4)
InfectedRegionD7Spatial
...
InfectedMat_Pelvis<- t(D7subcluster[["predictions"]]@data)
...
SpatialScatterPiePelvis <-plotSpatialScatterpie(
  ...
  cell_types = colnames(InfectedMat_PelvisCellType),
    ...
    scale_fill_manual(
        values = pal,
        breaks = names(pal)) +coord_sf(xlim = c(220, 340), ylim = c(150, 280))

SpatialScatterPiePelvis
# subset (Pyelonephritis_InfectionSubslidesD7, (slice8_imagerow >310 & slice8_imagerow <450 )  & (slice8_imagecol < 330 & slice8_imagecol> 220) )
...
Pelvis_nichesBiomarkersTop5Heatmap<- DoHeatmap(D7subcluster, features = Pelvis_nichesBiomarkersTop5 , slot = "scale.data") + scale_fill_gradientn(colors = c("blue", "white", "red")) + theme(axis.text.y = element_text(size = 10))
ggsave(
...
PelvisPelvisPTSubBiomarkersTop5DotPlot <-
  DotPlot(D7subcluster, features = Pelvis_nichesBiomarkersTop5 %>% unique()) +
  RotatedAxis()
...
PelvisCluster<- data.frame(Cell = names(Idents(D7subcluster)),   
