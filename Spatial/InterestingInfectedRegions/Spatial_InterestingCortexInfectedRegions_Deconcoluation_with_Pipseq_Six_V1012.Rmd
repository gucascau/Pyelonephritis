...
date: "2023-10-12"
Description: 
#   The script is to profile the interesting infected Abscess regions including D7 Abscess cortex regions
...
# Output directory :
Outdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V09062023/KidneyInfectedCortexAbscessRegions/"

# Cell types deconvolution:

CDecodir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V09062023/KidneyInfectedCortexAbscessRegions/Deconvolution/"

# BasicSpatialDim

CAbdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V09062023/KidneyInfectedCortexAbscessRegions/BasicSpatialDim/"

# Biomarkers
CBiomadir ="/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V09062023/KidneyInfectedCortexAbscessRegions/CellTypesBiomarkers/"


CDEGdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V09062023/KidneyInfectedCortexAbscessRegions/DEGs/"

# GOEnrichment

CGOEndir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V09062023/KidneyInfectedCortexAbscessRegions/GOenrich/"

# Progeny
CProgenydir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V09062023/KidneyInfectedCortexAbscessRegions/Progeny/"

# Transcript factor
CTFdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V09062023/KidneyInfectedCortexAbscessRegions/TranscriptFactors/"

# Niche niche Communication

CNichComdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V09062023/KidneyInfectedCortexAbscessRegions/NicheCommunicate/"

# Concentrate on the infection area abscess from Cortex

InfCortexdir ="/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V09062023/KidneyInfectedCortexAbscessRegions/"
...
dir.create(InfCortexdir, showWarnings = F)
#dir.create(InfCortexdir, showWarnings = F)
...
Pyelonephritis_Infection <- readRDS(file = paste0(Spindir,"Pyelonephritis_Infection_Deconvoluted.RDS"))
...
Pyelonephritis_Infection[["SCT"]][1:10,1:10]
Pyelonephritis_Infection[["predictions"]][1:10,1:10]
rownames(Pyelonephritis_Infection[["Spatial"]])
colnames(Pyelonephritis_Infection[["Spatial"]])
selectedslices<- c("slice1","slice3","slice4","slice7","slice8","slice9")
Idents(Pyelonephritis_Infection)
...
```
# ############################## 
# only concentrate on the specific abcess regions:
# ############################## 
```{r}
setwd(CAbdir)

# we only select the Day 7 
...
D7subcluster <- subset (Pyelonephritis_InfectionSubslidesD7,  (slice8_imagerow >130 & slice8_imagerow <280 )  & (slice8_imagecol < 280 & slice8_imagecol> 130))
D7subcluster <- subset (D7subcluster, idents = c("N13","N14","N15","N16","N17"))
InfectedRegionD7Spatial <-SpatialDimPlot(D7subcluster,crop = TRUE, label = TRUE, pt.size.factor = 4)
InfectedRegionD7Spatial

ggsave("Spatial_infectedCortex_day7_SpatialDimplot.pdf",
      plot = InfectedRegionD7Spatial,
      height = 3,
      width = 4 )

InfectedRegionD7SpatialDim<- DimPlot(D7subcluster,label = TRUE)
ggsave("Spatial_infectedCortex_day7_Dimplot.pdf",
      plot = InfectedRegionD7SpatialDim,
      height = 3,
      width = 4 )
...
D7subcluster[["predictions"]]
InfectedMat_Cortex<- t(D7subcluster[["predictions"]]@data)
colnames(InfectedMat_Cortex) <- ordered (colnames(InfectedMat_Cortex), levels =CelltypeOrder)

# change the order
InfectedMat_Cortex<- InfectedMat_Cortex[ ,CelltypeOrder]
# for final celltype matrix: 
InfectedMat_CortexCellType<- InfectedMat_Cortex[,!colnames(InfectedMat_Cortex) %in% c("max")]
InfectedMat_CortexCellTypeNozero <- InfectedMat_CortexCellType[,colSums(InfectedMat_CortexCellType)>0]


# correlation
InfectedMat_CortexCellTypeCorrelationMatrix<-plotCorrelationMatrix(InfectedMat_CortexCellTypeNozero)
InfectedMat_CortexCellTypeInteractions<- plotInteractions(InfectedMat_CortexCellTypeNozero, "heatmap") + scale_fill_gradientn(colors = c("blue", "white", "red"))

ggsave("SpatialScatterPieInfectedCortex_CorrelationMatrix.pdf", plot =InfectedMat_CortexCellTypeCorrelationMatrix, height = 6, width = 6 )

ggsave("SpatialScatterPieInfectedCortex_HeatmapColocalization.pdf", plot =InfectedMat_CortexCellTypeInteractions, height = 6, width = 6 )
# scatterpie plot
# scatterpie plot
# 
ct <- colnames(InfectedMat_CortexCellType)
ct
# we did not add the low proportion in the scatterpie
InfectedMat_CortexCellType[InfectedMat_CortexCellType < 0.1] <- 0
paletteMartin <- c(
    "#b6dbff", # HeaPT-S1
    "#99CCFF", # HeaPT-S2
    "#0099FF", # HeaPT-S3
...
names(pal) <- ct
#pal
rownames(D7subcluster)
colnames(D7subcluster)
str(D7subcluster)
#SPOTlight:::. extract_image(Pyelonephritis_Infection )
#rownames(Pyelonephritis_Infection)
SpatialScatterPieCortex <-plotSpatialScatterpie(
  x= D7subcluster,
  y= InfectedMat_CortexCellType,
  cell_types = colnames(InfectedMat_CortexCellType),
    img = TRUE,
    scatterpie_alpha = 1,
    pie_scale = 0.9) +
    scale_fill_manual(
        values = pal,
        breaks = names(pal)) +coord_sf(xlim = c(130, 278), ylim = c(320, 470))

SpatialScatterPieCortex
#D7subcluster <- subset (Pyelonephritis_InfectionSubslidesD7,  (slice8_imagerow >130 & slice8_imagerow <280 )  & (slice8_imagecol < 280 & slice8_imagecol> 130))

ggsave("SpatialScatterPieCortex.pdf",plot = SpatialScatterPieCortex, height = 3,width = 4.5)
# colnames(DeconvolutedMatrix)
# ncol(Pyelonephritis_Infection)
# nrow(DeconvolutedMatrix)
InfectedMat_CortexCellType
pdf(file = "SpatialScatterPieCortex_CellTypeNetwork.pdf", height = 8,width = 8)
plotInteractions(InfectedMat_CortexCellType, "network")
dev.off()
...
Finalcelltypemarkers<- c("Lrp2","Slc34a1","Slc13a3", # Proximal tubule cells (PT)
 ...
 for (i in Finalcelltypemarkers){
   CellTypeMarkerSpatialFeature<- SpatialFeaturePlot(D7subcluster, features = i, pt.size.factor = 4, crop = TRUE)
...
? FindAllMarkers
D7subcluster <- ScaleData(object = D7subcluster, features = rownames(D7subcluster))
# MAST was used to calculate the cluster
Cortex_nichesBiomarkers <-
  FindAllMarkers(
    object = D7subcluster,
...
write.csv(Cortex_nichesBiomarkers, "Cortex_nichesBiomarkers_Mast.csv")
...
Cortex_nichesBiomarkersTop5 <-
  as.data.frame(Cortex_nichesBiomarkers) %>% filter (avg_log2FC > 0.25) %>% group_by(cluster) %>% slice_max(order_by = avg_log2FC, n = 5) %>% pull (gene)

Cortex_nichesBiomarkersTop5 %>% unique()

Cortex_nichesBiomarkersTop5Heatmap<- DoHeatmap(D7subcluster, features = Cortex_nichesBiomarkersTop5 , slot = "scale.data") + scale_fill_gradientn(colors = c("blue", "white", "red")) + theme(axis.text.y = element_text(size = 10))
ggsave(
  "Cortex_nichesBiomarkersTop5_heatmap.pdf",
  plot = Cortex_nichesBiomarkersTop5Heatmap,
  height = 5,
  width = 5
)


CortexCortexPTSubBiomarkersTop5DotPlot <-
  DotPlot(D7subcluster, features = Cortex_nichesBiomarkersTop5 %>% unique()) +
  RotatedAxis()

ggsave(
  "CortexCortexPTSubBiomarkersTop5DotPlot.pdf",
  plot = CortexCortexPTSubBiomarkersTop5DotPlot,
  height = 4,
  width = 8
)
...
Top5PTMarker <-
  as.data.frame(Cortex_nichesBiomarkers) %>% 
...
for (i in Top5PTMarker) {
  ...
  PTmarkerSpatialFeature <-
    SpatialFeaturePlot(D7subcluster, features = i, slot = "scale.data",pt.size.factor = 4)
...
Cortex_nichesBiomarkers %>% head()
Cortex_infectionSpecifcUpgeneFreqFigure <-
  table(Cortex_nichesBiomarkers$cluster) %>% as.data.frame() %>%
  ggplot(aes(x = Var1, y = Freq)) + geom_bar(stat = "identity") + theme_classic() +
  xlab("Cortex Niches") + ylab("Differentially expressed gene number")

Cortex_infectionSpecifcUpgeneFreqFigure
ggsave(
  "Cortex_nichesBiomarkersnumbersFigure.pdf",
  plot = Cortex_infectionSpecifcUpgeneFreqFigure,
  height = 3,
  width = 3.5
)

# Create a GO comparison for the figure
Cortex_infectionSpecifcUpgene <- list()

Idents(D7subcluster)
for (n in Idents(D7subcluster)) {
  # put he DEG into the up-regulated gene for comparision
  Cortex_infectionSpecifcUpgene[[n]] <-
    Cortex_nichesBiomarkers %>% filter(cluster %in% n &
                                        avg_log2FC >= 0.25) %>% pull(gene)
  #}
}
names(Cortex_infectionSpecifcUpgene)
Cortex_infectionSpecifcUpgene$N13%>% head()
Cortex_infectionSpecifcUpgene$N14%>% head()
Cortex_infectionSpecifcUpgene$N15%>% head()
Cortex_infectionSpecifcUpgene$N16%>% head()
Cortex_infectionSpecifcUpgene$N17%>% head()


Cortex_infectionSpecifcUpgeneReorder<- Cortex_infectionSpecifcUpgene[c("N13","N14","N15","N16","N17")]
# change the name order: 
#names(Cortex_infectionSpecifcUpgene) <- c("N8","N10","N11","N12","N15")

Cortex_infectionSpecifcUpgeneReorder$N13%>% head()
Cortex_infectionSpecifcUpgeneReorder$N14%>% head()
Cortex_infectionSpecifcUpgeneReorder$N15%>% head()
Cortex_infectionSpecifcUpgeneReorder$N16%>% head()
Cortex_infectionSpecifcUpgeneReorder$N17%>% head()


# Comparing the Cluster
Cortex_infectionSpecifcUpgeneGOenrich <-
  clusterProfiler::compareCluster(
    geneClusters = Cortex_infectionSpecifcUpgeneReorder,
    fun = "enrichGO",
    OrgDb = "org.Mm.eg.db",
    ont = "BP",
    keyType = 'SYMBOL'
...
write.csv(Cortex_infectionSpecifcUpgeneGOenrich,
          "Cortex_infectionSpecifcUpgeneGOenrich.csv")

#Cortex_infectionSpecifcUpgeneGOenrich
#Cortex_infectionSpecifcUpgeneGOenrich@compareClusterResult %>% head()

# Here we only concentrate on selected features
selectedPathway<- c("regulation of innate immune response","regulation of adaptive immune response","response to lipopolysaccharide","defense response to bacterium","neutrophil activation", "pattern recognition receptor signaling pathway","phagocytosis", "I-kappaB kinase/NF-kappaB signaling","receptor signaling pathway via STAT","tumor necrosis factor production","T cell differentiation","extracellular matrix organization", "epithelium migration","organic acid catabolic process","sodium ion transport","nephron epithelium development","Wnt signaling pathway","leukocyte cell-cell adhesion","DNA replication","chromatin remodeling","DNA recombination")


Cortex_infectionSpecifcUpgeneGOenrichDotPlot <-
  dotplot(
    Cortex_infectionSpecifcUpgeneGOenrich,
    font.size = 12,
    label_format = 80,
...
ggsave(
  "Cortex_infectionSpecifcUpgeneGOenrichDotPlot.pdf",
  plot = Cortex_infectionSpecifcUpgeneGOenrichDotPlot,
  height = 7,
  width = 7.5
)
...
CortexCluster<- data.frame(Cell = names(Idents(D7subcluster)),   
                              CellType = as.character(Idents(D7subcluster)),
                              stringsAsFactors = FALSE)

?progeny
D7subcluster <- progeny(D7subcluster, scale=FALSE, organism="Mouse", top=500, perm=1, return_assay = TRUE,assay_name = "SCT")
D7subcluster
# setup the default assay as progeny
DefaultAssay(object = D7subcluster) <- "progeny"

D7subcluster <- Seurat::ScaleData(D7subcluster, assay = "progeny") 
## We can now directly apply Seurat functions in our Progeny scores. 
## For instance, we scale the pathway activity scores. 
progeny_scores_df <- 
  as.data.frame(t(GetAssayData(D7subcluster, slot = "scale.data", 
                               assay = "progeny"))) %>%
...
progeny_scores_df <- inner_join(progeny_scores_df, CortexCluster)
...
CortexForHeatmap<-as.data.frame(t(summarized_progeny_scores_df[,-1]))

# generate the heatmap for PT
pdf(file="CortexForHeatmap_Progeny_ForHeatmap.pdf",height = 3, width = 3.5)
progeny_hmap = pheatmap(CortexForHeatmap,fontsize=8, 
...
print(progeny_hmap)
dev.off()
test<- FeaturePlot(D7subcluster, features = "NFkB")
...
for (i in ProgenyFeatures ){
  p2 <-(FeaturePlot(D7subcluster, features = i) & 
  scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red'))& 
  ggtitle(paste0(i," activity"))
  
  ggsave(paste0(i, "_CortexInfected_cluster_featurePlot.pdf"), plot = p2, width = 6, height = 6)
  
  p3 <- (SpatialFeaturePlot(D7subcluster, features = i, pt.size.factor = 4) +ggtitle(paste0(i," activity")))
  ggsave(paste0(i, "_CortexInfected_cluster_SpatialfeaturePlot.pdf"), plot = p3, width = 6, height = 6)
...
D7subcluster@assays$SCT@data %>% head()
mat <- as.matrix(D7subcluster@assays$SCT@data)
# Run ulm
acts <- run_ulm(mat=mat, net=netTF, .source='source', .target='target',
                .mor='mor', minsize = 5)

# save the transcript factors
saveRDS(acts, file="Cortex_Transcriptfactor.RDS")


#acts <- readRDS("PT_Transcriptfactor.RDS")
# Activity inference with Univariate Linear Model (ULM)
# Extract ulm and store it in tfsulm in pbmc
D7subcluster[['tfsulm']] <- acts %>%
...
# Change assay
DefaultAssay(object = D7subcluster) <- "tfsulm"

# Scale the data
D7subcluster <- ScaleData(D7subcluster)
D7subcluster@assays$tfsulm@data <- D7subcluster@assays$tfsulm@scale.data

# Explore the top 20 more varaible TFs
n_tfs <- 20

t(as.matrix(D7subcluster@assays$tfsulm@data))
# Extract activities from object as a long dataframe
df <- t(as.matrix(D7subcluster@assays$tfsulm@data)) %>%
  as.data.frame() %>%
  mutate(cluster = Idents(D7subcluster)) %>%
...
ggsave("CortexInfectionHeatmap_Transcriptfactor_Progeny_Decouple.pdf", plot = PTTFheatmap, width = 3, height = 4)

# draw the transcriptor factor activity and their expression
# generate the figure of transcript factor activity:
for (m in tfs){
  p2 <- (SpatialFeaturePlot(D7subcluster, features = m,pt.size.factor = 4) +
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(m, ' activity')))
  
  ggsave(paste0("TFActivity_",m,"_CortexInfection_SpatialFeaturePlot.pdf"), plot = p2, width = 6, height = 6)
}

D7subcluster
DefaultAssay(object = D7subcluster) <- "SCT"

for (n in tfs){
  p3 <- (FeaturePlot(D7subcluster, features = n,slot= "scale.data") + 
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression')))
  
  p4 <- (SpatialFeaturePlot(D7subcluster, features = n, pt.size.factor = 4, slot= "scale.data") +
 # scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression (time)')))
 
  ggsave(paste0("TFExpression_",n,"_D7subcluster_FeaturePlot.pdf"), plot = p3, width = 6, height = 6)
  ggsave(paste0("TFExpression_",n,"_D7subcluster_SpatilFeaturePlot.pdf"), plot = p4, width = 6, height = 6)
...
GeneOrderByPheatmap
?DotPlot
TFDotPlot <- DotPlot(D7subcluster, features = rev(GeneOrderByPheatmap)) +
  RotatedAxis() + coord_flip()+ 
  #ggtitle("Predicted TF expression")+
   theme(legend.position = "right")
ggsave("Cortex_TFDotPlot.pdf", plot = TFDotPlot, width = 5, height = 6)
...
```{r}
DefaultAssay(D7subcluster) <- "SCT"
D7subcluster@meta.data %>% head()
#Idents(D7subcluster) <- D7subcluster@meta.data$Sub2

# reset the identity, Combine the PT cells

Idents(D7subcluster)
D7subcluster@meta.data
# adjust the levels
# Idents(D7subcluster) <-
# ordered(factor(Idents(D7subcluster)), levels=c( "5_1","10_1","10_0","2_1","2_0","5_0","5_2","4","7","0","1","9","6","3","8"))

levels(Idents(D7subcluster))
```
...
D7subcluster = alias_to_symbol_seurat(D7subcluster, "mouse")

D7subcluster@meta.data %>% head()
D7subcluster@meta.data$IntegrateAnnot %>% table() 

head(weighted_networks$gr) 
DimPlot(D7subcluster)

# If you use convert_to_alias before here, this one won't work
D7subcluster = Seurat::PrepSCTFindMarkers(D7subcluster, assay = "SCT", verbose = FALSE)
...
lr_network %>% head()
D7subcluster %>% subset(features = lr_network$ligand %>% intersect(rownames(D7subcluster)))
# calcualte the niches difference 
DE_sender = nichenetr::calculate_niche_de(seurat_obj  = D7subcluster %>% subset(features = lr_network$ligand %>% intersect(rownames(D7subcluster))), niches = niches, type = "sender", assay_oi = "SCT") # only ligands important for sender cell types
DE_receiver = nichenetr::calculate_niche_de(seurat_obj  = D7subcluster %>% subset(features = lr_network$receptor %>% unique()), niches = niches, type = "receiver", assay_oi = "SCT") # only receptors now, later on: DE analysis to find targets
...
DE_receiver_targets = nichenetr::calculate_niche_de_targets(seurat_obj = D7subcluster, niches = niches, lfc_cutoff = lfc_cutoff, expression_pct = expression_pct, assay_oi = "SCT") 
...
avg_expression_ligands = AverageExpression(D7subcluster, features = nichenet_output$top_ligands)
...
# #######
# For interesting infection regions : for the abscess of the Cortex
# we generate pie plot for day7 under infection regions for the Cortex abscess regions
```{r}
setwd(InfCortexdir)
Pyelonephritis_Infection$orig.ident
Pyelonephritis_InfectionSubslidesD7 <-
...
D7subcluster <- subset (Pyelonephritis_InfectionSubslidesD7, slice8_imagerow > 140 | slice8_imagecol > 200, invert = TRUE)
D7subcluster <- subset (D7subcluster, idents = c("N13","N14","N15","N16","N17"))
