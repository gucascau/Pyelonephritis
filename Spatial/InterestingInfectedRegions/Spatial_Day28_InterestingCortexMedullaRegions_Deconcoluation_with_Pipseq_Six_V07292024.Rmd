...
# Output directory :
Outdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD28CorMed/"

# Cell types deconvolution:

CDecodir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD28CorMed/Deconvolution/"

# BasicSpatialDim

CAbdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD28CorMed/BasicSpatialDim/"

# Biomarkers
CBiomadir ="/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD28CorMed/CellTypesBiomarkers/"


CDEGdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD28CorMed/DEGs/"

# GOEnrichment

CGOEndir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD28CorMed/GOenrich/"

# Progeny
CProgenydir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD28CorMed/Progeny/"

# Transcript factor
CTFdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD28CorMed/TranscriptFactors/"

# Niche niche Communication

CNichComdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD28CorMed/NicheCommunicate/"

# Concentrate on the infection area abscess from CorMed

InfCorMeddir ="/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD28CorMed/"
...
```
# ############################## 
# only concentrate on the specific infected regions with neutrophil and macro around the abcess regions at day 7
...
D28CorMedsubcluster <- subset (Pyelonephritis_InfectionSubslidesD28,(slice9_imagerow >100 & slice9_imagerow <330 )  & (slice9_imagecol < 500 & slice9_imagecol> 280))
SpatialDimPlot(D28CorMedsubcluster, pt.size.factor = 1500)
D28CorMedsubcluster <- subset (D28CorMedsubcluster, idents = c("N13","N14","N15","N16","N17"))
InfectedCorMedRegionD28Spatial <-SpatialDimPlot(D28CorMedsubcluster,label = TRUE, pt.size.factor = 1500)
?SpatialDimPlot
InfectedCorMedRegionD28Spatial
ggsave("Spatial_infectedPelivis_day28_SpatialDimplot.pdf",
      plot = InfectedCorMedRegionD28Spatial,
      height = 3,
      width = 4 )

InfectedCorMedRegionD28SpatialDim<- DimPlot(D28CorMedsubcluster,label = TRUE)
ggsave("Spatial_infectedPelivis_day28_Dimplot.pdf",
      plot = InfectedCorMedRegionD28SpatialDim,
      height = 3,
      width = 4 )
InfectedCorMedRegionD28SpatialDim
...
CelltypeOrder <- c("PT-S1S2-1","PT-S1S2-2","PT-S3-1","PT-S3-2", "PT-S1S2-N1",  "PT-S1S2-N2","PT-S1S2-N3","InfPT","ProfPT","PC1","PC2","ICA" ,"ICB", "DegCD", "DTL", "TAL","DCT-TAL"  ,"CNT","SMC" ,"URO", "DegURO","FIB" , "END","PL",  "ResMac" , "MONa","MONb", "MACa", "MACb", "NEU"  ,"T" ,"B","NK" )
 ...
 ordered((rownames(D28CorMedsubcluster[["predictions"]]@data)),levels =CelltypeOrder)
 
# get the summary of the selected regions
 GetTissueCoordinates(D28CorMedsubcluster)
 summary(GetTissueCoordinates(D28CorMedsubcluster))
```
# ############################## 
...
D28CorMedsubcluster[["predictions"]]
InfectedMat_CorMed<- t(D28CorMedsubcluster[["predictions"]]@data)
...
names(pal) <- ct
#pal
rownames(D28CorMedsubcluster)
colnames(D28CorMedsubcluster)
str(D28CorMedsubcluster)
...
SpatialScatterPieCorMed <-plotSpatialScatterpie(
  ...
  cell_types = colnames(InfectedMat_CorMedCellType),
    ...
    scale_fill_manual(
        values = pal,
        breaks = names(pal)) + coord_sf(xlim = c(280,480), ylim = c(270, 480))
...
DefaultAssay(D28CorMedsubcluster) <-"SCT"
Finalcelltypemarkers<- c("Lrp2","Slc34a1","Slc13a3", # Proximal tubule cells (PT)
 ...
 for (i in Finalcelltypemarkers){
   CellTypeMarkerSpatialFeature<- SpatialFeaturePlot(D28CorMedsubcluster, features = i, pt.size.factor = 6000, crop = TRUE)
...
? FindAllMarkers
D28CorMedsubcluster <- ScaleData(object = D28CorMedsubcluster, features = rownames(D28CorMedsubcluster))
# MAST was used to calculate the cluster
CorMed_nichesBiomarkers <-
  FindAllMarkers(
    object = D28CorMedsubcluster,
...
CorMed_nichesBiomarkersTop5Heatmap<- DoHeatmap(D28CorMedsubcluster, features = CorMed_nichesBiomarkersTop5 , slot = "scale.data") + scale_fill_gradientn(colors = c("blue", "white", "red")) + theme(axis.text.y = element_text(size = 10))
ggsave(
...
CorMedCorMedPTSubBiomarkersTop5DotPlot <-
  DotPlot(D28CorMedsubcluster, features = CorMed_nichesBiomarkersTop5 %>% unique()) +
  RotatedAxis()
...
for (i in Top5PTMarker) {
  ...
  PTmarkerSpatialFeature <-
    SpatialFeaturePlot(D28CorMedsubcluster, features = i, slot = "scale.data",pt.size.factor = 4)
...
Idents(D28CorMedsubcluster)
for (n in Idents(D28CorMedsubcluster)) {
...
CorMedCluster<- data.frame(Cell = names(Idents(D28CorMedsubcluster)),   
                              CellType = as.character(Idents(D28CorMedsubcluster)),
                              stringsAsFactors = FALSE)

?progeny
D28CorMedsubcluster <- progeny(D28CorMedsubcluster, scale=FALSE, organism="Mouse", top=500, perm=1, return_assay = TRUE,assay_name = "SCT")
D28CorMedsubcluster
# setup the default assay as progeny
DefaultAssay(object = D28CorMedsubcluster) <- "progeny"

D28CorMedsubcluster <- Seurat::ScaleData(D28CorMedsubcluster, assay = "progeny") 
## We can now directly apply Seurat functions in our Progeny scores. 
## For instance, we scale the pathway activity scores. 
progeny_scores_df <- 
  as.data.frame(t(GetAssayData(D28CorMedsubcluster, slot = "scale.data", 
                               assay = "progeny"))) %>%
...
progeny_hmap
dev.off()
test<- FeaturePlot(D28CorMedsubcluster, features = "NFkB")
...
for (i in ProgenyFeatures ){
  p2 <-(FeaturePlot(D28CorMedsubcluster, features = i) & 
  scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red'))& 
  ggtitle(paste0(i," activity"))
  
  ggsave(paste0(i, "_CorMedInfected_cluster_featurePlot.pdf"), plot = p2, width = 6, height = 6)
  
  p3 <- (SpatialFeaturePlot(D28CorMedsubcluster, features = i, pt.size.factor = 4) +ggtitle(paste0(i," activity")))
...
D28CorMedsubcluster@assays$SCT@data %>% head()
mat <- as.matrix(D28CorMedsubcluster@assays$SCT@data)
# Run ulm
...
D28CorMedsubcluster[['tfsulm']] <- acts %>%
...
# Change assay
DefaultAssay(object = D28CorMedsubcluster) <- "tfsulm"

# Scale the data
D28CorMedsubcluster <- ScaleData(D28CorMedsubcluster)
D28CorMedsubcluster@assays$tfsulm@data <- D28CorMedsubcluster@assays$tfsulm@scale.data

# Explore the top 20 more varaible TFs
n_tfs <- 20

t(as.matrix(D28CorMedsubcluster@assays$tfsulm@data))
# Extract activities from object as a long dataframe
df <- t(as.matrix(D28CorMedsubcluster@assays$tfsulm@data)) %>%
  as.data.frame() %>%
  mutate(cluster = Idents(D28CorMedsubcluster)) %>%
...
for (m in tfs){
  p2 <- (SpatialFeaturePlot(D28CorMedsubcluster, features = m,pt.size.factor = 4) +
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
...
D28CorMedsubcluster
DefaultAssay(object = D28CorMedsubcluster) <- "SCT"

for (n in tfs){
  p3 <- (FeaturePlot(D28CorMedsubcluster, features = n,slot= "scale.data") + 
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression')))
  
  p4 <- (SpatialFeaturePlot(D28CorMedsubcluster, features = n, pt.size.factor = 4, slot= "scale.data") +
 # scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression (time)')))
 
  ggsave(paste0("TFExpression_",n,"_D28CorMedsubcluster_FeaturePlot.pdf"), plot = p3, width = 6, height = 6)
  ggsave(paste0("TFExpression_",n,"_D28CorMedsubcluster_SpatilFeaturePlot.pdf"), plot = p4, width = 6, height = 6)
...
GeneOrderByPheatmap
?DotPlot
TFDotPlot <- DotPlot(D28CorMedsubcluster, features = rev(GeneOrderByPheatmap)) +
...
```{r}
DefaultAssay(D28CorMedsubcluster) <- "SCT"
D28CorMedsubcluster@meta.data %>% head()
#Idents(D28CorMedsubcluster) <- D28CorMedsubcluster@meta.data$Sub2

# reset the identity, Combine the PT cells

Idents(D28CorMedsubcluster)
D28CorMedsubcluster@meta.data
# adjust the levels
# Idents(D28CorMedsubcluster) <-
# ordered(factor(Idents(D28CorMedsubcluster)), levels=c( "5_1","10_1","10_0","2_1","2_0","5_0","5_2","4","7","0","1","9","6","3","8"))

levels(Idents(D28CorMedsubcluster))
```
...
D28CorMedsubcluster = alias_to_symbol_seurat(D28CorMedsubcluster, "mouse")

D28CorMedsubcluster@meta.data %>% head()
D28CorMedsubcluster@meta.data$IntegrateAnnot %>% table() 

head(weighted_networks$gr) 
DimPlot(D28CorMedsubcluster)

# If you use convert_to_alias before here, this one won't work
D28CorMedsubcluster = Seurat::PrepSCTFindMarkers(D28CorMedsubcluster, assay = "SCT", verbose = FALSE)
...
lr_network %>% head()
D28CorMedsubcluster %>% subset(features = lr_network$ligand %>% intersect(rownames(D28CorMedsubcluster)))
# calcualte the niches difference 
DE_sender = nichenetr::calculate_niche_de(seurat_obj  = D28CorMedsubcluster %>% subset(features = lr_network$ligand %>% intersect(rownames(D28CorMedsubcluster))), niches = niches, type = "sender", assay_oi = "SCT") # only ligands important for sender cell types
DE_receiver = nichenetr::calculate_niche_de(seurat_obj  = D28CorMedsubcluster %>% subset(features = lr_network$receptor %>% unique()), niches = niches, type = "receiver", assay_oi = "SCT") # only receptors now, later on: DE analysis to find targets
...
DE_receiver_targets = nichenetr::calculate_niche_de_targets(seurat_obj = D28CorMedsubcluster, niches = niches, lfc_cutoff = lfc_cutoff, expression_pct = expression_pct, assay_oi = "SCT") 
...
nichenet_output = nichenet_seuratobj_aggregate(
  seurat_obj = D28CorMedsubcluster, 
  receiver = "PT", 
...
avg_expression_ligands = AverageExpression(D28CorMedsubcluster, features = nichenet_output$top_ligands)
...
D28CorMedsubcluster <- subset (Pyelonephritis_InfectionSubslidesD28, slice8_imagerow > 140 | slice8_imagecol > 200, invert = TRUE)
D28CorMedsubcluster <- subset (D28CorMedsubcluster, idents = c("N13","N14","N15","N16","N17"))
InfectedCorMedRegionD1Spatial <-SpatialDimPlot(D28CorMedsubcluster,crop = TRUE, label = FALSE)
...
FinalIntergratedTableAbcess<- merge(D28CorMedsubcluster@meta.data[,c("orig.ident","Annotation")], t(D28CorMedsubcluster[["predictions"]]@data), all=TRUE, by="row.names") 
?Heatmap
...
pal <- colorRampPalette(paletteMartin)(length(ct))
names(pal) <- ct
D28CorMedsubclusterCellTypeMatrix <-t(D28CorMedsubcluster[["predictions"]]@data)

D28CorMedsubclusterCellTypeMatrixNoMax <- D28CorMedsubclusterCellTypeMatrix[,!colnames(D28CorMedsubclusterCellTypeMatrix) %in% c("max")]
D28CorMedsubclusterCellTypeMatrixNoMax
# generate the scatter pie plot
colnames(D28CorMedsubclusterCellTypeMatrixNoMax)
...
InfectedCorMedRegionD1SpatialPie<- plotSpatialScatterpie(
  x= D28CorMedsubcluster,
  y= D28CorMedsubclusterCellTypeMatrixNoMax,
  cell_types = colnames(D28CorMedsubclusterCellTypeMatrixNoMax),
    img = TRUE,
    scatterpie_alpha = 1,
...
# correlation
InfectedCorMedRegionD1SpatialPieCorrelationMatrix <- plotCorrelationMatrix(D28CorMedsubclusterCellTypeMatrixNoMax)
ggsave("Spatial_infected_day28_InfectedCorMedRegionD1SpatialPieCorrelationMatrix.pdf",
      plot = InfectedCorMedRegionD1SpatialPieCorrelationMatrix,
      height = 4.5,
      width = 4.5 )
# we only select the matrix that rowsum >0
D28CorMedsubclusterCellTypeMatrixNoMaxLarger<- D28CorMedsubclusterCellTypeMatrixNoMax[,colSums(D28CorMedsubclusterCellTypeMatrixNoMax[])>0]
InfectedPevRegionD1Interactions<- plotInteractions(D28CorMedsubclusterCellTypeMatrixNoMaxLarger, "heatmap") + scale_fill_gradientn(colors = c("blue", "white", "red"))
?plotInteractions
...
D28CorMedsubcluster <- subset (Pyelonephritis_InfectionSubslidesD28, (slice8_imagerow >310 & slice8_imagerow <450 )  & (slice8_imagecol < 330 & slice8_imagecol> 220) )
D28CorMedsubcluster <- subset (D28CorMedsubcluster, idents = c("N8","N10","N11","N12","N15"))
InfectedCorMedRegionD1Spatial <-SpatialDimPlot(D28CorMedsubcluster,label = TRUE)
InfectedCorMedRegionD1Spatial
ggsave("Spatial_infected_day28_SpatialDimplot.pdf",
      plot = InfectedCorMedRegionD1Spatial,
      height = 3,
...
InfectedCorMedRegionD1Spatial
D28CorMedsubcluster
DefaultAssay(D28CorMedsubcluster) <- "integrated"
nbs_2.markers <- FindAllMarkers(D28CorMedsubcluster,test.use ="MAST")
nbs_2.markers
??SubsetSTData
#nbs_2.markers$gene <- rownames(nbs_2.markers)

nbs_2.markers
#se.subset <- SubsetSTData(D28CorMedsubcluster, expression = nbs_2 %in% c("N12", "N15"))
...
DotPlot(D28CorMedsubcluster, features = sorted.marks$gene %>% unique()) +RotatedAxis()
#DoHeatmap(D28CorMedsubcluster, features = sorted.marks$gene, group.colors = c("red", "lightgray"))
