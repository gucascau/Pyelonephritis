...
date: "2024-07-29"
Description: 
#   This script is to profile the interesting infected Abscess regions at Day 7 for the pelvic regions
...
# Output directory :
Outdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedPelvicD7/"

# Cell types deconvolution:

CDecodir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedPelvicD7/Deconvolution/"

# BasicSpatialDim

CAbdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedPelvicD7/BasicSpatialDim/"

# Biomarkers
CBiomadir ="/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedPelvicD7/CellTypesBiomarkers/"


CDEGdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedPelvicD7/DEGs/"

# GOEnrichment

CGOEndir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedPelvicD7/GOenrich/"

# Progeny
CProgenydir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedPelvicD7/Progeny/"

# Transcript factor
CTFdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedPelvicD7/TranscriptFactors/"

# Niche niche Communication

CNichComdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedPelvicD7/NicheCommunicate/"

# Concentrate on the infection area abscess from CorMed

InfCorMeddir ="/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedPelvicD7/"
...
D7Pelsubcluster <- subset (Pyelonephritis_InfectionSubslidesD7,subset= (slice8_imagerow >310 & slice8_imagerow <450 )  & (slice8_imagecol < 330 & slice8_imagecol> 220))
#D7subcluster <- subset (D7subcluster, idents = c("N8","N10","N11","N12","N15"))
SpatialDimPlot(D7Pelsubcluster, pt.size.factor = 1500)
D7Pelsubcluster <- subset (D7Pelsubcluster, idents =  c("N8","N10","N11","N12","N15"))
InfectedPelRegionD7Spatial <-SpatialDimPlot(D7Pelsubcluster,label = TRUE, pt.size.factor = 3500)
?SpatialDimPlot
InfectedPelRegionD7Spatial
ggsave("Spatial_infectedPelivis_day7_SpatialDimplot.pdf",
      plot = InfectedPelRegionD7Spatial,
      height = 3,
      width = 4 )

InfectedPelRegionD7SpatialDim<- DimPlot(D7Pelsubcluster,label = TRUE)
ggsave("Spatial_infectedPelivis_day7_Dimplot.pdf",
      plot = InfectedPelRegionD7SpatialDim,
      height = 3,
      width = 4 )
InfectedPelRegionD7SpatialDim
...
CelltypeOrder <- c("PT-S1S2-1","PT-S1S2-2","PT-S3-1","PT-S3-2", "PT-S1S2-N1",  "PT-S1S2-N2","PT-S1S2-N3","InfPT","ProfPT","PC1","PC2","ICA" ,"ICB", "DegCD", "DTL", "TAL","DCT-TAL"  ,"CNT","SMC" ,"URO", "DegURO","FIB" , "END","PL",  "ResMac" , "MONa","MONb", "MACa", "MACb", "NEU"  ,"T" ,"B","NK" )
 ...
 ordered((rownames(D7Pelsubcluster[["predictions"]]@data)),levels =CelltypeOrder)
 
# get the summary of the selected regions
 GetTissueCoordinates(D7Pelsubcluster)
 summary(GetTissueCoordinates(D7Pelsubcluster))
```
# ############################## 
...
D7Pelsubcluster[["predictions"]]
InfectedMat_CorMed<- t(D7Pelsubcluster[["predictions"]]@data)
...
names(pal) <- ct
#pal
rownames(D7Pelsubcluster)
colnames(D7Pelsubcluster)
str(D7Pelsubcluster)
...
DefaultAssay(D7Pelsubcluster) <-"SCT"
Finalcelltypemarkers<- c("Lrp2","Slc34a1","Slc13a3", # Proximal tubule cells (PT)
 ...
 for (i in Finalcelltypemarkers){
   CellTypeMarkerSpatialFeature<- SpatialFeaturePlot(D7Pelsubcluster, features = i, pt.size.factor = 6000, crop = TRUE)
...
Seurat::SpatialDimPlot(D7Pelsubcluster,pt.size.factor = 1500)
# Prepare input data for CelChat analysis
data.input = Seurat::GetAssayData(D7Pelsubcluster, slot = "data", assay = "SCT") # normalized data matrix
#> Warning: The `slot` argument of `GetAssayData()` is deprecated as of SeuratObject 5.0.0.
Seurat::Idents(D7Pelsubcluster)
...
meta = data.frame(labels = Seurat::Idents(D7Pelsubcluster), samples = "sample1", row.names = names(Seurat::Idents(D7Pelsubcluster))) # manually create a dataframe consisting of the cell labels
meta
...
spatial.locs = Seurat::GetTissueCoordinates(D7Pelsubcluster, scale = NULL, cols = c("imagerow", "imagecol")) 
...
pdf(file="Spatial_D7_Pelvic_netVisual_circle_strength.pdf",height = 5, width = 4)
netVisual_circle(cellchat@net$weight, vertex.weight = rowSums(cellchat@net$weight), weight.scale = T, label.edge= F, title.name = "Interaction weights/strength")
dev.off()
#library(ComplexHeatmap)
pdf(file="Spatial_D7_Pelvic_NetVisual_interaction_counts.pdf",height = 5, width = 4)
...
# save t
pdf(file="Spatial_D7_Pelvic_TNFSignalingNetVisual.pdf",height = 7, width = 5)
...
dev.off()
CandidatePathways<- c("CXCL","CCL","TNF","IL1","IL2","IL4","IL6","IL12","IL16")
#CandidatePathways <-cellchat@netP$pathways

pdf(file="Spatial_D7_Pelvic_InnateImmuneSignalingNetVisual.pdf",height = 6, width = 4)
netAnalysis_contribution(cellchat, signaling = CandidatePathways)
dev.off()
saveRDS(cellchat, file = "cellchat_Spatial_D7_Pelvic.rds")


# read the cell chat object
cellchat@netP
#cellchat <- readRDS("cellchat_Spatial_D7_Pelvic.rds")
pdf(file="Spatial_D7_Pelvic_InnateImmuneSignalingnetVisual_bubble.pdf",height = 4.5, width = 4)
netVisual_bubble(cellchat, sources.use = 4, signaling = CandidatePathways, remove.isolate = FALSE)
dev.off()
# For the TNF signaling
pdf(file="Spatial_D7_Pelvic_TnfspatialFeature.pdf",height = 4.5, width = 4)
spatialFeaturePlot(cellchat, features = c("Tnf","Tnfrsf1a"), point.size = 1.2, color.heatmap = "Reds", direction = 1)
dev.off()

pdf(file="Spatial_D7_Pelvic_Il1bspatialFeature.pdf",height = 4.5, width = 4)
...
? FindAllMarkers
D7Pelsubcluster <- ScaleData(object = D7Pelsubcluster, features = rownames(D7Pelsubcluster))
# MAST was used to calculate the cluster
CorMed_nichesBiomarkers <-
  FindAllMarkers(
    object = D7Pelsubcluster,
...
CorMed_nichesBiomarkersTop5Heatmap<- DoHeatmap(D7Pelsubcluster, features = CorMed_nichesBiomarkersTop5 , slot = "scale.data") + scale_fill_gradientn(colors = c("blue", "white", "red")) + theme(axis.text.y = element_text(size = 10))
ggsave(
...
CorMedCorMedPTSubBiomarkersTop5DotPlot <-
  DotPlot(D7Pelsubcluster, features = CorMed_nichesBiomarkersTop5 %>% unique()) +
  RotatedAxis()
...
for (i in Top5PTMarker) {
  ...
  PTmarkerSpatialFeature <-
    SpatialFeaturePlot(D7Pelsubcluster, features = i, slot = "scale.data",pt.size.factor = 4)
...
Idents(D7Pelsubcluster)
for (n in Idents(D7Pelsubcluster)) {
...
CorMedCluster<- data.frame(Cell = names(Idents(D7Pelsubcluster)),   
                              CellType = as.character(Idents(D7Pelsubcluster)),
                              stringsAsFactors = FALSE)

?progeny
D7Pelsubcluster <- progeny(D7Pelsubcluster, scale=FALSE, organism="Mouse", top=500, perm=1, return_assay = TRUE,assay_name = "SCT")
D7Pelsubcluster
# setup the default assay as progeny
DefaultAssay(object = D7Pelsubcluster) <- "progeny"

D7Pelsubcluster <- Seurat::ScaleData(D7Pelsubcluster, assay = "progeny") 
## We can now directly apply Seurat functions in our Progeny scores. 
## For instance, we scale the pathway activity scores. 
progeny_scores_df <- 
  as.data.frame(t(GetAssayData(D7Pelsubcluster, slot = "scale.data", 
                               assay = "progeny"))) %>%
...
progeny_hmap
dev.off()
test<- FeaturePlot(D7Pelsubcluster, features = "NFkB")
...
for (i in ProgenyFeatures ){
  p2 <-(FeaturePlot(D7Pelsubcluster, features = i) & 
  scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red'))& 
  ggtitle(paste0(i," activity"))
  
  ggsave(paste0(i, "_CorMedInfected_cluster_featurePlot.pdf"), plot = p2, width = 6, height = 6)
  
  p3 <- (SpatialFeaturePlot(D7Pelsubcluster, features = i, pt.size.factor = 4) +ggtitle(paste0(i," activity")))
...
D7Pelsubcluster@assays$SCT@data %>% head()
mat <- as.matrix(D7Pelsubcluster@assays$SCT@data)
# Run ulm
...
# Change assay
DefaultAssay(object = D7Pelsubcluster) <- "tfsulm"

# Scale the data
D7Pelsubcluster <- ScaleData(D7Pelsubcluster)
D7Pelsubcluster@assays$tfsulm@data <- D7Pelsubcluster@assays$tfsulm@scale.data

# Explore the top 20 more varaible TFs
n_tfs <- 20

t(as.matrix(D7Pelsubcluster@assays$tfsulm@data))
# Extract activities from object as a long dataframe
df <- t(as.matrix(D7Pelsubcluster@assays$tfsulm@data)) %>%
  as.data.frame() %>%
  mutate(cluster = Idents(D7Pelsubcluster)) %>%
...
for (m in tfs){
  p2 <- (SpatialFeaturePlot(D7Pelsubcluster, features = m,pt.size.factor = 4) +
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
...
D7Pelsubcluster
DefaultAssay(object = D7Pelsubcluster) <- "SCT"

for (n in tfs){
  p3 <- (FeaturePlot(D7Pelsubcluster, features = n,slot= "scale.data") + 
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression')))
  
  p4 <- (SpatialFeaturePlot(D7Pelsubcluster, features = n, pt.size.factor = 4, slot= "scale.data") +
 # scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression (time)')))
 
  ggsave(paste0("TFExpression_",n,"_D7Pelsubcluster_FeaturePlot.pdf"), plot = p3, width = 6, height = 6)
  ggsave(paste0("TFExpression_",n,"_D7Pelsubcluster_SpatilFeaturePlot.pdf"), plot = p4, width = 6, height = 6)
...
GeneOrderByPheatmap
?DotPlot
TFDotPlot <- DotPlot(D7Pelsubcluster, features = rev(GeneOrderByPheatmap)) +
...
```{r}
DefaultAssay(D7Pelsubcluster) <- "SCT"
D7Pelsubcluster@meta.data %>% head()
#Idents(D7Pelsubcluster) <- D7Pelsubcluster@meta.data$Sub2

# reset the identity, Combine the PT cells

Idents(D7Pelsubcluster)
D7Pelsubcluster@meta.data
# adjust the levels
# Idents(D7Pelsubcluster) <-
# ordered(factor(Idents(D7Pelsubcluster)), levels=c( "5_1","10_1","10_0","2_1","2_0","5_0","5_2","4","7","0","1","9","6","3","8"))

levels(Idents(D7Pelsubcluster))
```
...
D7Pelsubcluster = alias_to_symbol_seurat(D7Pelsubcluster, "mouse")

D7Pelsubcluster@meta.data %>% head()
D7Pelsubcluster@meta.data$IntegrateAnnot %>% table() 

head(weighted_networks$gr) 
DimPlot(D7Pelsubcluster)

# If you use convert_to_alias before here, this one won't work
D7Pelsubcluster = Seurat::PrepSCTFindMarkers(D7Pelsubcluster, assay = "SCT", verbose = FALSE)
...
lr_network %>% head()
D7Pelsubcluster %>% subset(features = lr_network$ligand %>% intersect(rownames(D7Pelsubcluster)))
# calcualte the niches difference 
DE_sender = nichenetr::calculate_niche_de(seurat_obj  = D7Pelsubcluster %>% subset(features = lr_network$ligand %>% intersect(rownames(D7Pelsubcluster))), niches = niches, type = "sender", assay_oi = "SCT") # only ligands important for sender cell types
DE_receiver = nichenetr::calculate_niche_de(seurat_obj  = D7Pelsubcluster %>% subset(features = lr_network$receptor %>% unique()), niches = niches, type = "receiver", assay_oi = "SCT") # only receptors now, later on: DE analysis to find targets
...
DE_receiver_targets = nichenetr::calculate_niche_de_targets(seurat_obj = D7Pelsubcluster, niches = niches, lfc_cutoff = lfc_cutoff, expression_pct = expression_pct, assay_oi = "SCT") 
...
avg_expression_ligands = AverageExpression(D7Pelsubcluster, features = nichenet_output$top_ligands)
...
D7Pelsubcluster <- subset (Pyelonephritis_InfectionSubslidesD7, slice8_imagerow > 140 | slice8_imagecol > 200, invert = TRUE)
D7Pelsubcluster <- subset (D7Pelsubcluster, idents = c("N13","N14","N15","N16","N17"))
InfectedCorMedRegionD1Spatial <-SpatialDimPlot(D7Pelsubcluster,crop = TRUE, label = FALSE)
...
FinalIntergratedTableAbcess<- merge(D7Pelsubcluster@meta.data[,c("orig.ident","Annotation")], t(D7Pelsubcluster[["predictions"]]@data), all=TRUE, by="row.names") 
?Heatmap
...
pal <- colorRampPalette(paletteMartin)(length(ct))
names(pal) <- ct
D7PelsubclusterCellTypeMatrix <-t(D7Pelsubcluster[["predictions"]]@data)

D7PelsubclusterCellTypeMatrixNoMax <- D7PelsubclusterCellTypeMatrix[,!colnames(D7PelsubclusterCellTypeMatrix) %in% c("max")]
D7PelsubclusterCellTypeMatrixNoMax
# generate the scatter pie plot
colnames(D7PelsubclusterCellTypeMatrixNoMax)
...
InfectedCorMedRegionD1SpatialPie<- plotSpatialScatterpie(
  x= D7Pelsubcluster,
  y= D7PelsubclusterCellTypeMatrixNoMax,
  cell_types = colnames(D7PelsubclusterCellTypeMatrixNoMax),
    img = TRUE,
    scatterpie_alpha = 1,
...
# correlation
InfectedCorMedRegionD1SpatialPieCorrelationMatrix <- plotCorrelationMatrix(D7PelsubclusterCellTypeMatrixNoMax)
...
D7PelsubclusterCellTypeMatrixNoMaxLarger<- D7PelsubclusterCellTypeMatrixNoMax[,colSums(D7PelsubclusterCellTypeMatrixNoMax[])>0]
InfectedPevRegionD1Interactions<- plotInteractions(D7PelsubclusterCellTypeMatrixNoMaxLarger, "heatmap") + scale_fill_gradientn(colors = c("blue", "white", "red"))
?plotInteractions
...
D7Pelsubcluster <- subset (Pyelonephritis_InfectionSubslidesD7, (slice8_imagerow >310 & slice8_imagerow <450 )  & (slice8_imagecol < 330 & slice8_imagecol> 220) )
D7Pelsubcluster <- subset (D7Pelsubcluster, idents = c("N8","N10","N11","N12","N15"))
InfectedCorMedRegionD1Spatial <-SpatialDimPlot(D7Pelsubcluster,label = TRUE)
InfectedCorMedRegionD1Spatial
ggsave("Spatial_infected_day7_SpatialDimplot.pdf",
      plot = InfectedCorMedRegionD1Spatial,
      height = 3,
...
InfectedCorMedRegionD1Spatial
D7Pelsubcluster
DefaultAssay(D7Pelsubcluster) <- "integrated"
nbs_2.markers <- FindAllMarkers(D7Pelsubcluster,test.use ="MAST")
nbs_2.markers
??SubsetSTData
#nbs_2.markers$gene <- rownames(nbs_2.markers)

nbs_2.markers
#se.subset <- SubsetSTData(D7Pelsubcluster, expression = nbs_2 %in% c("N12", "N15"))
...
DotPlot(D7Pelsubcluster, features = sorted.marks$gene %>% unique()) +RotatedAxis()
#DoHeatmap(D7Pelsubcluster, features = sorted.marks$gene, group.colors = c("red", "lightgray"))
