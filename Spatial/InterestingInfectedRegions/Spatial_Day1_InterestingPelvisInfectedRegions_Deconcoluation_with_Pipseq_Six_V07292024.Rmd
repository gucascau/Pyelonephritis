...
# Output directory :
Outdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedRegionsD1Pelvis/"

# Cell types deconvolution:

CDecodir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedRegionsD1Pelvis/Deconvolution/"

# BasicSpatialDim

CAbdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedRegionsD1Pelvis/BasicSpatialDim/"

# Biomarkers
CBiomadir ="/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedRegionsD1Pelvis/CellTypesBiomarkers/"


CDEGdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedRegionsD1Pelvis/DEGs/"

# GOEnrichment

CGOEndir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedRegionsD1Pelvis/GOenrich/"

# Progeny
CProgenydir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedRegionsD1Pelvis/Progeny/"

# Transcript factor
CTFdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedRegionsD1Pelvis/TranscriptFactors/"

# Niche niche Communication

CNichComdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedRegionsD1Pelvis/NicheCommunicate/"

# Concentrate on the infection area abscess from pelvis

InfPelvisdir ="/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedRegionsD1Pelvis/"
...
```
# ############################## 
# only concentrate on the specific infected regions with neutrophil and macro at day 1
# Pelvic region
# ############################## 
```{r}
setwd(CAbdir)
...
Pyelonephritis_InfectionSubslidesD1 <-
    subset (Pyelonephritis_Infection, orig.ident == "PyeloD1_2")
#names(Pyelonephritis_InfectionSubslidesD1@images)
  # there seems to be an error in the susetting, so we removed other igmages
  #names(Pyelonephritis_InfectionSubslides@images)
SelectedImageName <- names(Pyelonephritis_InfectionSubslidesD1@images)
SelectedImageName
OtherImages <- Allimagmes[Allimagmes != SelectedImageName]
OtherImages
Pyelonephritis_InfectionSubslidesD1@images[OtherImages] = NULL
Pyelonephritis_InfectionSubslidesD1@meta.data %>% head()
# for a specific regions and specific clusters
# get the coordate of these cluster
coords <- GetTissueCoordinates(Pyelonephritis_InfectionSubslidesD1)
coords %>% head()
Pyelonephritis_InfectionSubslidesD1@meta.data %>% head()
summary(coords)
Pyelonephritis_InfectionSubslidesD1@images$slice3

SpatialDimPlot(Pyelonephritis_InfectionSubslidesD1,pt.size.factor = 1500 )
#DefaultAssay(Pyelonephritis_InfectionSubslidesD1) <-"Spatial"
# we then select the regions for the cluster : , invert = TRUE
D1Pelsubcluster <- subset (Pyelonephritis_InfectionSubslidesD1, slice3_imagerow > 250 &slice3_imagerow < 290 & slice3_imagecol >260  &slice3_imagecol <320)
SpatialDimPlot(D1Pelsubcluster, pt.size.factor = 1500)
D1Pelsubcluster <- subset (D1Pelsubcluster, idents = c("N8","N10","N11","N12","N15"))
InfectedPevRegionD1Spatial <-SpatialDimPlot(D1Pelsubcluster,label = TRUE, pt.size.factor = 1500)
?SpatialDimPlot
InfectedPevRegionD1Spatial
ggsave("Spatial_infectedPelivis_day1_SpatialDimplot.pdf",
      plot = InfectedPevRegionD1Spatial,
      height = 3,
      width = 4 )

InfectedPevRegionD1SpatialDim<- DimPlot(D1Pelsubcluster,label = TRUE)
ggsave("Spatial_infectedPelivis_day7_Dimplot.pdf",
      plot = InfectedPevRegionD1SpatialDim,
      height = 3,
      width = 4 )
InfectedPevRegionD1SpatialDim
...
CelltypeOrder <- c("PT-S1S2-1","PT-S1S2-2","PT-S3-1","PT-S3-2", "PT-S1S2-N1",  "PT-S1S2-N2","PT-S1S2-N3","InfPT","ProfPT","PC1","PC2","ICA" ,"ICB", "DegCD", "DTL", "TAL","DCT-TAL"  ,"CNT","SMC" ,"URO", "DegURO","FIB" , "END","PL",  "ResMac" , "MONa","MONb", "MACa", "MACb", "NEU"  ,"T" ,"B","NK" )
 ...
 ordered((rownames(D1Pelsubcluster[["predictions"]]@data)),levels =CelltypeOrder)
 
# get the summary of the selected regions
 GetTissueCoordinates(D1Pelsubcluster)
 summary(GetTissueCoordinates(D1Pelsubcluster))
```
# ############################## 
...
D1Pelsubcluster[["predictions"]]
InfectedMat_Pelvis<- t(D1Pelsubcluster[["predictions"]]@data)
colnames(InfectedMat_Pelvis) <- ordered (colnames(InfectedMat_Pelvis), levels =CelltypeOrder)

# change the order
InfectedMat_Pelvis<- InfectedMat_Pelvis[ ,CelltypeOrder]
# for final celltype matrix: 
InfectedMat_PelvisCellType<- InfectedMat_Pelvis[,!colnames(InfectedMat_Pelvis) %in% c("max")]
InfectedMat_PelvisCellTypeNozero <- InfectedMat_PelvisCellType[,colSums(InfectedMat_PelvisCellType)>0]


# correlation
InfectedMat_PelvisCellTypeCorrelationMatrix<-plotCorrelationMatrix(InfectedMat_PelvisCellTypeNozero)
InfectedMat_PelvisCellTypeCorrelationMatrix
InfectedMat_PelvisCellTypeInteractions<- plotInteractions(InfectedMat_PelvisCellTypeNozero, "heatmap") + scale_fill_gradientn(colors = c("blue", "white", "red"))

ggsave("SpatialScatterPieInfectedPelvis_CorrelationMatrix.pdf", plot =InfectedMat_PelvisCellTypeCorrelationMatrix, height = 6, width = 6 )

ggsave("SpatialScatterPieInfectedPelvis_HeatmapColocalization.pdf", plot =InfectedMat_PelvisCellTypeInteractions, height = 6, width = 6 )
# scatterpie plot
# scatterpie plot
# 
ct <- colnames(InfectedMat_PelvisCellType)
ct
# we did not add the low proportion in the scatterpie, we here also consider the low 
#InfectedMat_PelvisCellType[InfectedMat_PelvisCellType < 0.1] <- 0
...
names(pal) <- ct
#pal
rownames(D1Pelsubcluster)
colnames(D1Pelsubcluster)
str(D1Pelsubcluster)
#SPOTlight:::. extract_image(Pyelonephritis_Infection )
#rownames(Pyelonephritis_Infection)
SpatialScatterPiePelvis <-plotSpatialScatterpie(
  x= D1Pelsubcluster,
  y= InfectedMat_PelvisCellType,
  cell_types = colnames(InfectedMat_PelvisCellType),
    img = TRUE,
    scatterpie_alpha = 1,
    pie_scale = 2) +
    scale_fill_manual(
        values = pal,
        breaks = names(pal)) +coord_sf(xlim = c(260,315), ylim = c(300, 350))

SpatialScatterPiePelvis
# slice3_imagerow > 250 &slice3_imagerow < 290 & slice3_imagecol >260  &slice3_imagecol <320)

ggsave("SpatialScatterPiePelvis.pdf",plot = SpatialScatterPiePelvis, height = 3,width = 4.5)
# colnames(DeconvolutedMatrix)
# ncol(Pyelonephritis_Infection)
# nrow(DeconvolutedMatrix)
InfectedMat_PelvisCellType
pdf(file = "SpatialScatterPiePelvis_CellTypeNetwork.pdf", height = 8,width = 8)
plotInteractions(InfectedMat_PelvisCellType, "network")
dev.off()
...
DefaultAssay(D1Pelsubcluster) <-"SCT"
Finalcelltypemarkers<- c("Lrp2","Slc34a1","Slc13a3", # Proximal tubule cells (PT)
 ...
 for (i in Finalcelltypemarkers){
   CellTypeMarkerSpatialFeature<- SpatialFeaturePlot(D1Pelsubcluster, features = i, pt.size.factor = 6000, crop = TRUE)
...
? FindAllMarkers
D1Pelsubcluster <- ScaleData(object = D1Pelsubcluster, features = rownames(D1Pelsubcluster))
# MAST was used to calculate the cluster
Pelvis_nichesBiomarkers <-
  FindAllMarkers(
    object = D1Pelsubcluster,
    logfc.threshold = 0.25,
    test.use = "MAST",
    min.pct = 0.25
  ) # only markers that are in 25% of cells
# save all biomarkers as a dataframe
write.csv(Pelvis_nichesBiomarkers, "Pelvis_nichesBiomarkers_Mast.csv")
...
Pelvis_nichesBiomarkersTop5 <-
  as.data.frame(Pelvis_nichesBiomarkers) %>% filter (avg_log2FC > 0.25) %>% group_by(cluster) %>% slice_max(order_by = avg_log2FC, n = 5) %>% pull (gene)

Pelvis_nichesBiomarkersTop5 %>% unique()

Pelvis_nichesBiomarkersTop5Heatmap<- DoHeatmap(D1Pelsubcluster, features = Pelvis_nichesBiomarkersTop5 , slot = "scale.data") + scale_fill_gradientn(colors = c("blue", "white", "red")) + theme(axis.text.y = element_text(size = 10))
ggsave(
  "Pelvis_nichesBiomarkersTop5_heatmap.pdf",
  plot = Pelvis_nichesBiomarkersTop5Heatmap,
  height = 5,
  width = 5
)


PelvisPelvisPTSubBiomarkersTop5DotPlot <-
  DotPlot(D1Pelsubcluster, features = Pelvis_nichesBiomarkersTop5 %>% unique()) +
  RotatedAxis()

ggsave(
  "PelvisPelvisPTSubBiomarkersTop5DotPlot.pdf",
  plot = PelvisPelvisPTSubBiomarkersTop5DotPlot,
  height = 4,
  width = 8
)
...
Top5PTMarker <-
  as.data.frame(Pelvis_nichesBiomarkers) %>% 
...
for (i in Top5PTMarker) {
  ...
  PTmarkerSpatialFeature <-
    SpatialFeaturePlot(D1Pelsubcluster, features = i, slot = "scale.data",pt.size.factor = 4)
...
Pelvis_nichesBiomarkers %>% head()
Pelvis_infectionSpecifcUpgeneFreqFigure <-
  table(Pelvis_nichesBiomarkers$cluster) %>% as.data.frame() %>%
  ggplot(aes(x = Var1, y = Freq)) + geom_bar(stat = "identity") + theme_classic() +
  xlab("Pelvis Niches") + ylab("Differentially expressed gene number")

Pelvis_infectionSpecifcUpgeneFreqFigure
ggsave(
  "Pelvis_nichesBiomarkersnumbersFigure.pdf",
  plot = Pelvis_infectionSpecifcUpgeneFreqFigure,
  height = 3,
  width = 3.5
)

# Create a GO comparison for the figure
Pelvis_infectionSpecifcUpgene <- list()

Idents(D1Pelsubcluster)
for (n in Idents(D1Pelsubcluster)) {
  # put he DEG into the up-regulated gene for comparision
  Pelvis_infectionSpecifcUpgene[[n]] <-
    Pelvis_nichesBiomarkers %>% filter(cluster %in% n &
                                        avg_log2FC >= 0.25) %>% pull(gene)
  #}
}
Pelvis_infectionSpecifcUpgene$N8%>% head()
Pelvis_infectionSpecifcUpgene$N10%>% head()
Pelvis_infectionSpecifcUpgene$N11%>% head()
Pelvis_infectionSpecifcUpgene$N12%>% head()
Pelvis_infectionSpecifcUpgene$N15%>% head()

Pelvis_infectionSpecifcUpgeneReorder<- Pelvis_infectionSpecifcUpgene[c("N8","N10","N11","N12","N15")]
# change the name order: 
#names(Pelvis_infectionSpecifcUpgene) <- c("N8","N10","N11","N12","N15")

Pelvis_infectionSpecifcUpgeneReorder$N8%>% head()
Pelvis_infectionSpecifcUpgeneReorder$N10%>% head()
Pelvis_infectionSpecifcUpgeneReorder$N11%>% head()
Pelvis_infectionSpecifcUpgeneReorder$N12%>% head()
Pelvis_infectionSpecifcUpgeneReorder$N15%>% head()

# Comparing the Cluster
Pelvis_infectionSpecifcUpgeneGOenrich <-
  clusterProfiler::compareCluster(
    geneClusters = Pelvis_infectionSpecifcUpgeneReorder,
    fun = "enrichGO",
    OrgDb = "org.Mm.eg.db",
    ont = "BP",
    keyType = 'SYMBOL'
...
write.csv(Pelvis_infectionSpecifcUpgeneGOenrich,
          "Pelvis_infectionSpecifcUpgeneGOenrich.csv")

#Pelvis_infectionSpecifcUpgeneGOenrich
#Pelvis_infectionSpecifcUpgeneGOenrich@compareClusterResult %>% head()
...
Pelvis_infectionSpecifcUpgeneGOenrichDotPlot <-
  dotplot(
    Pelvis_infectionSpecifcUpgeneGOenrich,
    font.size = 12,
    label_format = 80,
...
ggsave(
  "Pelvis_infectionSpecifcUpgeneGOenrichDotPlot.pdf",
  plot = Pelvis_infectionSpecifcUpgeneGOenrichDotPlot,
  height = 7,
  width = 7.5
)
...
PelvisCluster<- data.frame(Cell = names(Idents(D1Pelsubcluster)),   
                              CellType = as.character(Idents(D1Pelsubcluster)),
                              stringsAsFactors = FALSE)

?progeny
D1Pelsubcluster <- progeny(D1Pelsubcluster, scale=FALSE, organism="Mouse", top=500, perm=1, return_assay = TRUE,assay_name = "SCT")
D1Pelsubcluster
# setup the default assay as progeny
DefaultAssay(object = D1Pelsubcluster) <- "progeny"

D1Pelsubcluster <- Seurat::ScaleData(D1Pelsubcluster, assay = "progeny") 
## We can now directly apply Seurat functions in our Progeny scores. 
## For instance, we scale the pathway activity scores. 
progeny_scores_df <- 
  as.data.frame(t(GetAssayData(D1Pelsubcluster, slot = "scale.data", 
                               assay = "progeny"))) %>%
...
progeny_scores_df <- inner_join(progeny_scores_df, PelvisCluster)
...
PelvisForHeatmap<-as.data.frame(t(summarized_progeny_scores_df[,-1]))

# generate the heatmap for PT
pdf(file="PelvisForHeatmap_Progeny_ForHeatmap.pdf",height = 3, width = 3.5)
progeny_hmap = pheatmap(PelvisForHeatmap,fontsize=8, 
...
progeny_hmap
dev.off()
test<- FeaturePlot(D1Pelsubcluster, features = "NFkB")
...
for (i in ProgenyFeatures ){
  p2 <-(FeaturePlot(D1Pelsubcluster, features = i) & 
  scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red'))& 
  ggtitle(paste0(i," activity"))
  
  ggsave(paste0(i, "_PelvisInfected_cluster_featurePlot.pdf"), plot = p2, width = 6, height = 6)
  
  p3 <- (SpatialFeaturePlot(D1Pelsubcluster, features = i, pt.size.factor = 4) +ggtitle(paste0(i," activity")))
  ggsave(paste0(i, "_PelvisInfected_cluster_SpatialfeaturePlot.pdf"), plot = p3, width = 6, height = 6)
...
D1Pelsubcluster@assays$SCT@data %>% head()
mat <- as.matrix(D1Pelsubcluster@assays$SCT@data)
# Run ulm
acts <- run_ulm(mat=mat, net=netTF, .source='source', .target='target',
                .mor='mor', minsize = 5)


saveRDS(acts, file="Pelvis_Transcriptfactor.RDS")


#acts <- readRDS("PT_Transcriptfactor.RDS")
# Activity inference with Univariate Linear Model (ULM)
# Extract ulm and store it in tfsulm in pbmc
D1Pelsubcluster[['tfsulm']] <- acts %>%
...
# Change assay
DefaultAssay(object = D1Pelsubcluster) <- "tfsulm"

# Scale the data
D1Pelsubcluster <- ScaleData(D1Pelsubcluster)
D1Pelsubcluster@assays$tfsulm@data <- D1Pelsubcluster@assays$tfsulm@scale.data

# Explore the top 20 more varaible TFs
n_tfs <- 20

t(as.matrix(D1Pelsubcluster@assays$tfsulm@data))
# Extract activities from object as a long dataframe
df <- t(as.matrix(D1Pelsubcluster@assays$tfsulm@data)) %>%
  as.data.frame() %>%
  mutate(cluster = Idents(D1Pelsubcluster)) %>%
...
ggsave("PelvisInfectionHeatmap_Transcriptfactor_Progeny_Decouple.pdf", plot = PTTFheatmap, width = 3, height = 4)

# draw the transcriptor factor activity and their expression
# generate the figure of transcript factor activity:
for (m in tfs){
  p2 <- (SpatialFeaturePlot(D1Pelsubcluster, features = m,pt.size.factor = 4) +
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(m, ' activity')))
  
  ggsave(paste0("TFActivity_",m,"_PelvisInfection_SpatialFeaturePlot.pdf"), plot = p2, width = 6, height = 6)
}

D1Pelsubcluster
DefaultAssay(object = D1Pelsubcluster) <- "SCT"

for (n in tfs){
  p3 <- (FeaturePlot(D1Pelsubcluster, features = n,slot= "scale.data") + 
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression')))
  
  p4 <- (SpatialFeaturePlot(D1Pelsubcluster, features = n, pt.size.factor = 4, slot= "scale.data") +
 # scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression (time)')))
 
  ggsave(paste0("TFExpression_",n,"_D1Pelsubcluster_FeaturePlot.pdf"), plot = p3, width = 6, height = 6)
  ggsave(paste0("TFExpression_",n,"_D1Pelsubcluster_SpatilFeaturePlot.pdf"), plot = p4, width = 6, height = 6)
...
GeneOrderByPheatmap
?DotPlot
TFDotPlot <- DotPlot(D1Pelsubcluster, features = rev(GeneOrderByPheatmap)) +
  RotatedAxis() + coord_flip()+ 
  #ggtitle("Predicted TF expression")+
   theme(legend.position = "right")
ggsave("Pelvis_TFDotPlot.pdf", plot = TFDotPlot, width = 5, height = 6)
...
```{r}
DefaultAssay(D1Pelsubcluster) <- "SCT"
D1Pelsubcluster@meta.data %>% head()
#Idents(D1Pelsubcluster) <- D1Pelsubcluster@meta.data$Sub2

# reset the identity, Combine the PT cells

Idents(D1Pelsubcluster)
D1Pelsubcluster@meta.data
# adjust the levels
# Idents(D1Pelsubcluster) <-
# ordered(factor(Idents(D1Pelsubcluster)), levels=c( "5_1","10_1","10_0","2_1","2_0","5_0","5_2","4","7","0","1","9","6","3","8"))

levels(Idents(D1Pelsubcluster))
```
...
D1Pelsubcluster = alias_to_symbol_seurat(D1Pelsubcluster, "mouse")

D1Pelsubcluster@meta.data %>% head()
D1Pelsubcluster@meta.data$IntegrateAnnot %>% table() 

head(weighted_networks$gr) 
DimPlot(D1Pelsubcluster)

# If you use convert_to_alias before here, this one won't work
D1Pelsubcluster = Seurat::PrepSCTFindMarkers(D1Pelsubcluster, assay = "SCT", verbose = FALSE)
...
lr_network %>% head()
D1Pelsubcluster %>% subset(features = lr_network$ligand %>% intersect(rownames(D1Pelsubcluster)))
# calcualte the niches difference 
DE_sender = nichenetr::calculate_niche_de(seurat_obj  = D1Pelsubcluster %>% subset(features = lr_network$ligand %>% intersect(rownames(D1Pelsubcluster))), niches = niches, type = "sender", assay_oi = "SCT") # only ligands important for sender cell types
DE_receiver = nichenetr::calculate_niche_de(seurat_obj  = D1Pelsubcluster %>% subset(features = lr_network$receptor %>% unique()), niches = niches, type = "receiver", assay_oi = "SCT") # only receptors now, later on: DE analysis to find targets
...
DE_receiver_targets = nichenetr::calculate_niche_de_targets(seurat_obj = D1Pelsubcluster, niches = niches, lfc_cutoff = lfc_cutoff, expression_pct = expression_pct, assay_oi = "SCT") 
...
avg_expression_ligands = AverageExpression(D1Pelsubcluster, features = nichenet_output$top_ligands)
...
#nrow(Pyelonephritis_InfectionSubslidesD1@meta.data )
...
Pyelonephritis_InfectionSubslidesD1
  # there seems to be an error in the susetting, so we removed other igmages
  #names(Pyelonephritis_InfectionSubslides@images)
SelectedImageName <- names(Pyelonephritis_InfectionSubslidesD1@images)[5]
SelectedImageName
OtherImages <- Allimagmes[Allimagmes != SelectedImageName]
OtherImages
Pyelonephritis_InfectionSubslidesD1@images[OtherImages] = NULL
...
#nrow(Pyelonephritis_InfectionSubslidesD1@meta.data )
Pyelonephritis_InfectionSubslidesD1Dimplot <-SpatialDimPlot(Pyelonephritis_InfectionSubslidesD1, crop = T)

ggsave("Pyelonephritis_InfectionSubslidesD1Dimplot.pdf", plot =Pyelonephritis_InfectionSubslidesD1Dimplot, height = 6, width = 6 )

# the prediction of the object
Pyelonephritis_InfectionSubslidesD1[["predictions"]]
InfectedMat_D7<- t(Pyelonephritis_InfectionSubslidesD1[["predictions"]]@data)
...
names(pal) <- ct
#pal
rownames(Pyelonephritis_InfectionSubslidesD1)
colnames(Pyelonephritis_InfectionSubslidesD1)
str(Pyelonephritis_InfectionSubslidesD1)
#SPOTlight:::. extract_image(Pyelonephritis_Infection )
#rownames(Pyelonephritis_Infection)
SpatialScatterPieDay7 <-plotSpatialScatterpie(
  x= Pyelonephritis_InfectionSubslidesD1,
...
Idents(Pyelonephritis_InfectionSubslidesD1)

# generate the Dotplot for the biomarkers
DefaultAssay(Pyelonephritis_InfectionSubslidesD1) <-"SCT"
Finalcelltypemarkers<- c("Lrp2","Slc34a1","Slc13a3", # Proximal tubule cells (PT)
                      "Slc12a3","Pvalb","Wnk1",# Distal connecting tublue (DCT)
...
SpatialD7MarkersDotPlot<- DotPlot(Pyelonephritis_InfectionSubslidesD1, features = Finalcelltypemarkers,cols = "Spectral", dot.scale = 4) +RotatedAxis() +theme(legend.text = element_text(size = 12), legend.title = element_text(size = 8))

ggsave("SpatialD7MarkersDotPlot.pdf", plot = SpatialD7MarkersDotPlot, height = 6, width = 14)
Pyelonephritis_InfectionSubslidesD1

for (i in Finalcelltypemarkers){
   CellTypeMarkerSpatialFeature<- SpatialFeaturePlot(Pyelonephritis_InfectionSubslidesD1, features = i, pt.size.factor = 1.6, crop = TRUE) 
   #&
       #   ggplot2::scale_fill_gradientn(colours = myPalette(4), limits=c(0,1))
...
#nrow(Pyelonephritis_InfectionSubslidesD1@meta.data )
SpatialDimPlot(Pyelonephritis_InfectionSubslidesD1)
...
D1Pelsubcluster <- subset (Pyelonephritis_InfectionSubslidesD7, slice8_imagerow > 140 | slice8_imagecol > 200, invert = TRUE)
D1Pelsubcluster <- subset (D1Pelsubcluster, idents = c("N13","N14","N15","N16","N17"))
InfectedPevRegionD1Spatial <-SpatialDimPlot(D1Pelsubcluster,crop = TRUE, label = FALSE)
InfectedPevRegionD1Spatial
...
FinalIntergratedTableAbcess<- merge(D1Pelsubcluster@meta.data[,c("orig.ident","Annotation")], t(D1Pelsubcluster[["predictions"]]@data), all=TRUE, by="row.names") 
?Heatmap
...
pal <- colorRampPalette(paletteMartin)(length(ct))
names(pal) <- ct
D1PelsubclusterCellTypeMatrix <-t(D1Pelsubcluster[["predictions"]]@data)

D1PelsubclusterCellTypeMatrixNoMax <- D1PelsubclusterCellTypeMatrix[,!colnames(D1PelsubclusterCellTypeMatrix) %in% c("max")]
D1PelsubclusterCellTypeMatrixNoMax
# generate the scatter pie plot
colnames(D1PelsubclusterCellTypeMatrixNoMax)
#D0subclusterCellTypeMatrixNoMax[D0subclusterCellTypeMatrixNoMax < 0.1] <- 0
InfectedPevRegionD1SpatialPie<- plotSpatialScatterpie(
  x= D1Pelsubcluster,
  y= D1PelsubclusterCellTypeMatrixNoMax,
  cell_types = colnames(D1PelsubclusterCellTypeMatrixNoMax),
    img = TRUE,
    scatterpie_alpha = 1,
...
ggsave("Spatial_infected_day7_SpatialCellType_Scatterpie.pdf",
      plot = InfectedPevRegionD1SpatialPie,
      height = 3,
      width = 4.5 )

# correlation
InfectedPevRegionD1SpatialPieCorrelationMatrix <- plotCorrelationMatrix(D1PelsubclusterCellTypeMatrixNoMax)
ggsave("Spatial_infected_day7_InfectedPevRegionD1SpatialPieCorrelationMatrix.pdf",
      plot = InfectedPevRegionD1SpatialPieCorrelationMatrix,
      height = 4.5,
      width = 4.5 )
# we only select the matrix that rowsum >0
D1PelsubclusterCellTypeMatrixNoMaxLarger<- D1PelsubclusterCellTypeMatrixNoMax[,colSums(D1PelsubclusterCellTypeMatrixNoMax[])>0]
InfectedPevRegionD1Interactions<- plotInteractions(D1PelsubclusterCellTypeMatrixNoMaxLarger, "heatmap") + scale_fill_gradientn(colors = c("blue", "white", "red"))
?plotInteractions
...
D1Pelsubcluster <- subset (Pyelonephritis_InfectionSubslidesD7, (slice8_imagerow >310 & slice8_imagerow <450 )  & (slice8_imagecol < 330 & slice8_imagecol> 220) )
D1Pelsubcluster <- subset (D1Pelsubcluster, idents = c("N8","N10","N11","N12","N15"))
InfectedPevRegionD1Spatial <-SpatialDimPlot(D1Pelsubcluster,label = TRUE)
InfectedPevRegionD1Spatial
ggsave("Spatial_infected_day7_SpatialDimplot.pdf",
      plot = InfectedPevRegionD1Spatial,
      height = 3,
...
InfectedPevRegionD1Spatial
D1Pelsubcluster
DefaultAssay(D1Pelsubcluster) <- "integrated"
nbs_2.markers <- FindAllMarkers(D1Pelsubcluster,test.use ="MAST")
nbs_2.markers
??SubsetSTData
#nbs_2.markers$gene <- rownames(nbs_2.markers)

nbs_2.markers
#se.subset <- SubsetSTData(D1Pelsubcluster, expression = nbs_2 %in% c("N12", "N15"))
...
DotPlot(D1Pelsubcluster, features = sorted.marks$gene %>% unique()) +RotatedAxis()
#DoHeatmap(D1Pelsubcluster, features = sorted.marks$gene, group.colors = c("red", "lightgray"))
