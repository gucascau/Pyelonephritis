...
# Output directory :
Outdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD5CorMed/"

# Cell types deconvolution:

CDecodir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD5CorMed/Deconvolution/"

# BasicSpatialDim

CAbdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD5CorMed/BasicSpatialDim/"

# Biomarkers
CBiomadir ="/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD5CorMed/CellTypesBiomarkers/"


CDEGdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD5CorMed/DEGs/"

# GOEnrichment

CGOEndir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD5CorMed/GOenrich/"

# Progeny
CProgenydir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD5CorMed/Progeny/"

# Transcript factor
CTFdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD5CorMed/TranscriptFactors/"

# Niche niche Communication

CNichComdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD5CorMed/NicheCommunicate/"

# Concentrate on the infection area abscess from CorMed

InfCorMeddir ="/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD5CorMed/"
...
D5CorMedsubcluster <- subset (Pyelonephritis_InfectionSubslidesD5, slice7_imagerow > 200 &slice7_imagerow < 300 & slice7_imagecol >280  &slice7_imagecol <400)
SpatialDimPlot(D5CorMedsubcluster, pt.size.factor = 1500)
D5CorMedsubcluster <- subset (D5CorMedsubcluster, idents = c("N8","N13","N14","N15","N16","N17"))
InfectedCorMedRegionD5Spatial <-SpatialDimPlot(D5CorMedsubcluster,label = TRUE, pt.size.factor = 1500)
?SpatialDimPlot
...
InfectedCorMedRegionD5SpatialDim<- DimPlot(D5CorMedsubcluster,label = TRUE)
...
CelltypeOrder <- c("PT-S1S2-1","PT-S1S2-2","PT-S3-1","PT-S3-2", "PT-S1S2-N1",  "PT-S1S2-N2","PT-S1S2-N3","InfPT","ProfPT","PC1","PC2","ICA" ,"ICB", "DegCD", "DTL", "TAL","DCT-TAL"  ,"CNT","SMC" ,"URO", "DegURO","FIB" , "END","PL",  "ResMac" , "MONa","MONb", "MACa", "MACb", "NEU"  ,"T" ,"B","NK" )
 ...
 ordered((rownames(D5CorMedsubcluster[["predictions"]]@data)),levels =CelltypeOrder)
 
# get the summary of the selected regions
 GetTissueCoordinates(D5CorMedsubcluster)
 summary(GetTissueCoordinates(D5CorMedsubcluster))
```
# ############################## 
...
D5CorMedsubcluster[["predictions"]]
InfectedMat_CorMed<- t(D5CorMedsubcluster[["predictions"]]@data)
...
names(pal) <- ct
#pal
rownames(D5CorMedsubcluster)
colnames(D5CorMedsubcluster)
str(D5CorMedsubcluster)
...
SpatialScatterPieCorMed <-plotSpatialScatterpie(
  ...
  cell_types = colnames(InfectedMat_CorMedCellType),
    ...
    scale_fill_manual(
        values = pal,
        breaks = names(pal)) +coord_sf(xlim = c(280, 370), ylim = c(200, 300))
...
DefaultAssay(D5CorMedsubcluster) <-"SCT"
Finalcelltypemarkers<- c("Lrp2","Slc34a1","Slc13a3", # Proximal tubule cells (PT)
 ...
 for (i in Finalcelltypemarkers){
   CellTypeMarkerSpatialFeature<- SpatialFeaturePlot(D5CorMedsubcluster, features = i, pt.size.factor = 6000, crop = TRUE)
...
? FindAllMarkers
D5CorMedsubcluster <- ScaleData(object = D5CorMedsubcluster, features = rownames(D5CorMedsubcluster))
# MAST was used to calculate the cluster
CorMed_nichesBiomarkers <-
  FindAllMarkers(
    object = D5CorMedsubcluster,
...
CorMed_nichesBiomarkersTop5Heatmap<- DoHeatmap(D5CorMedsubcluster, features = CorMed_nichesBiomarkersTop5 , slot = "scale.data") + scale_fill_gradientn(colors = c("blue", "white", "red")) + theme(axis.text.y = element_text(size = 10))
ggsave(
...
CorMedCorMedPTSubBiomarkersTop5DotPlot <-
  DotPlot(D5CorMedsubcluster, features = CorMed_nichesBiomarkersTop5 %>% unique()) +
  RotatedAxis()
...
for (i in Top5PTMarker) {
  ...
  PTmarkerSpatialFeature <-
    SpatialFeaturePlot(D5CorMedsubcluster, features = i, slot = "scale.data",pt.size.factor = 4)
...
Idents(D5CorMedsubcluster)
for (n in Idents(D5CorMedsubcluster)) {
...
CorMedCluster<- data.frame(Cell = names(Idents(D5CorMedsubcluster)),   
                              CellType = as.character(Idents(D5CorMedsubcluster)),
                              stringsAsFactors = FALSE)

?progeny
D5CorMedsubcluster <- progeny(D5CorMedsubcluster, scale=FALSE, organism="Mouse", top=500, perm=1, return_assay = TRUE,assay_name = "SCT")
D5CorMedsubcluster
# setup the default assay as progeny
DefaultAssay(object = D5CorMedsubcluster) <- "progeny"

D5CorMedsubcluster <- Seurat::ScaleData(D5CorMedsubcluster, assay = "progeny") 
## We can now directly apply Seurat functions in our Progeny scores. 
## For instance, we scale the pathway activity scores. 
progeny_scores_df <- 
  as.data.frame(t(GetAssayData(D5CorMedsubcluster, slot = "scale.data", 
                               assay = "progeny"))) %>%
...
progeny_hmap
dev.off()
test<- FeaturePlot(D5CorMedsubcluster, features = "NFkB")
...
for (i in ProgenyFeatures ){
  p2 <-(FeaturePlot(D5CorMedsubcluster, features = i) & 
  scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red'))& 
  ggtitle(paste0(i," activity"))
  
  ggsave(paste0(i, "_CorMedInfected_cluster_featurePlot.pdf"), plot = p2, width = 6, height = 6)
  
  p3 <- (SpatialFeaturePlot(D5CorMedsubcluster, features = i, pt.size.factor = 4) +ggtitle(paste0(i," activity")))
...
D5CorMedsubcluster@assays$SCT@data %>% head()
mat <- as.matrix(D5CorMedsubcluster@assays$SCT@data)
# Run ulm
...
D5CorMedsubcluster[['tfsulm']] <- acts %>%
...
# Change assay
DefaultAssay(object = D5CorMedsubcluster) <- "tfsulm"

# Scale the data
D5CorMedsubcluster <- ScaleData(D5CorMedsubcluster)
D5CorMedsubcluster@assays$tfsulm@data <- D5CorMedsubcluster@assays$tfsulm@scale.data

# Explore the top 20 more varaible TFs
n_tfs <- 20

t(as.matrix(D5CorMedsubcluster@assays$tfsulm@data))
# Extract activities from object as a long dataframe
df <- t(as.matrix(D5CorMedsubcluster@assays$tfsulm@data)) %>%
  as.data.frame() %>%
  mutate(cluster = Idents(D5CorMedsubcluster)) %>%
...
for (m in tfs){
  p2 <- (SpatialFeaturePlot(D5CorMedsubcluster, features = m,pt.size.factor = 4) +
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
...
D5CorMedsubcluster
DefaultAssay(object = D5CorMedsubcluster) <- "SCT"

for (n in tfs){
  p3 <- (FeaturePlot(D5CorMedsubcluster, features = n,slot= "scale.data") + 
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression')))
  
  p4 <- (SpatialFeaturePlot(D5CorMedsubcluster, features = n, pt.size.factor = 4, slot= "scale.data") +
 # scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression (time)')))
 
  ggsave(paste0("TFExpression_",n,"_D5CorMedsubcluster_FeaturePlot.pdf"), plot = p3, width = 6, height = 6)
  ggsave(paste0("TFExpression_",n,"_D5CorMedsubcluster_SpatilFeaturePlot.pdf"), plot = p4, width = 6, height = 6)
...
GeneOrderByPheatmap
?DotPlot
TFDotPlot <- DotPlot(D5CorMedsubcluster, features = rev(GeneOrderByPheatmap)) +
...
```{r}
DefaultAssay(D5CorMedsubcluster) <- "SCT"
D5CorMedsubcluster@meta.data %>% head()
#Idents(D5CorMedsubcluster) <- D5CorMedsubcluster@meta.data$Sub2

# reset the identity, Combine the PT cells

Idents(D5CorMedsubcluster)
D5CorMedsubcluster@meta.data
# adjust the levels
# Idents(D5CorMedsubcluster) <-
# ordered(factor(Idents(D5CorMedsubcluster)), levels=c( "5_1","10_1","10_0","2_1","2_0","5_0","5_2","4","7","0","1","9","6","3","8"))

levels(Idents(D5CorMedsubcluster))
```
...
D5CorMedsubcluster = alias_to_symbol_seurat(D5CorMedsubcluster, "mouse")

D5CorMedsubcluster@meta.data %>% head()
D5CorMedsubcluster@meta.data$IntegrateAnnot %>% table() 

head(weighted_networks$gr) 
DimPlot(D5CorMedsubcluster)

# If you use convert_to_alias before here, this one won't work
D5CorMedsubcluster = Seurat::PrepSCTFindMarkers(D5CorMedsubcluster, assay = "SCT", verbose = FALSE)
...
lr_network %>% head()
D5CorMedsubcluster %>% subset(features = lr_network$ligand %>% intersect(rownames(D5CorMedsubcluster)))
# calcualte the niches difference 
DE_sender = nichenetr::calculate_niche_de(seurat_obj  = D5CorMedsubcluster %>% subset(features = lr_network$ligand %>% intersect(rownames(D5CorMedsubcluster))), niches = niches, type = "sender", assay_oi = "SCT") # only ligands important for sender cell types
DE_receiver = nichenetr::calculate_niche_de(seurat_obj  = D5CorMedsubcluster %>% subset(features = lr_network$receptor %>% unique()), niches = niches, type = "receiver", assay_oi = "SCT") # only receptors now, later on: DE analysis to find targets
...
DE_receiver_targets = nichenetr::calculate_niche_de_targets(seurat_obj = D5CorMedsubcluster, niches = niches, lfc_cutoff = lfc_cutoff, expression_pct = expression_pct, assay_oi = "SCT") 
...
avg_expression_ligands = AverageExpression(D5CorMedsubcluster, features = nichenet_output$top_ligands)
...
D5CorMedsubcluster <- subset (Pyelonephritis_InfectionSubslidesD7, slice8_imagerow > 140 | slice8_imagecol > 200, invert = TRUE)
D5CorMedsubcluster <- subset (D5CorMedsubcluster, idents = c("N13","N14","N15","N16","N17"))
InfectedCorMedRegionD5Spatial <-SpatialDimPlot(D5CorMedsubcluster,crop = TRUE, label = FALSE)
InfectedCorMedRegionD5Spatial
...
FinalIntergratedTableAbcess<- merge(D5CorMedsubcluster@meta.data[,c("orig.ident","Annotation")], t(D5CorMedsubcluster[["predictions"]]@data), all=TRUE, by="row.names") 
?Heatmap
...
pal <- colorRampPalette(paletteMartin)(length(ct))
names(pal) <- ct
D5CorMedsubclusterCellTypeMatrix <-t(D5CorMedsubcluster[["predictions"]]@data)

D5CorMedsubclusterCellTypeMatrixNoMax <- D5CorMedsubclusterCellTypeMatrix[,!colnames(D5CorMedsubclusterCellTypeMatrix) %in% c("max")]
D5CorMedsubclusterCellTypeMatrixNoMax
# generate the scatter pie plot
colnames(D5CorMedsubclusterCellTypeMatrixNoMax)
...
InfectedCorMedRegionD5SpatialPie<- plotSpatialScatterpie(
  x= D5CorMedsubcluster,
  y= D5CorMedsubclusterCellTypeMatrixNoMax,
  cell_types = colnames(D5CorMedsubclusterCellTypeMatrixNoMax),
    img = TRUE,
    scatterpie_alpha = 1,
...
# correlation
InfectedCorMedRegionD5SpatialPieCorrelationMatrix <- plotCorrelationMatrix(D5CorMedsubclusterCellTypeMatrixNoMax)
...
D5CorMedsubclusterCellTypeMatrixNoMaxLarger<- D5CorMedsubclusterCellTypeMatrixNoMax[,colSums(D5CorMedsubclusterCellTypeMatrixNoMax[])>0]
InfectedPevRegionD1Interactions<- plotInteractions(D5CorMedsubclusterCellTypeMatrixNoMaxLarger, "heatmap") + scale_fill_gradientn(colors = c("blue", "white", "red"))
?plotInteractions
...
D5CorMedsubcluster <- subset (Pyelonephritis_InfectionSubslidesD7, (slice8_imagerow >310 & slice8_imagerow <450 )  & (slice8_imagecol < 330 & slice8_imagecol> 220) )
D5CorMedsubcluster <- subset (D5CorMedsubcluster, idents = c("N8","N10","N11","N12","N15"))
InfectedCorMedRegionD5Spatial <-SpatialDimPlot(D5CorMedsubcluster,label = TRUE)
InfectedCorMedRegionD5Spatial
ggsave("Spatial_infected_day7_SpatialDimplot.pdf",
      plot = InfectedCorMedRegionD5Spatial,
      height = 3,
...
InfectedCorMedRegionD5Spatial
D5CorMedsubcluster
DefaultAssay(D5CorMedsubcluster) <- "integrated"
nbs_2.markers <- FindAllMarkers(D5CorMedsubcluster,test.use ="MAST")
nbs_2.markers
??SubsetSTData
#nbs_2.markers$gene <- rownames(nbs_2.markers)

nbs_2.markers
#se.subset <- SubsetSTData(D5CorMedsubcluster, expression = nbs_2 %in% c("N12", "N15"))
...
DotPlot(D5CorMedsubcluster, features = sorted.marks$gene %>% unique()) +RotatedAxis()
#DoHeatmap(D5CorMedsubcluster, features = sorted.marks$gene, group.colors = c("red", "lightgray"))
