...
date: "2024-07-29"
Description: 
#   This script is to profile the interesting infected Abscess regions at Day 7 for the cortex and medulla regions
...
# Output directory :
Outdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD7CorMed/"

# Cell types deconvolution:

CDecodir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD7CorMed/Deconvolution/"

# BasicSpatialDim

CAbdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD7CorMed/BasicSpatialDim/"

# Biomarkers
CBiomadir ="/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD7CorMed/CellTypesBiomarkers/"


CDEGdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD7CorMed/DEGs/"

# GOEnrichment

CGOEndir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD7CorMed/GOenrich/"

# Progeny
CProgenydir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD7CorMed/Progeny/"

# Transcript factor
CTFdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD7CorMed/TranscriptFactors/"

# Niche niche Communication

CNichComdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD7CorMed/NicheCommunicate/"

# Concentrate on the infection area abscess from CorMed

InfCorMeddir ="/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/NichesDEGs_V07292024/InfectedCortexMedD7CorMed/"
...
D7CorMedsubcluster <- subset (Pyelonephritis_InfectionSubslidesD7,slice8_imagerow > 270 | slice8_imagecol > 270, invert = TRUE)
SpatialDimPlot(D7CorMedsubcluster, pt.size.factor = 1500)
D7CorMedsubcluster <- subset (D7CorMedsubcluster, idents = c("N13","N14","N15","N16","N17"))
InfectedCorMedRegionD7Spatial <-SpatialDimPlot(D7CorMedsubcluster,label = TRUE, pt.size.factor = 1500)
?SpatialDimPlot
...
InfectedCorMedRegionD7SpatialDim<- DimPlot(D7CorMedsubcluster,label = TRUE)
...
CelltypeOrder <- c("PT-S1S2-1","PT-S1S2-2","PT-S3-1","PT-S3-2", "PT-S1S2-N1",  "PT-S1S2-N2","PT-S1S2-N3","InfPT","ProfPT","PC1","PC2","ICA" ,"ICB", "DegCD", "DTL", "TAL","DCT-TAL"  ,"CNT","SMC" ,"URO", "DegURO","FIB" , "END","PL",  "ResMac" , "MONa","MONb", "MACa", "MACb", "NEU"  ,"T" ,"B","NK" )
 ...
 ordered((rownames(D7CorMedsubcluster[["predictions"]]@data)),levels =CelltypeOrder)
 
# get the summary of the selected regions
 GetTissueCoordinates(D7CorMedsubcluster)
 summary(GetTissueCoordinates(D7CorMedsubcluster))
```
# ############################## 
...
D7CorMedsubcluster[["predictions"]]
InfectedMat_CorMed<- t(D7CorMedsubcluster[["predictions"]]@data)
...
names(pal) <- ct
#pal
rownames(D7CorMedsubcluster)
colnames(D7CorMedsubcluster)
str(D7CorMedsubcluster)
...
SpatialScatterPieCorMed <-plotSpatialScatterpie(
  ...
  cell_types = colnames(InfectedMat_CorMedCellType),
    ...
    scale_fill_manual(
        values = pal,
        breaks = names(pal)) + coord_sf(xlim = c(130,260), ylim = c(330, 460))
...
DefaultAssay(D7CorMedsubcluster) <-"SCT"
Finalcelltypemarkers<- c("Lrp2","Slc34a1","Slc13a3", # Proximal tubule cells (PT)
 ...
 for (i in Finalcelltypemarkers){
   CellTypeMarkerSpatialFeature<- SpatialFeaturePlot(D7CorMedsubcluster, features = i, pt.size.factor = 6000, crop = TRUE)
...
? FindAllMarkers
D7CorMedsubcluster <- ScaleData(object = D7CorMedsubcluster, features = rownames(D7CorMedsubcluster))
# MAST was used to calculate the cluster
CorMed_nichesBiomarkers <-
  FindAllMarkers(
    object = D7CorMedsubcluster,
...
CorMed_nichesBiomarkersTop5Heatmap<- DoHeatmap(D7CorMedsubcluster, features = CorMed_nichesBiomarkersTop5 , slot = "scale.data") + scale_fill_gradientn(colors = c("blue", "white", "red")) + theme(axis.text.y = element_text(size = 10))
ggsave(
...
CorMedCorMedPTSubBiomarkersTop5DotPlot <-
  DotPlot(D7CorMedsubcluster, features = CorMed_nichesBiomarkersTop5 %>% unique()) +
  RotatedAxis()
...
for (i in Top5PTMarker) {
  ...
  PTmarkerSpatialFeature <-
    SpatialFeaturePlot(D7CorMedsubcluster, features = i, slot = "scale.data",pt.size.factor = 4)
...
Idents(D7CorMedsubcluster)
for (n in Idents(D7CorMedsubcluster)) {
...
CorMedCluster<- data.frame(Cell = names(Idents(D7CorMedsubcluster)),   
                              CellType = as.character(Idents(D7CorMedsubcluster)),
                              stringsAsFactors = FALSE)

?progeny
D7CorMedsubcluster <- progeny(D7CorMedsubcluster, scale=FALSE, organism="Mouse", top=500, perm=1, return_assay = TRUE,assay_name = "SCT")
D7CorMedsubcluster
# setup the default assay as progeny
DefaultAssay(object = D7CorMedsubcluster) <- "progeny"

D7CorMedsubcluster <- Seurat::ScaleData(D7CorMedsubcluster, assay = "progeny") 
## We can now directly apply Seurat functions in our Progeny scores. 
## For instance, we scale the pathway activity scores. 
progeny_scores_df <- 
  as.data.frame(t(GetAssayData(D7CorMedsubcluster, slot = "scale.data", 
                               assay = "progeny"))) %>%
...
progeny_hmap
dev.off()
test<- FeaturePlot(D7CorMedsubcluster, features = "NFkB")
...
for (i in ProgenyFeatures ){
  p2 <-(FeaturePlot(D7CorMedsubcluster, features = i) & 
  scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red'))& 
  ggtitle(paste0(i," activity"))
  
  ggsave(paste0(i, "_CorMedInfected_cluster_featurePlot.pdf"), plot = p2, width = 6, height = 6)
  
  p3 <- (SpatialFeaturePlot(D7CorMedsubcluster, features = i, pt.size.factor = 4) +ggtitle(paste0(i," activity")))
...
D7CorMedsubcluster@assays$SCT@data %>% head()
mat <- as.matrix(D7CorMedsubcluster@assays$SCT@data)
# Run ulm
...
D7CorMedsubcluster[['tfsulm']] <- acts %>%
...
# Change assay
DefaultAssay(object = D7CorMedsubcluster) <- "tfsulm"

# Scale the data
D7CorMedsubcluster <- ScaleData(D7CorMedsubcluster)
D7CorMedsubcluster@assays$tfsulm@data <- D7CorMedsubcluster@assays$tfsulm@scale.data

# Explore the top 20 more varaible TFs
n_tfs <- 20

t(as.matrix(D7CorMedsubcluster@assays$tfsulm@data))
# Extract activities from object as a long dataframe
df <- t(as.matrix(D7CorMedsubcluster@assays$tfsulm@data)) %>%
  as.data.frame() %>%
  mutate(cluster = Idents(D7CorMedsubcluster)) %>%
...
for (m in tfs){
  p2 <- (SpatialFeaturePlot(D7CorMedsubcluster, features = m,pt.size.factor = 4) +
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
...
D7CorMedsubcluster
DefaultAssay(object = D7CorMedsubcluster) <- "SCT"

for (n in tfs){
  p3 <- (FeaturePlot(D7CorMedsubcluster, features = n,slot= "scale.data") + 
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression')))
  
  p4 <- (SpatialFeaturePlot(D7CorMedsubcluster, features = n, pt.size.factor = 4, slot= "scale.data") +
 # scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression (time)')))
 
  ggsave(paste0("TFExpression_",n,"_D7CorMedsubcluster_FeaturePlot.pdf"), plot = p3, width = 6, height = 6)
  ggsave(paste0("TFExpression_",n,"_D7CorMedsubcluster_SpatilFeaturePlot.pdf"), plot = p4, width = 6, height = 6)
...
GeneOrderByPheatmap
?DotPlot
TFDotPlot <- DotPlot(D7CorMedsubcluster, features = rev(GeneOrderByPheatmap)) +
...
```{r}
DefaultAssay(D7CorMedsubcluster) <- "SCT"
D7CorMedsubcluster@meta.data %>% head()
#Idents(D7CorMedsubcluster) <- D7CorMedsubcluster@meta.data$Sub2

# reset the identity, Combine the PT cells

Idents(D7CorMedsubcluster)
D7CorMedsubcluster@meta.data
# adjust the levels
# Idents(D7CorMedsubcluster) <-
# ordered(factor(Idents(D7CorMedsubcluster)), levels=c( "5_1","10_1","10_0","2_1","2_0","5_0","5_2","4","7","0","1","9","6","3","8"))

levels(Idents(D7CorMedsubcluster))
```
...
D7CorMedsubcluster = alias_to_symbol_seurat(D7CorMedsubcluster, "mouse")

D7CorMedsubcluster@meta.data %>% head()
D7CorMedsubcluster@meta.data$IntegrateAnnot %>% table() 

head(weighted_networks$gr) 
DimPlot(D7CorMedsubcluster)

# If you use convert_to_alias before here, this one won't work
D7CorMedsubcluster = Seurat::PrepSCTFindMarkers(D7CorMedsubcluster, assay = "SCT", verbose = FALSE)
...
lr_network %>% head()
D7CorMedsubcluster %>% subset(features = lr_network$ligand %>% intersect(rownames(D7CorMedsubcluster)))
# calcualte the niches difference 
DE_sender = nichenetr::calculate_niche_de(seurat_obj  = D7CorMedsubcluster %>% subset(features = lr_network$ligand %>% intersect(rownames(D7CorMedsubcluster))), niches = niches, type = "sender", assay_oi = "SCT") # only ligands important for sender cell types
DE_receiver = nichenetr::calculate_niche_de(seurat_obj  = D7CorMedsubcluster %>% subset(features = lr_network$receptor %>% unique()), niches = niches, type = "receiver", assay_oi = "SCT") # only receptors now, later on: DE analysis to find targets
...
DE_receiver_targets = nichenetr::calculate_niche_de_targets(seurat_obj = D7CorMedsubcluster, niches = niches, lfc_cutoff = lfc_cutoff, expression_pct = expression_pct, assay_oi = "SCT") 
...
avg_expression_ligands = AverageExpression(D7CorMedsubcluster, features = nichenet_output$top_ligands)
...
D7CorMedsubcluster <- subset (Pyelonephritis_InfectionSubslidesD7, slice8_imagerow > 140 | slice8_imagecol > 200, invert = TRUE)
D7CorMedsubcluster <- subset (D7CorMedsubcluster, idents = c("N13","N14","N15","N16","N17"))
InfectedCorMedRegionD1Spatial <-SpatialDimPlot(D7CorMedsubcluster,crop = TRUE, label = FALSE)
...
FinalIntergratedTableAbcess<- merge(D7CorMedsubcluster@meta.data[,c("orig.ident","Annotation")], t(D7CorMedsubcluster[["predictions"]]@data), all=TRUE, by="row.names") 
?Heatmap
...
pal <- colorRampPalette(paletteMartin)(length(ct))
names(pal) <- ct
D7CorMedsubclusterCellTypeMatrix <-t(D7CorMedsubcluster[["predictions"]]@data)

D7CorMedsubclusterCellTypeMatrixNoMax <- D7CorMedsubclusterCellTypeMatrix[,!colnames(D7CorMedsubclusterCellTypeMatrix) %in% c("max")]
D7CorMedsubclusterCellTypeMatrixNoMax
# generate the scatter pie plot
colnames(D7CorMedsubclusterCellTypeMatrixNoMax)
...
InfectedCorMedRegionD1SpatialPie<- plotSpatialScatterpie(
  x= D7CorMedsubcluster,
  y= D7CorMedsubclusterCellTypeMatrixNoMax,
  cell_types = colnames(D7CorMedsubclusterCellTypeMatrixNoMax),
    img = TRUE,
    scatterpie_alpha = 1,
...
# correlation
InfectedCorMedRegionD1SpatialPieCorrelationMatrix <- plotCorrelationMatrix(D7CorMedsubclusterCellTypeMatrixNoMax)
...
D7CorMedsubclusterCellTypeMatrixNoMaxLarger<- D7CorMedsubclusterCellTypeMatrixNoMax[,colSums(D7CorMedsubclusterCellTypeMatrixNoMax[])>0]
InfectedPevRegionD1Interactions<- plotInteractions(D7CorMedsubclusterCellTypeMatrixNoMaxLarger, "heatmap") + scale_fill_gradientn(colors = c("blue", "white", "red"))
?plotInteractions
...
D7CorMedsubcluster <- subset (Pyelonephritis_InfectionSubslidesD7, (slice8_imagerow >310 & slice8_imagerow <450 )  & (slice8_imagecol < 330 & slice8_imagecol> 220) )
D7CorMedsubcluster <- subset (D7CorMedsubcluster, idents = c("N8","N10","N11","N12","N15"))
InfectedCorMedRegionD1Spatial <-SpatialDimPlot(D7CorMedsubcluster,label = TRUE)
InfectedCorMedRegionD1Spatial
ggsave("Spatial_infected_day7_SpatialDimplot.pdf",
      plot = InfectedCorMedRegionD1Spatial,
      height = 3,
...
InfectedCorMedRegionD1Spatial
D7CorMedsubcluster
DefaultAssay(D7CorMedsubcluster) <- "integrated"
nbs_2.markers <- FindAllMarkers(D7CorMedsubcluster,test.use ="MAST")
nbs_2.markers
??SubsetSTData
#nbs_2.markers$gene <- rownames(nbs_2.markers)

nbs_2.markers
#se.subset <- SubsetSTData(D7CorMedsubcluster, expression = nbs_2 %in% c("N12", "N15"))
...
DotPlot(D7CorMedsubcluster, features = sorted.marks$gene %>% unique()) +RotatedAxis()
#DoHeatmap(D7CorMedsubcluster, features = sorted.marks$gene, group.colors = c("red", "lightgray"))
