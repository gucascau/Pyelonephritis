...
# Output directory :
Outdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/InfecdDevNichesDEGs_V07022025/InfectedCortexMedD1CorMed/"

# Cell types deconvolution:

CDecodir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/InfecdDevNichesDEGs_V07022025/InfectedCortexMedD1CorMed/Deconvolution/"

# BasicSpatialDim

CAbdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/InfecdDevNichesDEGs_V07022025/InfectedCortexMedD1CorMed/BasicSpatialDim/"

# Biomarkers
CBiomadir ="/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/InfecdDevNichesDEGs_V07022025/InfectedCortexMedD1CorMed/CellTypesBiomarkers/"


CDEGdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/InfecdDevNichesDEGs_V07022025/InfectedCortexMedD1CorMed/DEGs/"

# GOEnrichment

CGOEndir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/InfecdDevNichesDEGs_V07022025/InfectedCortexMedD1CorMed/GOenrich/"

# Progeny
CProgenydir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/InfecdDevNichesDEGs_V07022025/InfectedCortexMedD1CorMed/Progeny/"

# Transcript factor
CTFdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/InfecdDevNichesDEGs_V07022025/InfectedCortexMedD1CorMed/TranscriptFactors/"

# Niche niche Communication

CNichComdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/InfecdDevNichesDEGs_V07022025/InfectedCortexMedD1CorMed/NicheCommunicate/"

# Concentrate on the infection area abscess from CorMed

InfCorMeddir ="/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/InfecdDevNichesDEGs_V07022025/InfectedCortexMedD1CorMed/"
...
# We have three assays: Spatial,SCT, integrated
# I will create prediction assay for prediction
...
prediction_matrix<- Pyelonephritis_Infection@meta.data[, CelltypeOrder]
...
SpatialFeaturePlot(Pyelonephritis_Infection, features = c("END", "FIB"), pt.size.factor = 1600)
...
D1CorMedsubcluster <- subset (Pyelonephritis_InfectionSubslidesD1, slice3_imagerow > 230 &slice3_imagerow < 310 & slice3_imagecol >300  &slice3_imagecol <380)
SpatialDimPlot(D1CorMedsubcluster, pt.size.factor = 1500)
D1CorMedsubcluster <- subset (D1CorMedsubcluster, idents = c("N8","N15"))
InfectedCorMedRegionD1Spatial <-SpatialDimPlot(D1CorMedsubcluster,label = TRUE, pt.size.factor = 1500)
?SpatialDimPlot
...
InfectedCorMedRegionD1SpatialDim<- DimPlot(D1CorMedsubcluster,label = TRUE)
ggsave("Spatial_infectedPelivis_day7_Dimplot.pdf",
      plot = InfectedCorMedRegionD1SpatialDim,
      height = 3,
      width = 4 )
InfectedCorMedRegionD1SpatialDim
...
# we have changed the name with "_" to "-" therefore, we need to change the name of CelltypeOrder

 

ordered((rownames(D1CorMedsubcluster[["predictions"]]@data)),levels =CelltypeOrder)
 
# get the summary of the selected regions
 GetTissueCoordinates(D1CorMedsubcluster)
 summary(GetTissueCoordinates(D1CorMedsubcluster))
```
# ############################## 
...
D1CorMedsubcluster[["predictions"]]
InfectedMat_CorMed<- t(D1CorMedsubcluster[["predictions"]]@data)
CelltypeOrder<- rownames(D1CorMedsubcluster[["predictions"]]@data)
CelltypeOrder
colnames(InfectedMat_CorMed) <- ordered (colnames(InfectedMat_CorMed), levels =CelltypeOrder)

InfectedMat_CorMed %>% head()
# change the order
...
names(pal) <- ct
#pal
rownames(D1CorMedsubcluster)
colnames(D1CorMedsubcluster)
str(D1CorMedsubcluster)
# 
# # Step 1: Convert to SingleCellExperiment
# sce <- as.SingleCellExperiment(D1CorMedsubcluster)
# 
# # Step 2: Extract spatial coordinates from Seurat
# coords_df <- GetTissueCoordinates(D1CorMedsubcluster)
# coords <- as.matrix(coords_df) 
# # Step 3: Create SpatialExperiment
...
# )
# 
# InfectedMat_CorMedCellType <- InfectedMat_CorMedCellType[colnames(spe), ]

#SPOTlight:::. extract_image(Pyelonephritis_Infection )
#rownames(Pyelonephritis_Infection)
D1CorMedsubcluster
 spe <- GetTissueCoordinates(D1CorMedsubcluster)[,c("imagerow","imagecol")]
...
+coord_sf(xlim = c(300, 390), ylim = c(290, 360)) +
  annotation_custom(tissue_grob, xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf) +
  scale_y_reverse() +  # Flip y-axis to match image
  theme_void()
...
DefaultAssay(D1CorMedsubcluster) <-"SCT"
Finalcelltypemarkers<- c("Lrp2","Slc34a1","Slc13a3", # Proximal tubule cells (PT)
 ...
 for (i in Finalcelltypemarkers){
   CellTypeMarkerSpatialFeature<- SpatialFeaturePlot(D1CorMedsubcluster, features = i, pt.size.factor = 6000, crop = TRUE)
...
? FindAllMarkers
D1CorMedsubcluster <- ScaleData(object = D1CorMedsubcluster, features = rownames(D1CorMedsubcluster))
# MAST was used to calculate the cluster
CorMed_nichesBiomarkers <-
  FindAllMarkers(
    object = D1CorMedsubcluster,
...
CorMed_nichesBiomarkersTop5Heatmap<- DoHeatmap(D1CorMedsubcluster, features = CorMed_nichesBiomarkersTop5 , slot = "scale.data") + scale_fill_gradientn(colors = c("blue", "white", "red")) + theme(axis.text.y = element_text(size = 10))
ggsave(
...
CorMedCorMedPTSubBiomarkersTop5DotPlot <-
  DotPlot(D1CorMedsubcluster, features = CorMed_nichesBiomarkersTop5 %>% unique()) +
  RotatedAxis()
...
for (i in Top5PTMarker) {
  ...
  PTmarkerSpatialFeature <-
    SpatialFeaturePlot(D1CorMedsubcluster, features = i, slot = "scale.data",pt.size.factor = 4)
...
Idents(D1CorMedsubcluster)
for (n in Idents(D1CorMedsubcluster)) {
...
CorMedCluster<- data.frame(Cell = names(Idents(D1CorMedsubcluster)),   
                              CellType = as.character(Idents(D1CorMedsubcluster)),
                              stringsAsFactors = FALSE)

?progeny
D1CorMedsubcluster <- progeny(D1CorMedsubcluster, scale=FALSE, organism="Mouse", top=500, perm=1, return_assay = TRUE,assay_name = "SCT")
D1CorMedsubcluster
# setup the default assay as progeny
DefaultAssay(object = D1CorMedsubcluster) <- "progeny"

D1CorMedsubcluster <- Seurat::ScaleData(D1CorMedsubcluster, assay = "progeny") 
## We can now directly apply Seurat functions in our Progeny scores. 
## For instance, we scale the pathway activity scores. 
progeny_scores_df <- 
  as.data.frame(t(GetAssayData(D1CorMedsubcluster, slot = "scale.data", 
                               assay = "progeny"))) %>%
...
progeny_hmap
dev.off()
test<- FeaturePlot(D1CorMedsubcluster, features = "NFkB")
...
for (i in ProgenyFeatures ){
  p2 <-(FeaturePlot(D1CorMedsubcluster, features = i) & 
  scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red'))& 
  ggtitle(paste0(i," activity"))
  
  ggsave(paste0(i, "_CorMedInfected_cluster_featurePlot.pdf"), plot = p2, width = 6, height = 6)
  
  p3 <- (SpatialFeaturePlot(D1CorMedsubcluster, features = i, pt.size.factor = 4) +ggtitle(paste0(i," activity")))
...
D1CorMedsubcluster@assays$SCT@data %>% head()
mat <- as.matrix(D1CorMedsubcluster@assays$SCT@data)
# Run ulm
...
D1CorMedsubcluster[['tfsulm']] <- acts %>%
...
# Change assay
DefaultAssay(object = D1CorMedsubcluster) <- "tfsulm"

# Scale the data
D1CorMedsubcluster <- ScaleData(D1CorMedsubcluster)
D1CorMedsubcluster@assays$tfsulm@data <- D1CorMedsubcluster@assays$tfsulm@scale.data

# Explore the top 20 more varaible TFs
n_tfs <- 20

t(as.matrix(D1CorMedsubcluster@assays$tfsulm@data))
# Extract activities from object as a long dataframe
df <- t(as.matrix(D1CorMedsubcluster@assays$tfsulm@data)) %>%
  as.data.frame() %>%
  mutate(cluster = Idents(D1CorMedsubcluster)) %>%
...
for (m in tfs){
  p2 <- (SpatialFeaturePlot(D1CorMedsubcluster, features = m,pt.size.factor = 4) +
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
...
D1CorMedsubcluster
DefaultAssay(object = D1CorMedsubcluster) <- "SCT"

for (n in tfs){
  p3 <- (FeaturePlot(D1CorMedsubcluster, features = n,slot= "scale.data") + 
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression')))
  
  p4 <- (SpatialFeaturePlot(D1CorMedsubcluster, features = n, pt.size.factor = 4, slot= "scale.data") +
 # scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression (time)')))
 
  ggsave(paste0("TFExpression_",n,"_D1CorMedsubcluster_FeaturePlot.pdf"), plot = p3, width = 6, height = 6)
  ggsave(paste0("TFExpression_",n,"_D1CorMedsubcluster_SpatilFeaturePlot.pdf"), plot = p4, width = 6, height = 6)
...
GeneOrderByPheatmap
?DotPlot
TFDotPlot <- DotPlot(D1CorMedsubcluster, features = rev(GeneOrderByPheatmap)) +
...
```{r}
DefaultAssay(D1CorMedsubcluster) <- "SCT"
D1CorMedsubcluster@meta.data %>% head()
#Idents(D1CorMedsubcluster) <- D1CorMedsubcluster@meta.data$Sub2

# reset the identity, Combine the PT cells

Idents(D1CorMedsubcluster)
D1CorMedsubcluster@meta.data
# adjust the levels
# Idents(D1CorMedsubcluster) <-
# ordered(factor(Idents(D1CorMedsubcluster)), levels=c( "5_1","10_1","10_0","2_1","2_0","5_0","5_2","4","7","0","1","9","6","3","8"))

levels(Idents(D1CorMedsubcluster))
```
...
D1CorMedsubcluster = alias_to_symbol_seurat(D1CorMedsubcluster, "mouse")

D1CorMedsubcluster@meta.data %>% head()
D1CorMedsubcluster@meta.data$IntegrateAnnot %>% table() 

head(weighted_networks$gr) 
DimPlot(D1CorMedsubcluster)

# If you use convert_to_alias before here, this one won't work
D1CorMedsubcluster = Seurat::PrepSCTFindMarkers(D1CorMedsubcluster, assay = "SCT", verbose = FALSE)
...
lr_network %>% head()
D1CorMedsubcluster %>% subset(features = lr_network$ligand %>% intersect(rownames(D1CorMedsubcluster)))
# calcualte the niches difference 
DE_sender = nichenetr::calculate_niche_de(seurat_obj  = D1CorMedsubcluster %>% subset(features = lr_network$ligand %>% intersect(rownames(D1CorMedsubcluster))), niches = niches, type = "sender", assay_oi = "SCT") # only ligands important for sender cell types
DE_receiver = nichenetr::calculate_niche_de(seurat_obj  = D1CorMedsubcluster %>% subset(features = lr_network$receptor %>% unique()), niches = niches, type = "receiver", assay_oi = "SCT") # only receptors now, later on: DE analysis to find targets
...
DE_receiver_targets = nichenetr::calculate_niche_de_targets(seurat_obj = D1CorMedsubcluster, niches = niches, lfc_cutoff = lfc_cutoff, expression_pct = expression_pct, assay_oi = "SCT") 
...
avg_expression_ligands = AverageExpression(D1CorMedsubcluster, features = nichenet_output$top_ligands)
...
D1CorMedsubcluster <- subset (Pyelonephritis_InfectionSubslidesD7, slice8_imagerow > 140 | slice8_imagecol > 200, invert = TRUE)
D1CorMedsubcluster <- subset (D1CorMedsubcluster, idents = c("N13","N14","N15","N16","N17"))
InfectedCorMedRegionD1Spatial <-SpatialDimPlot(D1CorMedsubcluster,crop = TRUE, label = FALSE)
InfectedCorMedRegionD1Spatial
...
FinalIntergratedTableAbcess<- merge(D1CorMedsubcluster@meta.data[,c("orig.ident","Annotation")], t(D1CorMedsubcluster[["predictions"]]@data), all=TRUE, by="row.names") 
?Heatmap
...
pal <- colorRampPalette(paletteMartin)(length(ct))
names(pal) <- ct
D1CorMedsubclusterCellTypeMatrix <-t(D1CorMedsubcluster[["predictions"]]@data)

D1CorMedsubclusterCellTypeMatrixNoMax <- D1CorMedsubclusterCellTypeMatrix[,!colnames(D1CorMedsubclusterCellTypeMatrix) %in% c("max")]
D1CorMedsubclusterCellTypeMatrixNoMax
# generate the scatter pie plot
colnames(D1CorMedsubclusterCellTypeMatrixNoMax)
...
InfectedCorMedRegionD1SpatialPie<- plotSpatialScatterpie(
  x= D1CorMedsubcluster,
  y= D1CorMedsubclusterCellTypeMatrixNoMax,
  cell_types = colnames(D1CorMedsubclusterCellTypeMatrixNoMax),
    img = TRUE,
    scatterpie_alpha = 1,
...
# correlation
InfectedCorMedRegionD1SpatialPieCorrelationMatrix <- plotCorrelationMatrix(D1CorMedsubclusterCellTypeMatrixNoMax)
...
D1CorMedsubclusterCellTypeMatrixNoMaxLarger<- D1CorMedsubclusterCellTypeMatrixNoMax[,colSums(D1CorMedsubclusterCellTypeMatrixNoMax[])>0]
InfectedPevRegionD1Interactions<- plotInteractions(D1CorMedsubclusterCellTypeMatrixNoMaxLarger, "heatmap") + scale_fill_gradientn(colors = c("blue", "white", "red"))
?plotInteractions
...
D1CorMedsubcluster <- subset (Pyelonephritis_InfectionSubslidesD7, (slice8_imagerow >310 & slice8_imagerow <450 )  & (slice8_imagecol < 330 & slice8_imagecol> 220) )
D1CorMedsubcluster <- subset (D1CorMedsubcluster, idents = c("N8","N10","N11","N12","N15"))
InfectedCorMedRegionD1Spatial <-SpatialDimPlot(D1CorMedsubcluster,label = TRUE)
InfectedCorMedRegionD1Spatial
ggsave("Spatial_infected_day7_SpatialDimplot.pdf",
      plot = InfectedCorMedRegionD1Spatial,
      height = 3,
...
InfectedCorMedRegionD1Spatial
D1CorMedsubcluster
DefaultAssay(D1CorMedsubcluster) <- "integrated"
nbs_2.markers <- FindAllMarkers(D1CorMedsubcluster,test.use ="MAST")
nbs_2.markers
??SubsetSTData
#nbs_2.markers$gene <- rownames(nbs_2.markers)

nbs_2.markers
#se.subset <- SubsetSTData(D1CorMedsubcluster, expression = nbs_2 %in% c("N12", "N15"))
...
DotPlot(D1CorMedsubcluster, features = sorted.marks$gene %>% unique()) +RotatedAxis()
#DoHeatmap(D1CorMedsubcluster, features = sorted.marks$gene, group.colors = c("red", "lightgray"))
