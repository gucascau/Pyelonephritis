---
title: "SpatialDeconcoluation_with_Pipseq"
...
Spindir ="/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Integration_v04142023/SpatialClusters_V0425/"

# Output directory :
Outdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_V08232023/"

# Cell types deconvolution:

Ctdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_V08232023/CellTypes/"

# Cell types heatmap for scaled component

CAbdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_V08232023/CellTypesAbundance/"

# Cell types heatmap for cell type correlated

CCordir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_V08232023/CellTypesCorrelation/"

# 

CPiedir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_V08232023/CellTypesScatterPieDir/"
...
getwd()
setwd("/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_V08232023/")
```
...
##############################################################################################
# with reference
allen_reference <- readRDS(file = paste0(Scindir, "PIPSeq_ImmuneSub_Annotated_v0823.RDS"))
...
CelltypeOrder <- c("heaPT-S1","heaPT-S2","heaPT-S3","heaPT-S4", "heaPT-S5", "DegPT", "MalaPT" , "InfiPT", "HybridPT", "CNT" ,"TAL", "DCT-TAL","IC","PC" ,"URO","END","FIB", "PL","ResIMM","RecMAC","M1-M2" ,"NEU","T","B", "NK","max")

ClusterOrder <- c( "8","3","6","9","1","0","7","4","5_2","5_0","2_0","2_1","10_0","10_1","5_1")

ClusterAllCluster_Median <-
  FinalIntergratedTable %>% group_by(Sub2) %>% summarise(
    `heaPT-S1` = median(`heaPT-S1`),
   `heaPT-S2` = median(`heaPT-S2`),
    `heaPT-S3` = median(`heaPT-S3`),
    `heaPT-S4` = median(`heaPT-S4`),
    `heaPT-S5` = median(`heaPT-S5`),
    DegPT = median(`DegPT`),
    MalaPT = median(`MalaPT`),
    InfiPT = median(`InfiPT`),
    HybridPT = median(`HybridPT`),
    CNT = median(`CNT`),
    CNT = median(`CNT`),
    TAL = median(`TAL`),
    ...
    FIB =
      median(`FIB`),
    PL = median(`PL`),
    ResIMM = median(`ResIMM`),
    RecMAC = median(`RecMAC`),
   `M1-M2` = median(`M1-M2`),
    NEU = median(`NEU`),
    `T` = median(`T`),
    B = median(`B`),
...
ClusterTimepointGroup_Median<-FinalIntergratedTable %>% group_by(orig.ident, Sub2) %>% summarise(
    `heaPT-S1` = median(`heaPT-S1`),
   `heaPT-S2` = median(`heaPT-S2`),
    `heaPT-S3` = median(`heaPT-S3`),
    `heaPT-S4` = median(`heaPT-S4`),
...
ClusterTimepointGroup_Median$Sub2 <- ordered(factor(ClusterTimepointGroup_Median$Sub2), levels= ClusterOrder)
factor(ClusterTimepointGroup_Median$Sub2)
...
for (Pdate in Condition){
 ...
 Pdate_Median_tidy
  Pdate_Median_tidyHeatmap<- na.omit(Pdate_Median_tidy) %>% heatmap(Property, Sub2, Value, scale="row",cluster_rows = FALSE, cluster_columns = FALSE ,palette_value=circlize::colorRamp2(seq(-4, 4, length.out = 11), rev(RColorBrewer::brewer.pal(11, "RdBu")) ) )
...
###############################################################################################
setwd(CCordir)
levels(as.factor(ClusterTimepointGroup_Median$Sub2))
...
for (Pdate in Condition){
  ...
  #i =i+1
  #Pdate=c("PyeloD0_1")
  Pdate_Median_tidy_matrix <-FinalIntergratedTable %>% filter(orig.ident==Pdate) %>% dplyr::select(-c("Row.names","ConditionGroup",'orig.ident','Sub2',"max"))
...
SelectedImageName <- names(Pyelonephritis_InfectionSubslidesD7@images)[8]
...
InfectedMat_D7
# for final celltype matrix: 
InfectedMat_D7CellType<- InfectedMat_D7[,!colnames(mat) %in% c("max")]
...
# for a sepcific regions and specific clusters
...
D0subcluster <- subset (Pyelonephritis_InfectionSubslidesD7, slice8_imagerow > 300 | slice8_imagecol > 300, invert = TRUE)
#D0subcluster <- subset (D0subcluster, idents = c("5_1","10_1","10_0"))
SpatialDimPlot(D0subcluster,crop = TRUE, label = FALSE)
D0subcluster[["Spatial"]]
D0subcluster

D0subclusterCellTypeMatrix <-t(D0subcluster[["predictions"]]@data)
D0subclusterCellTypeMatrix
D0subclusterCellTypeMatrixNoMax <- D0subclusterCellTypeMatrix[,!colnames(D0subclusterCellTypeMatrix) %in% c("max")]
# generate the scatter pie plot

plotSpatialScatterpie(
  x= D0subcluster,
  y= D0subclusterCellTypeMatrixNoMax,
  cell_types = colnames(DeconvolutedMatrix),
    img = TRUE,
    scatterpie_alpha = 1,
    pie_scale = 0.6,axis ="v") +
    scale_fill_manual(
        values = pal,
        breaks = names(pal)) +
...
# correlation
plotCorrelationMatrix(D0subclusterCellTypeMatrixNoMax)
plotInteractions(D0subclusterCellTypeMatrixNoMax, "heatmap")
# 
...
# for final celltype matrix: 
InfectedMat_D0CellType<- InfectedMat_D0[,!colnames(mat) %in% c("max")]
...
paletteMartin <- c(
    "#FFD700","#ff6db6","#00FA9A","#6db6ff", "#009292",  "#ffb6db", 
    "#490092", "#006ddb", "#b66dff",  "#b6dbff", "#F5DEB3",
   "#24ff24",  "#db6d00","#ffff6d",  "#004949", "#000000","#924900", "#920000" )
...
ggsave("SpatialScatterPieDay0.pdf",plot = SpatialScatterPieDay0, height = 3,width = 4.5)
# colnames(DeconvolutedMatrix)
# ncol(Pyelonephritis_Infection)
# nrow(DeconvolutedMatrix)
...
# cluster 5_1
FinalIntergratedTable %>% filter(Sub2 == "5_1")%>% slice_head(n=10)
# cluster 10_1
FinalIntergratedTable %>% filter(Sub2 == "10_1" ) %>% slice_head(n=20)
# cluster 5_0
FinalIntergratedTable %>% filter(Sub2 == "5_0")%>% slice_head(n=10) 
#%>% dplyr::select("FIB","M1-M2" ,"END","NEU","RecMAC", "T", "HybridPT", "B","TAL","InfiPT","ResIMM")
# cluster 10_0
FinalIntergratedTable %>% filter(Sub2 == "10_0" ) %>% slice_head(n=20)
```
