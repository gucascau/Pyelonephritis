...
# Output directory :
Outdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_V07022025/"

dir.create(Outdir)
# Cell types deconvolution:

Ctdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_V07022025/CellTypes/"

# Cell types heatmap for scaled component

CAbdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_V07022025/CellTypesAbundance/"

# Cell types heatmap for cell type correlated

CCordir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_V07022025/CellTypesCorrelation/"

# Biomarkers
CBiomadir ="/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_V07022025/CellTypesBiomarkers/"

# InfectionCellTypesScatterPieDir

CPiedir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_V07022025/CellTypesScatterPieDir/"

# Concentrate on the infection area abscess and margion region

InfCPiedir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_V07022025/InfectionCellTypesScatterPieDir/"
# InfectionCellTypesScatterPieDir

# Concentrate on the infection area abscess from pelvis

InfPelvisdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_V07022025/InfPelvisRegionDir/"

# Concentrate on the Control area 

Contrdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_V07022025/ControlD0/"
# Concentrate on the day 7 area
Day7dir =  "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_V07022025/Day7/"
...
getwd()
setwd("/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_V07022025/")
```
...
# transfer the spatial object into AnnData format
# Ref: https://mojaveazure.github.io/seurat-disk/articles/convert-anndata.html
```{r}
# we refer the conversion h5seurat and annDat
# https://mojaveazure.github.io/seurat-disk/articles/convert-anndata.html
# save seurat object as h5seruat file
SaveH5Seurat(Pyelonephritis_Infection, filename = "Pyelonephritis_Infection.h5Seurat")

Convert("Pyelonephritis_Infection.h5Seurat", dest = "h5ad")
...
##############################################################################################
# with reference
allen_reference <- readRDS(file = paste0(Scindir, "PIPSeq_PTICPCENDFIBUROIMMFinalAnn_Annotated_v07292024.rds"))

# final annotation ImmuneSub
allen_reference@meta.data %>% head()
levels(as.factor(allen_reference@meta.data$ImmuneSub))
...
allen_reference@meta.data <-
  allen_reference@meta.data %>%mutate(
    FinalAnnotation =
      case_when(
        ImmuneSub %in% c("PTS1-S2-E1") ~ "PT-S1S2-1",
        #
        ImmuneSub %in% c("PTS1-S2-E2") ~ "PT-S1S2-2",
        #
        ImmuneSub %in% c("PTS3-E1") ~ "PT-S3-1",
        #
        ImmuneSub %in% c("PTS3-E2") ~ "PT-S3-2",
        #
        ImmuneSub %in% c("NewPTS1-S2-E1") ~ "PT-S1S2-N1",
        #
        ImmuneSub %in% c("NewPTS1-S2-E2") ~ "PT-S1S2-N2",
        #
        ImmuneSub %in% c("NewPTS1-S2-E3") ~ "PT-S1S2-N3",
        #
        ImmuneSub %in% c("Degenerated_URO") ~ "DegURO",
        ImmuneSub %in% c("PC1") ~ "PC",
        ImmuneSub %in% c("PC2") ~ "PC",
        ImmuneSub %in% c("infPT") ~ "InfPT",
        # B
        ImmuneSub %in% c("PROF_EPI") ~ "ProfPT",
        #
        .default = as.character(ImmuneSub)
      )
  )

Idents(allen_reference) <- "FinalAnnotation"
length(levels(Idents(allen_reference)))
# set up the order 
CelltypeOrder <- c("PT-S1S2-1","PT-S1S2-2","PT-S3-1","PT-S3-2", "PT-S1S2-N1",  "PT-S1S2-N2","PT-S1S2-N3","InfPT","ProfPT","PC","ICA" ,"ICB", "DegCD", "DTL", "TAL","DCT-TAL"  ,"CNT","SMC" ,"URO", "DegURO","FIB" , "END","PL",  "ResMac" , "MONa","MONb", "MACa", "MACb", "NEU"  ,"T" ,"B","NK" )

Idents(allen_reference)<- ordered(factor(Idents(allen_reference)),levels=CelltypeOrder)
# save the reference of single cells
saveRDS(allen_reference, file = "PIPSeq_Final_UsedForScanpy.RDS")

# save into h5seurat
SaveH5Seurat(allen_reference, filename = "PIPSeq_Final_UsedForScanpy.h5Seurat")

getwd()
# transfer into h5ad
Convert("PIPSeq_Final_UsedForScanpy.h5Seurat", dest = "h5ad")

# DefaultAssay(allen_reference) <-"RNA"
# 
# DotPlot(allen_reference, features = c("C3","Ccl3","Ccl4","Ccl5"))+ guides(color = guide_colorbar(title = 'AverExp'),
#          size = guide_legend(title = "PercExp")) + RotatedAxis() 
# save the single cell object as the final one
# /Users/XXW004/Documents/Projects/RuizRosado/PIPseq/Results/Evaluation_V022723/CellEvaluation_Subclustering_V0803
# setwd("/Users/XXW004/Documents/Projects/RuizRosado/PIPseq/Results/Evaluation_V022723/CellEvaluation_Subclustering_V0803/FinalSingleCellObj_V08012024")
# saveRDS(allen_reference, file = "PIPSeq_Final_Annotated_v08012024.rds")
...
# Due to the insufficient immmune cell numbers

# we only select the epitheial cells and stroma cells for the 
Idents(allen_reference)
EpithelialStromaCelltypes <- c("PT-S1S2-1","PT-S1S2-2","PT-S3-1","PT-S3-2", "PT-S1S2-N1",  "PT-S1S2-N2","PT-S1S2-N3","InfPT","ProfPT","PC","ICA" ,"ICB", "DegCD", "DTL", "TAL","DCT-TAL"  ,"CNT","SMC" ,"URO", "DegURO","FIB" , "END","PL" )

EpithelialStromaObjs <- subset(allen_reference, idents = EpithelialStromaCelltypes)
# save the Epithelial Cells and Stroma cells

saveRDS(EpithelialStromaObjs, file = "PIPSeq_EpithelialStromaObjs_v07022025.rds")

# save into h5seurat
SaveH5Seurat(EpithelialStromaObjs, filename = "PIPSeq_EpithelialStromaObjs_v07022025.h5Seurat")

# transfer into h5ad
Convert("PIPSeq_EpithelialStromaObjs_v07022025.h5Seurat", dest = "h5ad")

```

# we also read the CD45 enriched immune cells
```{r}
...
CD45_ImmuneCellObj_subimm @meta.data %>% head()
...
#
CD45_ImmuneCellObj_subimm <-  SCTransform(CD45_ImmuneCellObj_subimm, vars.to.regress = "percent.mt", verbose = FALSE)
CD45_ImmuneCellObj_subimm
CD45_ImmuneCellObj_subimm@meta.data %>% head()
# save the reference of single cells
saveRDS(CD45_ImmuneCellObj_subimm, file = "PIPSeq_Final_CD45ImmuneCells_UsedForScanpy.RDS")
CD45_ImmuneCellObj_subimm
# save into h5seurat
# DefaultAssay(CD45_ImmuneCellObj_subimm)
# DefaultLayer(CD45_ImmuneCellObj_subimm)
# 
# Layers(CD45_ImmuneCellObj_subimm[["RNA"]])
#   
# Layers(CD45_ImmuneCellObj_subimm[["RNA"]])
# CD45_ImmuneCellObj_subimm[["RNA"]]@data 
# LayerData(CD45_ImmuneCellObj_subimm[["RNA"]], layer = "X") <- GetAssayData(CD45_ImmuneCellObj_subimm, assay = "RNA", layer = "data")
# # Ensure it's a dgCMatrix
# mat <- LayerData(CD45_ImmuneCellObj_subimm[["RNA"]], layer = "X")
# if (!inherits(mat, "dgCMatrix")) {
#   LayerData(CD45_ImmuneCellObj_subimm[["RNA"]], layer = "X") <- as(mat, "dgCMatrix")
# }


SaveH5Seurat(CD45_ImmuneCellObj_subimm, filename = "PIPSeq_Final_CD45ImmuneCells_UsedForScanpy.h5Seurat",overwrite = TRUE)
# transfer into h5ad
Convert("PIPSeq_Final_CD45ImmuneCells_UsedForScanpy.h5Seurat", assay = DefaultAssay(object = CD45_ImmuneCellObj_subimm), dest = "h5ad",verbose = TRUE,overwrite = TRUE)
?Convert
```


# we then combine the epithelial/stroma cells and immune cells
```{r}
Layers(CD45_ImmuneCellObj_subimm[["RNA"]])
Layers(EpithelialStromaObjs[["RNA"]])

table(EpithelialStromaObjs @meta.data$DataSet)

# update the CD45 Dataset

table(CD45_ImmuneCellObj_subimm @meta.data $ orig.ident)
CD45_ImmuneCellObj_subimm @meta.data %>% mutate(DataSet = orig.ident)

merged_obj <- merge(EpithelialStromaObjs, y = CD45_ImmuneCellObj_subimm, project = "MergedProject")

table(Idents(merged_obj))
# set up the identity
merged_obj @meta.data<- merged_obj @meta.data %>% mutate(FinalCombinedAnnation= Idents(merged_obj))
 # set up the dateset
merged_obj @meta.data<- merged_obj @meta.data %>% mutate(FinalCombinedDate= case_when(is.na(DataSet) ~ orig.ident,
                                                                                      TRUE ~ DataSet))
merged_obj @meta.data<- merged_obj @meta.data %>% mutate(FinalSource= case_when(is.na(DataSet) ~ "Immune",
                                                                                      TRUE ~ "EpithelialStroma")) 
...
merged_obj <- FindNeighbors(merged_obj, dims = 1:50, reduction = "pca")
merged_obj <- FindClusters(merged_obj, resolution = 2, cluster.name = "unintegrated_clusters")
merged_obj <- RunUMAP(merged_obj, dims = 1:50, reduction = "pca", reduction.name = "umap.unintegrated")
...
merged_obj <- RunUMAP(merged_obj, reduction = "harmony", dims = 1:50, reduction.name = "umap.harmony")
merged_obj@meta.data %>% head()

Idents(merged_obj) <- merged_obj@meta.data$FinalCombinedAnnation
DimPlot(merged_obj, reduction = "umap.harmony",label = T)

p1 <- DimPlot(
  merged_obj,
  reduction = "umap.harmony",
  group.by = c("FinalSource", "FinalCombinedAnnation", "harmony_clusters"),
  combine = FALSE, label.size = 2
)

# merged_obj <- RunHarmony(merged_obj, 
# 				group.by.vars = c("FinalSource"), 
# 				reduction = "pca", assay.use = "SCT", reduction.save = "harmony")
# merged_obj <- RunUMAP(merged_obj, dims = 1:50, verbose = TRUE, reduction = "harmony")
# merged_obj <- FindNeighbors(merged_obj, dims = 1:50,  verbose = TRUE, reduction = "harmony")
# 
# merged_obj<- FindClusters(merged_obj,  verbose = TRUE, resolution = 0.5)
DimPlot(merged_obj, label = T)

DefaultAssay(merged_obj) <- "SCT"
GetAssayData(merged_obj, assay = "SCT", layer = "scale.data")

# Check if data exists
data_matrix <- GetAssayData(merged_obj, assay = "SCT")
nrow(data_matrix)
ncol(data_matrix)
data_matrix_RNA <-GetAssayData(merged_obj, assay = "RNA",layer = "scale.data")
nrow(data_matrix_RNA)
ncol(data_matrix_RNA)
DefaultAssay(merged_obj) <- "RNA"
merged_obj <- JoinLayers(merged_obj)
merged_obj
#DefaultLayer(merged_obj[["SCT"]])

saveRDS(merged_obj, file = "PIPSeq_EpithelialStromaImmMergeObjs_v07022025.rds")
# save the object into scanpy
# save into h5seurat
SaveH5Seurat(merged_obj, filename = "PIPSeq_EpithelialStromaImmMergeObjs_v07022025.h5Seurat", overwrite = T)

# transfer into h5ad
Convert("PIPSeq_EpithelialStromaImmMergeObjs_v07022025.h5Seurat", dest = "h5ad", overwrite = T)
...
DimPlot(object = merged_obj,
        group.by = "FinalCombinedAnnation")

# spacexr doesn't like labels with "/" in it, so let's change that
merged_obj$Celltype <-
  gsub(pattern = "/", 
       replacement = "-", 
       x = merged_obj$FinalCombinedAnnation)
GetAssayData(object = merged_obj, layer = "counts")
# create the spacexr reference object
spacexr_reference <-
    spacexr::Reference(counts = GetAssayData(object = merged_obj, 
                                             layer = "counts"),
                       cell_types = as.factor(merged_obj$Celltype))

# get x and y coordinates for our visium data

GetTissueCoordinates(object = Pyelonephritis_Infection, 
                             image = "slice9") %>% head()
coords <- 
  rbind(GetTissueCoordinates(object = Pyelonephritis_Infection, 
                             image = "slice1"),
        GetTissueCoordinates(object = Pyelonephritis_Infection, 
                             image = "slice3"),
        GetTissueCoordinates(object = Pyelonephritis_Infection, 
...
my_data <- 
  spacexr::SpatialRNA(coords = coords,
                      counts = GetAssayData(object = Pyelonephritis_Infection, 
                                            layer = "counts",
                                            assay = "Spatial"))
#my_data
table(spacexr_reference@cell_types)
...
rctd_obj <- 
  spacexr::create.RCTD(spatialRNA = my_data, 
                       ...
                       max_cores = 10,
                       CELL_MIN_INSTANCE = 5, # min number of cells per cell type lower if rare cell type
                       fc_cutoff = 0, # minimum log-fold-change (across cell types) for genes to be included in the platform effect normalization step (default = 0.5)
                       gene_cutoff = 0
...
# save the object of rctd
saveRDS(rctd_out, file = "rctd_out_UsingEpiStroImm.rds")

# add the deconvolution weights to the visium meta data 
Pyelonephritis_Infection <- 
  AddMetaData(object = Pyelonephritis_Infection, 
              metadata = norm_weights)

head(Pyelonephritis_Infection@meta.data)

saveRDS(Pyelonephritis_Infection, file = "Pyelonephritis_Infection_ATLAS_RCTD_Deco_UsingEpiStroImm.rds")

Pyelonephritis_Infection <- readRDS(file = "Pyelonephritis_Infection_ATLAS_RCTD_Deco_UsingEpiStroImm.rds")

myPalette <- colorRampPalette(rev(brewer.pal(11, "Spectral")))

CelltypeOrder<- levels(as.factor(merged_obj$Celltype))
for (i in CelltypeOrder) {
  
  i= "Neutrophils_3"
  
  SpatialFeatureCellType <-
    SpatialFeaturePlot(
      Pyelonephritis_Infection,
      features = i,
      pt.size.factor = 1800,
    ...
    ggplot2::scale_fill_gradientn(colours = myPalette(4), limits = c(0, 0.6))
  ggsave(
    filename = paste0(CAbdir,"SpatialCellProportion_", i, ".pdf"),
    plot = SpatialFeatureCellType,
    height = 4.5,
    width = 20
  )
...
# 
CellTypes<-levels(as.factor(merged_obj@meta.data$Celltype))
write.csv(CellTypes, file = "CellTypes.csv")

# CellTypesOrdered<- c("PT-S1S2-1","PT-S1S2-2","PT-S3-1","PT-S3-2", "PT-S1S2-N1",  "PT-S1S2-N2","PT-S1S2-N3","InfPT","ProfPT","PC","ICA" ,"ICB", "DegCD", "DTL", "TAL","DCT-TAL"  ,"CNT","SMC" ,"URO", "DegURO","FIB" , "END","PL","ResMac_1", "ResMac_2","MonoDerivedMac1", "ResDendric", "ResMac_3", "MonoDerivedMac2","Neutrophils_0", "Neutrophils_1", "Neutrophils_2", "Neutrophils_3","B cells", "T cells", "NK cells" )
# c("PTS1","PTS2","PTS3","PTS3T2","PEC","DTL","DTL-ATL","ATL","MTAL","DCT","DCT-CNT","CNT","CTAL","DC","Podo","LOH","PC","ICA","ICB","CD-Trans","Fib","Endo","Glom-Endo","Asc-Vasa-Recta","Desc-Vasa-Recta","Vas-Afferens","Vas-Efferens","Macro","Neutro","T lymph","NK","B lymph","MC","MD","Per","Unknown")
# load some colors
colorset <- 
...
length(CelltypeOrder)
Pyelonephritis_Infection@meta.data$orig.ident
split <- 
  SplitObject(Pyelonephritis_Infection, 
              split.by = "orig.ident")

names(split)
celltype_colors <-
  colorset[1:length(CelltypeOrder)]
celltype_colors
names(celltype_colors) <- CelltypeOrder
  
GetTissueCoordinates(Pyelonephritis_Infection) %>% head()
Images(Pyelonephritis_Infection)
scatterpies <- list()
#remove.packages("SPOTlight")
#devtools::install_github("MarcElosua/SPOTlight")
library(SPOTlight)
#library(png)
for (infectday in names(split)){
  
 # split[["PyeloD0_1"]]
  infectday <- c("PyeloD5_2")
  spe <- split[[infectday]]
  #spe <- split[["PyeloD0_1"]]
  #spe@meta.data[,CelltypeOrder]
  mat <- spe@meta.data[,CelltypeOrder]
  
  # reduce anything less than 0.1% to 0, we keep the order
  #mat[mat < 0.001] <- 0
  #Idents(spe) <- rownames(spe@meta.data)
  DefaultAssay(spe) <- "Spatial"
  spe <- GetTissueCoordinates(spe)[,c("imagerow","imagecol")]
  dim <- spe %>%
  as.data.frame()
  colnames(mat)
  spe
 
  scatterpies[[infectday]] <-
...
#scatterpies$PyeloD1_2
names(scatterpies)
for (i in names(scatterpies)) {
  
  i= "PyeloD5_2"
  spatialdeconvoluted <- scatterpies[[i]]
  
  
  ggsave(
    filename = paste0(CPiedir, "spatial_dencovoluted_RCTD", i, ".pdf"),
    plot = spatialdeconvoluted,
    width = 8,
    height = 12
  )
}

CombinedPlots<- wrap_plots(scatterpies)
ggsave("combined_plot_deconvoluted2.pdf", CombinedPlots, width = 30, height = 30)

```

# generate the marker genes
```{r}

# macrophage
c("Cx3cr1", "Fcgr1","H2-Aa", "Adgre1")
...
for (i in CelltypeOrder){
  ...
  CellTypeSpatialFeature<- SpatialFeaturePlot(Pyelonephritis_Infection, features = i, pt.size.factor = 1500, crop = TRUE,  images = selectedslices) &
...
dev.off()
FinalIntergratedTableAbcessHeatmap
...
```{r}
setwd(Outdir)
saveRDS(file = "Pyelonephritis_Infection_Deconvoluted_v07292024.RDS", object = Pyelonephritis_Infection)

Pyelonephritis_Infection <- readRDS("Pyelonephritis_Infection_Deconvoluted_v07292024.RDS")
...
SpatialFeaturePlot(Pyelonephritis_Infection, features = "Lrp2", pt.size.factor = 1600, crop = TRUE) & ggplot2::scale_fill_gradientn(colours = myPalette(4), limits=c(0,10))
...
for (i in Finalcelltypemarkers){
   CellTypeMarkerSpatialFeature<- SpatialFeaturePlot(Pyelonephritis_Infection, features = i, pt.size.factor = 1600, crop = TRUE) & ggplot2::scale_fill_gradientn(colours = myPalette(4), limits=c(0,10))
...
# for PT cells with the injury markers in spatial
```{r}
# S1, S2, S3 markers 
PTCBiomadir<- paste0(BioMarkerDir,"PTs/")
dir.create(PTCBiomadir)
setwd(PTCBiomadir)

PTS1_S3_Biomadir<- paste0(PTCBiomadir,"PTS1_S3/")
dir.create(PTS1_S3_Biomadir)
setwd(PTS1_S3_Biomadir)
S1_S3_Markers<- c("Lrp2","Aqp1","Slc34a1","Ggt1","Atp1a1","Atp1b1","Slc5a2","Slc5a12","Slc22a8","Slc4a4","Slc13a3","Slc13a2","Slc22a6","Slc13a1","Slc22a2","Cubn","Slc7a12","Slc22a7","Slc7a13","Aqp7")
for (j in S1_S3_Markers){
   CellTypeMarkerSpatialFeature<- SpatialFeaturePlot(Pyelonephritis_Infection, features = j, pt.size.factor = 1600, crop = TRUE) & ggplot2::scale_fill_gradientn(colours = myPalette(4), limits=c(0,10))
  
  ggsave(filename = paste0(j,"_SpatialFeaturePrediction_Splits.pdf"), plot = CellTypeMarkerSpatialFeature, height = 5, width = 25)
  
}
PTInjury_Biomadir<- paste0(PTCBiomadir,"PTInjury/")
dir.create(PTInjury_Biomadir)
setwd(PTInjury_Biomadir)
PTInjury_Biomadir
# Final Injury markers:
FinalInjuryMarkers<- c("Havcr1","Lcn2","Clu","Nrg1","Slc5a2","Agt", "Spp1", "Fn1","C3","Dab2","Vcam1","Kcnip4","Smad1","Sema5a","Dcdc2a","Ypel2", "Hspd1","Hif1a","Hmox1","Defb1","Lyz2","Nfkbia","Il1b","Ccl2")
for (n in FinalInjuryMarkers){
   CellTypeMarkerSpatialFeature<- SpatialFeaturePlot(Pyelonephritis_Infection, features = n, pt.size.factor = 1600, crop = TRUE) & ggplot2::scale_fill_gradientn(colours = myPalette(4), limits=c(0,10))
  
  ggsave(filename = paste0(n,"_SpatialFeaturePrediction_Splits.pdf"), plot = CellTypeMarkerSpatialFeature, height = 5, width = 25)
  
}
PTMetabolic_Biomadir<- paste0(PTCBiomadir,"PTMetabolic/")
dir.create(PTMetabolic_Biomadir)
setwd(PTMetabolic_Biomadir)

# 
MetabolicMarkers<-c("Slc22a6", "Slc22a8", "Slc6a19", "Slc22a12", "Slc13a3", "Slc9a3", "Slc1a1","Ldhd")
for (m in MetabolicMarkers){
   CellTypeMarkerSpatialFeature<- SpatialFeaturePlot(Pyelonephritis_Infection, features =m, pt.size.factor = 1600, crop = TRUE) & ggplot2::scale_fill_gradientn(colours = myPalette(4), limits=c(0,10))
  
  ggsave(filename = paste0(m,"_SpatialFeaturePrediction_Splits.pdf"), plot = CellTypeMarkerSpatialFeature, height = 5, width = 25)
...
setwd("/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_V07022025/InfMadullaRegionDir/")
...
InfectedRegionD7Spatial <-SpatialDimPlot(D7subcluster,crop = TRUE, label = FALSE,pt.size.factor)
InfectedRegionD7Spatial
?SpatialDimPlot
SpatialDimPlot(D7subcluster,crop = TRUE, label = T,pt.size.factor=1500)
...
InfectedRegionD7Spatial <-SpatialDimPlot(D7subcluster,label = TRUE,pt.size.factor = 1500)
InfectedRegionD7Spatial
ggsave("Spatial_infected_day7_SpatialDimplot.pdf",
      plot = InfectedRegionD7Spatial,
      height = 3,
...
plotInteractions(D7subclusterCellTypeMatrixNoMax, "network")
# generate the scatter pie plot
colnames(D7subclusterCellTypeMatrixNoMax)
