...
date: "2023-08-23"
Description: 
#   Deconvolute the spatial spots using PIPseq Immune single cells 

#     Note:
#    #     We used the sorted CD45 single cells with neutrophil subtypes, macrophage subtypes and other immune cells 
...
# unlink("~/.R/library/spatstat.explore", recursive = TRUE, force = TRUE)
...
# install.packages('Seurat')
# # 
# devtools::install_github("satijalab/seurat", ref = "4.1.4")
...
# 
# # Install the `numeric_total_size` version from my fork
# remotes::install_github("ycl6/future@numeric_total_size")
# # Remove future
# remove.packages("future")
# 
# # Install the `numeric_total_size` version from PR 755
...
Scindir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Datasets/SharedFromGama/ATLAS_files/"
...
# Output directory :
Outdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_Immune_V12092024/"

# Cell types deconvolution:

Ctdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_Immune_V12092024/CellTypes/"

# Cell types heatmap for scaled component

CAbdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_Immune_V12092024/CellTypesAbundance/"

# Cell types heatmap for cell type correlated

CCordir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_Immune_V12092024/CellTypesCorrelation/"

# Biomarkers
CBiomadir ="/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_Immune_V12092024/CellTypesBiomarkers/"

# InfectionCellTypesScatterPieDir

CPiedir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_Immune_V12092024/CellTypesScatterPieDir/"

# Concentrate on the infection area abscess and margion region

InfCPiedir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_Immune_V12092024/InfectionCellTypesScatterPieDir/"
# InfectionCellTypesScatterPieDir

# Concentrate on the infection area abscess from pelvis

InfPelvisdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_Immune_V12092024/InfPelvisRegionDir/"

# Concentrate on the Control area 

Contrdir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_Immune_V12092024/ControlD0/"
# Concentrate on the day 7 area
Day7dir =  "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_Immune_V12092024/Day7/"
...
getwd()
setwd("/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_Immune_V12092024/")
```
...
# Reading the RDS file from the spatial
...
##############################################################################################
# with reference
allen_reference <- readRDS(file = paste0(Scindir, "reclustered_cells.rds"))

allen_reference@meta.data %>% head()

Idents(allen_reference) <- allen_reference@meta.data$seurat_clusters
DimPlot(allen_reference, label = T)
allen_reference@meta.data <- allen_reference@meta.data %>% mutate(cell_type_raw = case_when(
  seurat_clusters == 0 ~"Neutrophils",
  seurat_clusters == 1 ~"Neutrophils",
  seurat_clusters == 2 ~"Neutrophils",
  seurat_clusters == 3 ~"T cells",
  seurat_clusters == 4 ~"Macrophage",
  seurat_clusters == 5 ~"Macrophage",
  seurat_clusters == 6 ~"Macrophage",
  seurat_clusters == 7 ~"NK cells",
  seurat_clusters == 8 ~"B cells",
...
# Then I will join the layer for these dataset

allen_reference<- JoinLayers(allen_reference)
allen_reference
```


```{r}
# check the expression profile of one cell 
# testdata <- LayerData(allen_reference, layer ="data")
# New<- JoinLayers(allen_reference, overwrite = TRUE)
# data.input <- GetAssayData(New, assay = "RNA", slot = "data")
# data.input[,"AAAAACTGATATCCCT_1"]
# # neutrophil
# allen_reference@meta.data["AAAAACTGATATCCCT_1",]
# we also put the identity of the neutrophil and macrophage into the object

Neutrophil_ref <- readRDS(file = paste0(Scindir, "Neutrophils.rds"))

Neutrophil_ref <- JoinLayers(Neutrophil_ref)


# check the neutrophil clusters
# Neutrophil_ref@meta.data %>% head()
# New2<- JoinLayers(Neutrophil_ref, overwrite = TRUE)
# DimPlot(Neutrophil_ref, split.by = "orig.ident")
# 
# data.input2 <- GetAssayData(New2, assay = "RNA", slot = "data")
# Neutrophil_ref@meta.data["AAAAACTGATATCCCT_1",]
# # neutrophil
# DimPlot(Neutrophil_ref)

# add the meta data with one column:
Neutrophil_ref @meta.data<-Neutrophil_ref @meta.data %>% dplyr::mutate(NetrophilSub = paste0("Neutrophils_",RNA_snn_res.0.3))

# generate the meta data frame from Neutrophil meta data
NeutromataAnnotation <- Neutrophil_ref@meta.data %>% dplyr::select(NetrophilSub)
colnames(NeutromataAnnotation)
Neutrophil_ref@meta.data%>% head()
Idents(Neutrophil_ref) <- "NetrophilSub"

DimPlot(Neutrophil_ref,label = T)
## add the Neutrophil meta data to original object
allen_reference<-AddMetaData(object = allen_reference, metadata = NeutromataAnnotation)

Idents(allen_reference) <- allen_reference@meta.data$NetrophilSub

# check the reference: 
DimPlot(allen_reference)

# add the macrophage 


Macrophages_ref <- readRDS(file = paste0(Scindir, "Macrophages.rds"))
Macrophages_ref
# join Layer 
Macrophages_ref <- JoinLayers(Macrophages_ref)
Macrophages_ref
Macrophages_ref @meta.data %>% head()

DimPlot(Macrophages_ref, label = T)
Macrophage_cluster_names <- c(
  "0" = "ResMac_1",
  "1" = "ResMac_2",
  "2" = "MonoDerivedMac1",
   "3" = "ResDendric",
...
Macrophages_ref <- RenameIdents(Macrophages_ref, Macrophage_cluster_names)

#

DimPlot(Macrophages_ref,label = T)
# add the meta data with one column:
Macrophages_ref @meta.data<-Macrophages_ref @meta.data %>% dplyr::mutate(MacrophageSub = Idents(Macrophages_ref))

Macrophages_ref @meta.data %>% head()
# generate the meta data frame from Macrophages meta data
MacrophageMetaAnnotation <- Macrophages_ref@meta.data %>% dplyr::select(MacrophageSub)
colnames(MacrophageMetaAnnotation)

## add the Macrophages meta data to original object
allen_reference<-AddMetaData(object = allen_reference, metadata = MacrophageMetaAnnotation)

Idents(allen_reference) <- allen_reference@meta.data$MacrophageSub
...
# add the meta data
allen_reference@meta.data<- allen_reference@meta.data %>% mutate(FinalAnn= case_when(
  !is.na(NetrophilSub) ~ NetrophilSub,
  !is.na(MacrophageSub) ~ MacrophageSub,
  TRUE ~cell_type_raw
))
# allen_reference@meta.data  %>%
#   mutate(Ann_WithNeu = ifelse(is.na(NetrophilSub),cell_type, NetrophilSub))

Idents(allen_reference) <- allen_reference@meta.data$FinalAnn
# Update the cell type
# allen_reference@meta.data  %>% 
#   mutate(Ann_WithNeu = ifelse(is.na(NeutromataAnnotation), cell_type,NeutromataAnnotation)) %>% mutate(Ann_WithNeuMac = ifelse(is.na(Ann_WithNeu), cell_type,NeutromataAnnotation))

DimPlot(allen_reference)

# save this final object with annotation
saveRDS(allen_reference, file = "CD45_ImmuneCell_withSubCluster.rds")
#
allen_reference<- readRDS("CD45_ImmuneCell_withSubCluster.rds")

# In order to better understand these cells, we only selected the single cells that have well annotation
allen_reference@meta.data %>% head()

allen_reference_subimm <- subset(allen_reference, idents = c("ResMac_1", "ResMac_2","MonoDerivedMac1", "ResDendric", "ResMac_3", "MonoDerivedMac2","Neutrophils_0", "Neutrophils_1", "Neutrophils_2", "Neutrophils_3","B cells", "T cells", "NK cells" ))

allen_reference_subimm

allen_reference_subimm <- JoinLayers(allen_reference_subimm)
levels(as.factor(allen_reference_subimm@meta.data$FinalAnn))

Allimmune<-subset(allen_reference, idents = c("ResMac_1", "ResMac_2","MonoDerivedMac1", "ResDendric", "ResMac_3", "MonoDerivedMac2","Neutrophils_0", "Neutrophils_1", "Neutrophils_2", "Neutrophils_3","B cells", "T cells", "NK cells" ))
...
# allen_reference_subimm@meta.data <-
#   allen_reference_subimm@meta.data %>%mutate(
#     FinalAnnotation =
#       case_when(
...
Idents(allen_reference_subimm) <- "FinalAnn"
length(levels(Idents(allen_reference_subimm)))
# set up the order 
...
Idents(allen_reference_subimm)<- ordered(factor(Idents(allen_reference_subimm)),levels=CelltypeOrder)


DimPlot(allen_reference_subimm,label = T, label.size = 2)
...
# I will joinalyer first,
allen_reference_subimm

allen_reference_subimm <-SCTransform(allen_reference_subimm, ncells = 15000, verbose = FALSE) %>%
    RunPCA(verbose = FALSE) %>%
    RunUMAP(dims = 1:30)
#allen_reference_subimm <- JoinLayers(allen_reference_subimm)

allen_reference_subimm@meta.data %>% head()
#Layers(allen_reference_subimm)
### integrate datasetss
# allen_reference_subimm <- IntegrateLayers(object = allen_reference_subimm, method = CCAIntegration, normalization.method = "SCT", verbose = F)
# ## Visualize the effect of integration on embedding
# allen_reference_subimm <- RunUMAP(allen_reference_subimm, dims = 1:30, reduction = "integrated.dr")
# DimPlot(allen_reference_subimm, reduction = "umap")
```


```{r}
levels(Idents(allen_reference_subimm))
table(Idents(allen_reference_subimm))

#Idents(allen_reference) <- str_split(allen_reference@meta.data$Annot,":",simplify = T)[,2]
# the annotation is stored in the 'subclass' column of object metadata
DimPlot(allen_reference_subimm,  label = TRUE)

Layers(Pyelonephritis_Infection)
...
Pyelonephritis_Infection <- SCTransform(Pyelonephritis_Infection, assay = "Spatial", verbose = FALSE) %>%
  RunPCA(verbose = FALSE)
DimPlot(Pyelonephritis_Infection,  label = TRUE)

# DefaultAssay(allen_reference_subimm) <-"integrated"
# 
# allen_reference_subimm[["integrated"]]
anchors <- FindTransferAnchors(reference = allen_reference_subimm, query = Pyelonephritis_Infection, normalization.method = "SCT")
?FindTransferAnchors

# there are some errors: Error: Given reference assay (SCT) has 4 reference sct models. Please provide a reference assay with a  single reference sct model.
# Check the SCT Models Inspect the reference assay to see how many SCT models it contains:


predictions.assay <- TransferData(anchorset = anchors, 
                                  refdata = Idents(allen_reference_subimm),
...
# set up the order 
CelltypeOrder <- c("Neutrophils-0", "Neutrophils-1", "Neutrophils-2", "Neutrophils-3","ResMac-1", "ResMac-2","MonoDerivedMac1", "ResDendric", "ResMac-3", "MonoDerivedMac2","B cells", "T cells","NK cells" )
...
# the cell type proportion of each spot
 ...
 saveRDS(Pyelonephritis_Infection, file = "Pyelonephritis_Infection_ImmunePredicted.RDS")
...
```{r}
# check the distribution of novel markers
Pyelonephritis_Infection
# change the assay to RNA
DefaultAssay(Pyelonephritis_Infection) <-"integrated"
SpatialFeaturePlot(Pyelonephritis_Infection, features = c("Icam1"), pt.size.factor = 1500, crop = TRUE,  images = selectedslices) &
          ggplot2::scale_fill_gradientn(colours = myPalette(4), limits=c(0,1))
  
SpatialFeaturePlot(Pyelonephritis_Infection, features = c("Eif5"), pt.size.factor = 1500, crop = TRUE,  images = selectedslices) &
          ggplot2::scale_fill_gradientn(colours = myPalette(4), limits=c(0,1))

 
#Icam1, Gbp2b,
# Alcam, Atf3
SpatialFeaturePlot(Pyelonephritis_Infection, features = c("Cd24a"), pt.size.factor = 1500, crop = TRUE,  images = selectedslices) &
          ggplot2::scale_fill_gradientn(colours = myPalette(4), limits=c(0,1))

#H2-Aa
SpatialFeaturePlot(Pyelonephritis_Infection, features = c("H2-Aa"), pt.size.factor = 1500, crop = TRUE,  images = selectedslices) &
          ggplot2::scale_fill_gradientn(colours = myPalette(4), limits=c(0,1))

SpatialFeaturePlot(Pyelonephritis_Infection, features = c("Lrg1"), pt.size.factor = 1500, crop = TRUE,  images = selectedslices) &
          ggplot2::scale_fill_gradientn(colours = myPalette(4), limits=c(0,1))
    ...
    plot_list <- lapply(c(column_1, column_2),
                        function(column) {
                            max_color <- ifelse(column == column_1,
                                                "#FF0000", "#00FF00")
                            SpatialFeaturePlot(cells_obj, column_1) +
...
SpatialFeaturePlotBlend(Pyelonephritis_Infection, "H2-Aa", "S100a8")
#SpatialFeaturePlotBlend(Pyelonephritis_Infection, features = c("H2-Aa","S100a8")) 

# here we only concentrate on the slide 5
Pyelonephritis_Infection@meta.data %>% pull(orig.ident)
...
Allimagmes= names(Pyelonephritis_Infection @images)
  ...
  SelectedImageName <- "slice8"
  SelectedImageName
  OtherImages <- Allimagmes[Allimagmes != SelectedImageName]
  OtherImages
  Pyelonephritis_InfectionSubslides@images[OtherImages] = NULL
DefaultAssay(Pyelonephritis_InfectionSubslides)< "SCT"

SpatialFeaturePlot(Pyelonephritis_InfectionSubslides, features = c("H2-Aa", "S100a8"),pt.size.factor = 1500)
?SpatialFeaturePlot
SpatialFeaturePlotBlend(object = Pyelonephritis_InfectionSubslides, features = c("H2-Aa", "S100a8"),
                           # top_left = "blue", bottom_right = "orange",
                           # bottom_left = "white", top_right = "#FF0000",
                        assay = "Spatial",pt.size.factor = 1500)

SpatialFeaturePlotBlend(object = Pyelonephritis_InfectionSubslides, features = c("H2-Aa", "Lrg1"),
                           top_left = "blue", bottom_right = "green",
                           bottom_left = "white", top_right = "#FF0000",
                        assay = "Spatial",pt.size.factor = 1500,alpha = 0.5)

SpatialFeaturePlotBlend(object = Pyelonephritis_InfectionSubslides, features = c("Sell", "Cxcr2"),
                           top_left = "blue", bottom_right = "yellow",
                           bottom_left = "white", top_right = "black",
                        assay = "Spatial",pt.size.factor = 1500,alpha = 0.5)

SpatialFeaturePlotBlend(object = Pyelonephritis_InfectionSubslides, features = c("Sell", "Cxcr2"),
                           top_left = "lightblue", bottom_right = "red",
                           bottom_left = "white", top_right = "black",
                        assay = "Spatial",pt.size.factor = 1500,alpha = 0.5)
...
colnames(t(Pyelonephritis_Infection[["predictions"]]@data))
t(Pyelonephritis_Infection[["predictions"]]@data)[,"NK cells"]
...
CelltypeOrder <- c("Neutrophils-0", "Neutrophils-1", "Neutrophils-2", "Neutrophils-3","ResMac-1", "ResMac_2","MonoDerivedMac1", "ResDendric", "ResMac_3", "MonoDerivedMac2","B cells", "T cells","NK cells" )
...
ClusterAllCluster_Median <-
  FinalIntergratedTable %>% group_by(Annotation) %>% summarise(
    `Neutrophils-0` = median(`Neutrophils-0`),
   `Neutrophils-1` = median(`Neutrophils-1`),
    `Neutrophils-2` = median(`Neutrophils-2`),
   `Neutrophils-3` = median(`Neutrophils-3`),
    `ResMac-1` = median(`ResMac-1`),
    `ResMac-2` = median(`ResMac-2`),
    `MonoDerivedMac1` = median(`MonoDerivedMac1`),
    ResDendric = median(`ResDendric`),
   `ResMac-3` = median(`ResMac-3`),
    MonoDerivedMac2 = median(`MonoDerivedMac2`),
    `B cells` = median(`B cells`),
    `T cells` = median(`T cells`),
   `NK cells` = median(`NK cells`),
    .groups = 'drop'
  ) %>% as.data.frame()
...
ClusterAllCluster_Mean <-
  FinalIntergratedTable %>% group_by(Annotation) %>% summarise(
     `Neutrophils-0` = mean(`Neutrophils-0`),
   `Neutrophils-1` = mean(`Neutrophils-1`),
    `Neutrophils-2` = mean(`Neutrophils-2`),
   `Neutrophils-3` = mean(`Neutrophils-3`),
    `ResMac-1` = mean(`ResMac-1`),
    `ResMac-2` = mean(`ResMac-2`),
    `MonoDerivedMac1` = mean(`MonoDerivedMac1`),
    ResDendric = mean(`ResDendric`),
   `ResMac-3` = mean(`ResMac-3`),
    MonoDerivedMac2 = mean(`MonoDerivedMac2`),
    `B cells` = mean(`B cells`),
    `T cells` = mean(`T cells`),
   `NK cells` = mean(`NK cells`),
    .groups = 'drop'
  ) %>% as.data.frame()
...
### we calculate the mean value for each cluster and also based on the timepoints
...
setwd("/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_Immune_V12092024/InfMadullaRegionDir/")
