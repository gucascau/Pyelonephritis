...
patchwork<- SpatialFeaturePlot(Pyelonephritis.integrated, features = c("Lcn2","Cst3","Havcr1"),  ncol = 11, crop = TRUE) + theme(axis.title.x=element_blank(),axis.title.y=element_blank(),axis.text.x=element_blank(),axis.text.y=element_blank(),axis.ticks.x=element_blank(),axis.ticks.y=element_blank());

test<-wrap_plots(plots = patchwork, ncol = 1)

patchwork


get_shared_scale(patchwork, scale = scale_fill_viridis_c("z"))
#patchwork+ scale_fill_continuous(limits = c(0,6), breaks = c(0,3,6)) +scale_colour_gradient(low = "white", high = "red")
set_scale_union(test, scale = scale_fill_viridis_c("z"))  
  
  theme() +scale_fill_continuous(limits = c(0,6), breaks = c(0,3,6)) +scale_colour_gradient(low = "white", high = "red")
print(map)
#patchwork
# 
...
pdf("VlnPlots_Pyeloneprhitis_KidneyInjuredBiomarker_v0323.pdf", height = 12, width = 40)
VlnPlot(object = Pyelonephritis.integrated,features = c("Lcn2","Cst3","Havcr1"), ncol = 1, group.by = "group",split.by = "seurat_clusters",pt.size = 0) +
  #stat_summary(fun.y = median, fun.ymin = median, fun.ymax = median, geom = "crossbar", width = 0.5) +
  theme(legend.position = 'right')
dev.off()

VlnPlot(object = Pyelonephritis.integrated,features = c("Calca"), ncol = 1, group.by = "group",split.by = "seurat_clusters",pt.size = 0) + theme(legend.position = 'right')
SpatialFeaturePlot(Pyelonephritis.integrated, features = c("Calca"),  ncol = 11, crop = TRUE) 
