---
title: "Single Cell cell-cell communication at day7"
...
Outdir = "/Users/XXW004/Documents/Projects/RuizRosado/PIPseq/Results/Evaluation_V022723/CellEvaluation_Subclustering_V0803/Cell_CellCommunication/AllDates/"
...
```
# ############################## 
# We concentrate on all dates
# ############################## 
```{r}

# we  select the all dates
SingleCellObj$DataSet
Pyelonephritis_InfectionAllDates <- SingleCellObj
   # subset (SingleCellObj, subset= (DataSet == "7DPI"))

nrow(Pyelonephritis_InfectionAllDates@meta.data)
SingleCellObj
### create a cellchat object

cellChat <- createCellChat(object = Pyelonephritis_InfectionAllDates, group.by = "DataSet", assay = "RNA")
...
saveRDS(cellchat, file = "SingleCellAllDates.rds")

# read the object
#cellchat<- readRDS("SingleCellAllDates.rds")
...
pdf("SignalingSedingFromAllCellTypes_AllDates.pdf", height = 6, width = 12)
...
nrow(mat)
pdf("SignalingSedingFromEachCelltype_AllDates.pdf", height = 30, width = 30)
...
#cellchat@net$LRs
pdf("Pathwayshow_AllDates.pdf", height = 6, width = 6)
netVisual_aggregate(cellchat, signaling = pathways.show,  vertex.receiver = vertex.receiver)
dev.off()
# Circle plot
pdf("Pathwayshow_Cxcl_CCL_AllDates.pdf", height = 6, width = 6)
par(mfrow=c(1,1))
netVisual_aggregate(cellchat, signaling = pathways.show, layout = "circle")
dev.off()
# using the chord diagram
pdf("Pathwayshow_Cxcl_CCL_AllDates_Chord.pdf", height = 5, width = 5)
...
pdf("Pathwayshow_Cxcl_CCL_AllDates_netAnalysis_contribution.pdf", height = 5, width = 5)
...
for (i in pairLR.CXCL$interaction_name){
  pdf(paste0(i,"PairedPathwayshow_Chord_AllDates.pdf"), height = 4, width = 4)
...
pdf("PairedPathwayshow_CXCL1_CXCR2_Chord_allDates.pdf", height = 5, width = 5)
netVisual_individual(cellchat, signaling = pathways.show, pairLR.use = LR.show, layout = "chord")
dev.off()

pdf("PairedPathwayshow_CCL_CXCL_DotPlots_allDates.pdf", height = 5, width = 10)
...
Pyelonephritis_InfectionAllDates[["predictions"]]
InfectedMat_CorMed<- t(Pyelonephritis_InfectionAllDates[["predictions"]]@data)
...
#DefaultAssay(Pyelonephritis_InfectionAllDates) <- "Spatial"
...
names(pal) <- ct
#pal
rownames(Pyelonephritis_InfectionAllDates)
colnames(Pyelonephritis_InfectionAllDates)
str(Pyelonephritis_InfectionAllDates)
...
SpatialScatterPieCorMed <-plotSpatialScatterpie(
  x= Pyelonephritis_InfectionAllDates,
...
Seurat::SpatialDimPlot(Pyelonephritis_InfectionAllDates,pt.size.factor = 1500)
# Prepare input data for CelChat analysis
data.input = Seurat::GetAssayData(Pyelonephritis_InfectionAllDates, slot = "data", assay = "SCT") # normalized data matrix
#> Warning: The `slot` argument of `GetAssayData()` is deprecated as of SeuratObject 5.0.0.
Seurat::Idents(Pyelonephritis_InfectionAllDates)
...
meta = data.frame(labels = Seurat::Idents(Pyelonephritis_InfectionAllDates), samples = "sample1", row.names = names(Seurat::Idents(Pyelonephritis_InfectionAllDates))) # manually create a dataframe consisting of the cell labels
meta
...
spatial.locs = Seurat::GetTissueCoordinates(Pyelonephritis_InfectionAllDates, scale = NULL, cols = c("imagerow", "imagecol")) 
...
RequestedClusters<- levels(Idents(Pyelonephritis_InfectionAllDates))
RequestedClusters
...
DefaultAssay(Pyelonephritis_InfectionAllDates) <-"SCT"
Finalcelltypemarkers<- c("Lrp2","Slc34a1","Slc13a3", # Proximal tubule cells (PT)
 ...
 for (i in Finalcelltypemarkers){
   CellTypeMarkerSpatialFeature<- SpatialFeaturePlot(Pyelonephritis_InfectionAllDates, features = i, pt.size.factor = 6000, crop = TRUE)
...
? FindAllMarkers
Pyelonephritis_InfectionAllDates <- ScaleData(object = Pyelonephritis_InfectionAllDates, features = rownames(Pyelonephritis_InfectionAllDates))
# MAST was used to calculate the cluster
CorMed_nichesBiomarkers <-
  FindAllMarkers(
    object = Pyelonephritis_InfectionAllDates,
...
CorMed_nichesBiomarkersTop5Heatmap<- DoHeatmap(Pyelonephritis_InfectionAllDates, features = CorMed_nichesBiomarkersTop5 , slot = "scale.data") + scale_fill_gradientn(colors = c("blue", "white", "red")) + theme(axis.text.y = element_text(size = 10))
ggsave(
...
CorMedCorMedPTSubBiomarkersTop5DotPlot <-
  DotPlot(Pyelonephritis_InfectionAllDates, features = CorMed_nichesBiomarkersTop5 %>% unique()) +
  RotatedAxis()
...
for (i in Top5PTMarker) {
  ...
  PTmarkerSpatialFeature <-
    SpatialFeaturePlot(Pyelonephritis_InfectionAllDates, features = i, slot = "scale.data",pt.size.factor = 4)
...
Idents(Pyelonephritis_InfectionAllDates)
for (n in Idents(Pyelonephritis_InfectionAllDates)) {
...
CorMedCluster<- data.frame(Cell = names(Idents(Pyelonephritis_InfectionAllDates)),   
                              CellType = as.character(Idents(Pyelonephritis_InfectionAllDates)),
                              stringsAsFactors = FALSE)

?progeny
Pyelonephritis_InfectionAllDates <- progeny(Pyelonephritis_InfectionAllDates, scale=FALSE, organism="Mouse", top=500, perm=1, return_assay = TRUE,assay_name = "SCT")
Pyelonephritis_InfectionAllDates
# setup the default assay as progeny
DefaultAssay(object = Pyelonephritis_InfectionAllDates) <- "progeny"

Pyelonephritis_InfectionAllDates <- Seurat::ScaleData(Pyelonephritis_InfectionAllDates, assay = "progeny") 
## We can now directly apply Seurat functions in our Progeny scores. 
## For instance, we scale the pathway activity scores. 
progeny_scores_df <- 
  as.data.frame(t(GetAssayData(Pyelonephritis_InfectionAllDates, slot = "scale.data", 
                               assay = "progeny"))) %>%
...
progeny_hmap
dev.off()
test<- FeaturePlot(Pyelonephritis_InfectionAllDates, features = "NFkB")
...
for (i in ProgenyFeatures ){
  p2 <-(FeaturePlot(Pyelonephritis_InfectionAllDates, features = i) & 
  scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red'))& 
  ggtitle(paste0(i," activity"))
  
  ggsave(paste0(i, "_CorMedInfected_cluster_featurePlot.pdf"), plot = p2, width = 6, height = 6)
  
  p3 <- (SpatialFeaturePlot(Pyelonephritis_InfectionAllDates, features = i, pt.size.factor = 4) +ggtitle(paste0(i," activity")))
...
Pyelonephritis_InfectionAllDates@assays$SCT@data %>% head()
mat <- as.matrix(Pyelonephritis_InfectionAllDates@assays$SCT@data)
# Run ulm
...
Pyelonephritis_InfectionAllDates[['tfsulm']] <- acts %>%
...
# Change assay
DefaultAssay(object = Pyelonephritis_InfectionAllDates) <- "tfsulm"

# Scale the data
Pyelonephritis_InfectionAllDates <- ScaleData(Pyelonephritis_InfectionAllDates)
Pyelonephritis_InfectionAllDates@assays$tfsulm@data <- Pyelonephritis_InfectionAllDates@assays$tfsulm@scale.data

# Explore the top 20 more varaible TFs
n_tfs <- 20

t(as.matrix(Pyelonephritis_InfectionAllDates@assays$tfsulm@data))
# Extract activities from object as a long dataframe
df <- t(as.matrix(Pyelonephritis_InfectionAllDates@assays$tfsulm@data)) %>%
  as.data.frame() %>%
  mutate(cluster = Idents(Pyelonephritis_InfectionAllDates)) %>%
...
for (m in tfs){
  p2 <- (SpatialFeaturePlot(Pyelonephritis_InfectionAllDates, features = m,pt.size.factor = 4) +
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
...
Pyelonephritis_InfectionAllDates
DefaultAssay(object = Pyelonephritis_InfectionAllDates) <- "SCT"

for (n in tfs){
  p3 <- (FeaturePlot(Pyelonephritis_InfectionAllDates, features = n,slot= "scale.data") + 
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression')))
  
  p4 <- (SpatialFeaturePlot(Pyelonephritis_InfectionAllDates, features = n, pt.size.factor = 4, slot= "scale.data") +
 # scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression (time)')))
 
  ggsave(paste0("TFExpression_",n,"_Pyelonephritis_InfectionAllDates_FeaturePlot.pdf"), plot = p3, width = 6, height = 6)
  ggsave(paste0("TFExpression_",n,"_Pyelonephritis_InfectionAllDates_SpatilFeaturePlot.pdf"), plot = p4, width = 6, height = 6)
...
GeneOrderByPheatmap
?DotPlot
TFDotPlot <- DotPlot(Pyelonephritis_InfectionAllDates, features = rev(GeneOrderByPheatmap)) +
...
```{r}
DefaultAssay(Pyelonephritis_InfectionAllDates) <- "SCT"
Pyelonephritis_InfectionAllDates@meta.data %>% head()
#Idents(Pyelonephritis_InfectionAllDates) <- Pyelonephritis_InfectionAllDates@meta.data$Sub2

# reset the identity, Combine the PT cells

Idents(Pyelonephritis_InfectionAllDates)
Pyelonephritis_InfectionAllDates@meta.data
# adjust the levels
# Idents(Pyelonephritis_InfectionAllDates) <-
# ordered(factor(Idents(Pyelonephritis_InfectionAllDates)), levels=c( "5_1","10_1","10_0","2_1","2_0","5_0","5_2","4","7","0","1","9","6","3","8"))

levels(Idents(Pyelonephritis_InfectionAllDates))
```
...
Pyelonephritis_InfectionAllDates = alias_to_symbol_seurat(Pyelonephritis_InfectionAllDates, "mouse")

Pyelonephritis_InfectionAllDates@meta.data %>% head()
Pyelonephritis_InfectionAllDates@meta.data$IntegrateAnnot %>% table() 

head(weighted_networks$gr) 
DimPlot(Pyelonephritis_InfectionAllDates)

# If you use convert_to_alias before here, this one won't work
Pyelonephritis_InfectionAllDates = Seurat::PrepSCTFindMarkers(Pyelonephritis_InfectionAllDates, assay = "SCT", verbose = FALSE)
...
lr_network %>% head()
Pyelonephritis_InfectionAllDates %>% subset(features = lr_network$ligand %>% intersect(rownames(Pyelonephritis_InfectionAllDates)))
# calcualte the niches difference 
DE_sender = nichenetr::calculate_niche_de(seurat_obj  = Pyelonephritis_InfectionAllDates %>% subset(features = lr_network$ligand %>% intersect(rownames(Pyelonephritis_InfectionAllDates))), niches = niches, type = "sender", assay_oi = "SCT") # only ligands important for sender cell types
DE_receiver = nichenetr::calculate_niche_de(seurat_obj  = Pyelonephritis_InfectionAllDates %>% subset(features = lr_network$receptor %>% unique()), niches = niches, type = "receiver", assay_oi = "SCT") # only receptors now, later on: DE analysis to find targets
...
DE_receiver_targets = nichenetr::calculate_niche_de_targets(seurat_obj = Pyelonephritis_InfectionAllDates, niches = niches, lfc_cutoff = lfc_cutoff, expression_pct = expression_pct, assay_oi = "SCT") 
...
nichenet_output = nichenet_seuratobj_aggregate(
  seurat_obj = Pyelonephritis_InfectionAllDates, 
  receiver = "PT", 
...
avg_expression_ligands = AverageExpression(Pyelonephritis_InfectionAllDates, features = nichenet_output$top_ligands)
...
Pyelonephritis_InfectionAllDates
  # there seems to be an error in the susetting, so we removed other igmages
  #names(Pyelonephritis_InfectionSubslides@images)
SelectedImageName <- names(Pyelonephritis_InfectionAllDates@images)[5]
SelectedImageName
OtherImages <- Allimagmes[Allimagmes != SelectedImageName]
OtherImages
Pyelonephritis_InfectionAllDates@images[OtherImages] = NULL

#nrow(Pyelonephritis_InfectionAllDates@meta.data )
Pyelonephritis_InfectionAllDatesDimplot <-SpatialDimPlot(Pyelonephritis_InfectionAllDates, crop = T)

ggsave("Pyelonephritis_InfectionAllDatesDimplot.pdf", plot =Pyelonephritis_InfectionAllDatesDimplot, height = 6, width = 6 )

# the prediction of the object
Pyelonephritis_InfectionAllDates[["predictions"]]
InfectedMat_D7<- t(Pyelonephritis_InfectionAllDates[["predictions"]]@data)
...
SpatialD7MarkersDotPlot<- DotPlot(Pyelonephritis_InfectionAllDates, features = Finalcelltypemarkers,cols = "Spectral", dot.scale = 4) +RotatedAxis() +theme(legend.text = element_text(size = 12), legend.title = element_text(size = 8))

ggsave("SpatialD7MarkersDotPlot.pdf", plot = SpatialD7MarkersDotPlot, height = 6, width = 14)
Pyelonephritis_InfectionAllDates

for (i in Finalcelltypemarkers){
   CellTypeMarkerSpatialFeature<- SpatialFeaturePlot(Pyelonephritis_InfectionAllDates, features = i, pt.size.factor = 1.6, crop = TRUE) 
   #&
       #   ggplot2::scale_fill_gradientn(colours = myPalette(4), limits=c(0,1))
...
#nrow(Pyelonephritis_InfectionAllDates@meta.data )
SpatialDimPlot(Pyelonephritis_InfectionAllDates)
...
coords <- GetTissueCoordinates(Pyelonephritis_InfectionAllDates)
coords
Pyelonephritis_InfectionAllDates@meta.data %>% head()
summary(coords)
Pyelonephritis_InfectionAllDates@images$slice8

# we select the regions for the cluster
Pyelonephritis_InfectionAllDates <- subset (Pyelonephritis_InfectionAllDates, slice8_imagerow > 140 | slice8_imagecol > 200, invert = TRUE)
Pyelonephritis_InfectionAllDates <- subset (Pyelonephritis_InfectionAllDates, idents = c("N13","N14","N15","N16","N17"))
InfectedCorMedRegionD1Spatial <-SpatialDimPlot(Pyelonephritis_InfectionAllDates,crop = TRUE, label = FALSE)
...
Pyelonephritis_InfectionAllDates@meta.data[,c("orig.ident","Annotation")]
## merge is way too troublesome, therefore
FinalIntergratedTableAbcess<- merge(Pyelonephritis_InfectionAllDates@meta.data[,c("orig.ident","Annotation")], t(Pyelonephritis_InfectionAllDates[["predictions"]]@data), all=TRUE, by="row.names") 
?Heatmap
...
pal <- colorRampPalette(paletteMartin)(length(ct))
names(pal) <- ct
Pyelonephritis_InfectionAllDatesCellTypeMatrix <-t(Pyelonephritis_InfectionAllDates[["predictions"]]@data)

Pyelonephritis_InfectionAllDatesCellTypeMatrixNoMax <- Pyelonephritis_InfectionAllDatesCellTypeMatrix[,!colnames(Pyelonephritis_InfectionAllDatesCellTypeMatrix) %in% c("max")]
Pyelonephritis_InfectionAllDatesCellTypeMatrixNoMax
# generate the scatter pie plot
colnames(Pyelonephritis_InfectionAllDatesCellTypeMatrixNoMax)
...
InfectedCorMedRegionD1SpatialPie<- plotSpatialScatterpie(
  x= Pyelonephritis_InfectionAllDates,
  y= Pyelonephritis_InfectionAllDatesCellTypeMatrixNoMax,
  cell_types = colnames(Pyelonephritis_InfectionAllDatesCellTypeMatrixNoMax),
    img = TRUE,
    scatterpie_alpha = 1,
...
# correlation
InfectedCorMedRegionD1SpatialPieCorrelationMatrix <- plotCorrelationMatrix(Pyelonephritis_InfectionAllDatesCellTypeMatrixNoMax)
...
Pyelonephritis_InfectionAllDatesCellTypeMatrixNoMaxLarger<- Pyelonephritis_InfectionAllDatesCellTypeMatrixNoMax[,colSums(Pyelonephritis_InfectionAllDatesCellTypeMatrixNoMax[])>0]
InfectedPevRegionD1Interactions<- plotInteractions(Pyelonephritis_InfectionAllDatesCellTypeMatrixNoMaxLarger, "heatmap") + scale_fill_gradientn(colors = c("blue", "white", "red"))
?plotInteractions
...
Pyelonephritis_InfectionAllDates <- subset (Pyelonephritis_InfectionAllDates, (slice8_imagerow >310 & slice8_imagerow <450 )  & (slice8_imagecol < 330 & slice8_imagecol> 220) )
Pyelonephritis_InfectionAllDates <- subset (Pyelonephritis_InfectionAllDates, idents = c("N8","N10","N11","N12","N15"))
InfectedCorMedRegionD1Spatial <-SpatialDimPlot(Pyelonephritis_InfectionAllDates,label = TRUE)
InfectedCorMedRegionD1Spatial
ggsave("Spatial_infected_day7_SpatialDimplot.pdf",
      plot = InfectedCorMedRegionD1Spatial,
      height = 3,
...
InfectedCorMedRegionD1Spatial
Pyelonephritis_InfectionAllDates
DefaultAssay(Pyelonephritis_InfectionAllDates) <- "integrated"
nbs_2.markers <- FindAllMarkers(Pyelonephritis_InfectionAllDates,test.use ="MAST")
nbs_2.markers
??SubsetSTData
#nbs_2.markers$gene <- rownames(nbs_2.markers)

nbs_2.markers
#se.subset <- SubsetSTData(Pyelonephritis_InfectionAllDates, expression = nbs_2 %in% c("N12", "N15"))
...
DotPlot(Pyelonephritis_InfectionAllDates, features = sorted.marks$gene %>% unique()) +RotatedAxis()
#DoHeatmap(Pyelonephritis_InfectionAllDates, features = sorted.marks$gene, group.colors = c("red", "lightgray"))
