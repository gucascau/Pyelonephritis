...
Outdir = "/Users/XXW004/Documents/Projects/RuizRosado/PIPseq/Results/Evaluation_V022723/CellEvaluation_Subclustering_V0803/Cell_CellCommunication/ComparingDates/"
...
```
# ############################## 
# We merge the date 0, 3, 7, 28
# ############################## 
```{r}
ptm = Sys.time()
CellChatDay0 <- readRDS("/Users/XXW004/Documents/Projects/RuizRosado/PIPseq/Results/Evaluation_V022723/CellEvaluation_Subclustering_V0803/Cell_CellCommunication/Control/cellchat_PN_D0_final.rds")
CellChatDay3 <- readRDS("/Users/XXW004/Documents/Projects/RuizRosado/PIPseq/Results/Evaluation_V022723/CellEvaluation_Subclustering_V0803/Cell_CellCommunication/OverallDay3/cellchat_PN_D3_final.rds")
CellChatDay7 <- readRDS("/Users/XXW004/Documents/Projects/RuizRosado/PIPseq/Results/Evaluation_V022723/CellEvaluation_Subclustering_V0803/Cell_CellCommunication/OverallDay7/cellchat_PN_D7_final.rds")
CellChatDay28 <- readRDS("/Users/XXW004/Documents/Projects/RuizRosado/PIPseq/Results/Evaluation_V022723/CellEvaluation_Subclustering_V0803/Cell_CellCommunication/OverallDay28/cellchat_PN_D28_final.rds")

# the cell type has slightly different
# we load the each and update the 

CellChatDay0<- updateCellChat(CellChatDay0)
CellChatDay3<- updateCellChat(CellChatDay3)
CellChatDay7<- updateCellChat(CellChatDay7)
CellChatDay28<- updateCellChat(CellChatDay28)

#group.new0 = levels(CellChatDay0@idents)


levels(CellChatDay0@idents)
levels(CellChatDay3@idents)
levels(CellChatDay7@idents)
levels(CellChatDay28@idents)
### due to the four objects have different cell clusters, we must create common idents and lift the sample that is missing it.
union(union(union(levels(CellChatDay0@idents),levels(CellChatDay3@idents)),levels(CellChatDay7@idents)),levels(CellChatDay28@idents))

# the new group contain all the new clusters
group.new = unique(union(union(union(levels(CellChatDay0@idents),levels(CellChatDay3@idents)),levels(CellChatDay7@idents)),levels(CellChatDay28@idents)))
CellChatDay0@idents

#CellChatDay0@idents<- liftCellChat(CellChatDay0@idents, group.new)


# CellChatDay3 <- liftCellChat(CellChatDay3, group.new0)

object.list <- list(D0 = CellChatDay0, D3 = CellChatDay3,D7 =CellChatDay7,D28 = CellChatDay28 )
object.list
# merge the object
cellchat <- mergeCellChat(object.list, add.names = names(object.list), cell.prefix = TRUE)
cellchat <- liftCellChat(cellchat , group.new = levels( cellchat@idents$joint) )

Group<-levels(cellchat@meta$datasets)

execution.time = Sys.time() - ptm
print(as.numeric(execution.time, units = "secs"))

# we export the merged 
# Users can now export the merged CellChat object and the list of the two separate objects for later use
save(object.list, file = "cellchat_object.list_PN_alldates.RData")
save(cellchat, file = "cellchat_merged_PN_alldatesRData")
```
...
gg1<-compareInteractions(cellchat, show.legend = T, group = Group)
gg2<-compareInteractions(cellchat, show.legend = F, group = Group, measure = "weight")

pdf("CellChat_Alldates_TotalInteractions.pdf", height =3.5, width = 7)
gg1 + gg2
dev.off()
```
...
```{r}
pdf("CellChat_Alldates_netVisual_diffInteraction.pdf", height =8, width = 10)
...
pdf("CellChat_Alldates_netVisual_circle.pdf", height =8, width = 10)
par(mfrow = c(1,4), xpd=TRUE)
for (i in 1:length(object.list)) {
...
gg <- list()
pdf("CellChat_Alldates_netAnalysis_signalingRole_scatter.pdf", height =10, width = 12)
object.list[[2]] <-netAnalysis_computeCentrality(object.list[[2]])
tes2<-netAnalysis_signalingRole_scatter(object.list[[2]], title = names(object.list)[2],weight.MinMax =c(0,600))

pdf("CellChat_Alldates_netAnalysis_signalingRole_scatter_d3.pdf", height =10, width = 12)
tes2

dev.off()
#netAnalysis_computeCentrality(object.list[[1]])
for (i in 1:length(object.list)) {
  object.list[[i]]<-netAnalysis_computeCentrality(object.list[[i]])
  gg[[i]] <- netAnalysis_signalingRole_scatter(object.list[[i]], title = names(object.list)[i],weight.MinMax =c(0,600))
}
...
pdf("CellChat_Alldates_netAnalysis_rankNet.pdf", height =10, width = 12)
gg1 <- rankNet(cellchat, mode = "comparison", measure = "weight", sources.use = NULL, targets.use = NULL, stacked = F, do.stat = TRUE)
gg1
dev.off()

cellchat
netVisual_bubble(cellchat, sources.use = 4, targets.use = c(5:30),  comparison = c(1, 2,3,4), angle.x = 45)

levels( cellchat@idents$joint)
gg1 <- netVisual_bubble(cellchat, sources.use = 4, targets.use = c(5:33),  comparison = c(1, 2,3,4),  title.name = "Increased signaling in LS", angle.x = 45, remove.isolate = T)
gg1

gg2 <- netVisual_bubble(cellchat, sources.use = 4, targets.use = c(1:33),  comparison = c(1, 2,3,4), title.name = "Decreased signaling in LS", angle.x = 45, remove.isolate = T)
gg2
gg2 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.down, sources.use = 4, targets.use = c(5:11), comparison = c(1, 2,2,3,4),  angle.x = 90, remove.isolate = T,title.name = paste0("Down-regulated signaling in ", names(object.list)[2]))
# library(reticulate)
