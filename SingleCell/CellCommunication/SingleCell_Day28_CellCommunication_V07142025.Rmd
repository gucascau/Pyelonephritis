...
Outdir = "/Users/XXW004/Documents/Projects/RuizRosado/PIPseq/Results/CellCommunication_V07212025/OverallDay28/"
...
Pyelonephritis_InfectionDay28 <- 
    subset (SingleCellObj, subset= (FinalCombinedDate == "28DPI"))
Pyelonephritis_InfectionDay28
nrow(Pyelonephritis_InfectionDay28@meta.data)
Pyelonephritis_InfectionDay28 %>% tail()
### create a cellchat object


cellchat <- createCellChat(object = Pyelonephritis_InfectionDay28, group.by = "FinalCombinedAnnation", assay = "RNA")
...
nrow(mat)
pdf("SignalingSedingFromEachCelltype_Day28.pdf", height = 25, width = 25)
...
for (i in pairLR.CXCL$interaction_name){
  pdf(paste0(i,"PairedPathwayshow_Chord_Day28.pdf"), height = 6, width = 7)
...
#### day 7
# we select each dates and perform the cellchat
seurat.obj.Day28 <- subset (SingleCellObj, subset= (DataSet == "7DPI"))

cellChatDay28 <- createCellChat(object = seurat.obj.Day28, group.by = "DataSet", assay = "RNA")
...
# subset the expression data of signaling genes for saving computation cost
cellChatDay28@DB <- CellChatDB

#Preprocessing the expression data for cell-cell communication analysis
# subset the expression data of signaling genes for saving computation cost
#cellchat <- subsetData(cellchat) # This step is necessary even if using the whole 
cellChatDay28 <- subsetData(cellChatDay28) # This step is necessary even if using the whole database

#future::plan("multisession", workers = 4) # do parallel

cellChatDay28 <- identifyOverExpressedGenes(cellChatDay28)
cellChatDay28 <- identifyOverExpressedInteractions(cellChatDay28)
#> The number of highly variable ligand-receptor pairs used for signaling inference is 692
...
#### Part II: Inference of cell-cell communication network
ptm = Sys.time()
cellChatDay28 <- computeCommunProb(cellChatDay28, type = "triMean")
...
object.list <- list(NL = cellChatDay0, LS = cellChatDay28)
