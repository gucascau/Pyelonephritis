...
Outdir = "/Users/XXW004/Documents/Projects/RuizRosado/PIPseq/Results/CellCommunication_V07212025/OverallDay7/"
...
```
# ############################## 
# We concentrate on day 7
# ############################## 
```{r}

# we  select the all dates
SingleCellObj@meta.data %>% head()
Pyelonephritis_InfectionDay7 <- 
    subset (SingleCellObj, subset= (FinalCombinedDate == "7DPI"))
Pyelonephritis_InfectionDay7
nrow(Pyelonephritis_InfectionDay7@meta.data)
Pyelonephritis_InfectionDay7 %>% tail()
### create a cellchat object


cellchat <- createCellChat(object = Pyelonephritis_InfectionDay7, group.by = "FinalCombinedAnnation", assay = "RNA")
...
saveRDS(cellchat, file = "SingleCell_Date7_Cellchat.rds")

# read the object of date 0
cellchat <- readRDS("SingleCell_Date7_Cellchat.rds")
# read the object
...
pdf("SignalingSedingFromAllCellTypes_Day7.pdf", height = 6, width = 12)
...
nrow(mat)
pdf("SignalingSedingFromEachCelltype_Day7.pdf", height = 25, width = 25)
...
#cellchat@net$LRs
pdf("Pathwayshow_Day7.pdf", height = 6, width = 6)
netVisual_aggregate(cellchat, signaling = cellchat@netP$pathways,  vertex.receiver = vertex.receiver)
dev.off()
# Circle plot
pdf("Pathwayshow_Cxcl_CCL_Day7.pdf", height = 6, width = 6)
par(mfrow=c(1,1))
netVisual_aggregate(cellchat, signaling = pathways.show, layout = "circle")
dev.off()
# using the chord diagram
pdf("Pathwayshow_Cxcl_CCL_Day7_Chord.pdf", height = 5, width = 5)
...
pdf("Pathwayshow_Cxcl_CCL_Day7_netAnalysis_contribution.pdf", height = 5, width = 4)
...
for (i in pairLR.CXCL$interaction_name){
  pdf(paste0(i,"PairedPathwayshow_Chord_Day7.pdf"), height = 6, width = 7)
...
pdf("PairedPathwayshow_CXCL1_CXCR2_Chord_Day7.pdf", height = 5, width = 5)
netVisual_individual(cellchat, signaling = pathways.show, pairLR.use = LR.show, layout = "chord")
dev.off()

pdf("PairedPathwayshow_CCL_CXCL_DotPlots_Day7.pdf", height = 5, width = 10)
...
saveRDS(cellchat, file = "cellchat_PN_D7_final.rds")
cellchat
```
...
Pyelonephritis_InfectionDay7[["predictions"]]
InfectedMat_CorMed<- t(Pyelonephritis_InfectionDay7[["predictions"]]@data)
...
#DefaultAssay(Pyelonephritis_InfectionDay7) <- "Spatial"
...
names(pal) <- ct
#pal
rownames(Pyelonephritis_InfectionDay7)
colnames(Pyelonephritis_InfectionDay7)
str(Pyelonephritis_InfectionDay7)
...
SpatialScatterPieCorMed <-plotSpatialScatterpie(
  x= Pyelonephritis_InfectionDay7,
...
Seurat::SpatialDimPlot(Pyelonephritis_InfectionDay7,pt.size.factor = 1500)
# Prepare input data for CelChat analysis
data.input = Seurat::GetAssayData(Pyelonephritis_InfectionDay7, slot = "data", assay = "SCT") # normalized data matrix
#> Warning: The `slot` argument of `GetAssayData()` is deprecated as of SeuratObject 5.0.0.
Seurat::Idents(Pyelonephritis_InfectionDay7)
...
meta = data.frame(labels = Seurat::Idents(Pyelonephritis_InfectionDay7), samples = "sample1", row.names = names(Seurat::Idents(Pyelonephritis_InfectionDay7))) # manually create a dataframe consisting of the cell labels
meta
...
spatial.locs = Seurat::GetTissueCoordinates(Pyelonephritis_InfectionDay7, scale = NULL, cols = c("imagerow", "imagecol")) 
...
RequestedClusters<- levels(Idents(Pyelonephritis_InfectionDay7))
RequestedClusters
...
DefaultAssay(Pyelonephritis_InfectionDay7) <-"SCT"
Finalcelltypemarkers<- c("Lrp2","Slc34a1","Slc13a3", # Proximal tubule cells (PT)
 ...
 for (i in Finalcelltypemarkers){
   CellTypeMarkerSpatialFeature<- SpatialFeaturePlot(Pyelonephritis_InfectionDay7, features = i, pt.size.factor = 6000, crop = TRUE)
...
? FindAllMarkers
Pyelonephritis_InfectionDay7 <- ScaleData(object = Pyelonephritis_InfectionDay7, features = rownames(Pyelonephritis_InfectionDay7))
# MAST was used to calculate the cluster
CorMed_nichesBiomarkers <-
  FindAllMarkers(
    object = Pyelonephritis_InfectionDay7,
...
CorMed_nichesBiomarkersTop5Heatmap<- DoHeatmap(Pyelonephritis_InfectionDay7, features = CorMed_nichesBiomarkersTop5 , slot = "scale.data") + scale_fill_gradientn(colors = c("blue", "white", "red")) + theme(axis.text.y = element_text(size = 10))
ggsave(
...
CorMedCorMedPTSubBiomarkersTop5DotPlot <-
  DotPlot(Pyelonephritis_InfectionDay7, features = CorMed_nichesBiomarkersTop5 %>% unique()) +
  RotatedAxis()
...
for (i in Top5PTMarker) {
  ...
  PTmarkerSpatialFeature <-
    SpatialFeaturePlot(Pyelonephritis_InfectionDay7, features = i, slot = "scale.data",pt.size.factor = 4)
...
Idents(Pyelonephritis_InfectionDay7)
for (n in Idents(Pyelonephritis_InfectionDay7)) {
...
CorMedCluster<- data.frame(Cell = names(Idents(Pyelonephritis_InfectionDay7)),   
                              CellType = as.character(Idents(Pyelonephritis_InfectionDay7)),
                              stringsAsFactors = FALSE)

?progeny
Pyelonephritis_InfectionDay7 <- progeny(Pyelonephritis_InfectionDay7, scale=FALSE, organism="Mouse", top=500, perm=1, return_assay = TRUE,assay_name = "SCT")
Pyelonephritis_InfectionDay7
# setup the default assay as progeny
DefaultAssay(object = Pyelonephritis_InfectionDay7) <- "progeny"

Pyelonephritis_InfectionDay7 <- Seurat::ScaleData(Pyelonephritis_InfectionDay7, assay = "progeny") 
## We can now directly apply Seurat functions in our Progeny scores. 
## For instance, we scale the pathway activity scores. 
progeny_scores_df <- 
  as.data.frame(t(GetAssayData(Pyelonephritis_InfectionDay7, slot = "scale.data", 
                               assay = "progeny"))) %>%
...
progeny_hmap
dev.off()
test<- FeaturePlot(Pyelonephritis_InfectionDay7, features = "NFkB")
...
for (i in ProgenyFeatures ){
  p2 <-(FeaturePlot(Pyelonephritis_InfectionDay7, features = i) & 
  scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red'))& 
  ggtitle(paste0(i," activity"))
  
  ggsave(paste0(i, "_CorMedInfected_cluster_featurePlot.pdf"), plot = p2, width = 6, height = 6)
  
  p3 <- (SpatialFeaturePlot(Pyelonephritis_InfectionDay7, features = i, pt.size.factor = 4) +ggtitle(paste0(i," activity")))
...
Pyelonephritis_InfectionDay7@assays$SCT@data %>% head()
mat <- as.matrix(Pyelonephritis_InfectionDay7@assays$SCT@data)
# Run ulm
...
Pyelonephritis_InfectionDay7[['tfsulm']] <- acts %>%
...
# Change assay
DefaultAssay(object = Pyelonephritis_InfectionDay7) <- "tfsulm"

# Scale the data
Pyelonephritis_InfectionDay7 <- ScaleData(Pyelonephritis_InfectionDay7)
Pyelonephritis_InfectionDay7@assays$tfsulm@data <- Pyelonephritis_InfectionDay7@assays$tfsulm@scale.data

# Explore the top 20 more varaible TFs
n_tfs <- 20

t(as.matrix(Pyelonephritis_InfectionDay7@assays$tfsulm@data))
# Extract activities from object as a long dataframe
df <- t(as.matrix(Pyelonephritis_InfectionDay7@assays$tfsulm@data)) %>%
  as.data.frame() %>%
  mutate(cluster = Idents(Pyelonephritis_InfectionDay7)) %>%
...
for (m in tfs){
  p2 <- (SpatialFeaturePlot(Pyelonephritis_InfectionDay7, features = m,pt.size.factor = 4) +
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
...
Pyelonephritis_InfectionDay7
DefaultAssay(object = Pyelonephritis_InfectionDay7) <- "SCT"

for (n in tfs){
  p3 <- (FeaturePlot(Pyelonephritis_InfectionDay7, features = n,slot= "scale.data") + 
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression')))
  
  p4 <- (SpatialFeaturePlot(Pyelonephritis_InfectionDay7, features = n, pt.size.factor = 4, slot= "scale.data") +
 # scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression (time)')))
 
  ggsave(paste0("TFExpression_",n,"_Pyelonephritis_InfectionDay7_FeaturePlot.pdf"), plot = p3, width = 6, height = 6)
  ggsave(paste0("TFExpression_",n,"_Pyelonephritis_InfectionDay7_SpatilFeaturePlot.pdf"), plot = p4, width = 6, height = 6)
...
GeneOrderByPheatmap
?DotPlot
TFDotPlot <- DotPlot(Pyelonephritis_InfectionDay7, features = rev(GeneOrderByPheatmap)) +
...
```{r}
DefaultAssay(Pyelonephritis_InfectionDay7) <- "SCT"
Pyelonephritis_InfectionDay7@meta.data %>% head()
#Idents(Pyelonephritis_InfectionDay7) <- Pyelonephritis_InfectionDay7@meta.data$Sub2

# reset the identity, Combine the PT cells

Idents(Pyelonephritis_InfectionDay7)
Pyelonephritis_InfectionDay7@meta.data
# adjust the levels
# Idents(Pyelonephritis_InfectionDay7) <-
# ordered(factor(Idents(Pyelonephritis_InfectionDay7)), levels=c( "5_1","10_1","10_0","2_1","2_0","5_0","5_2","4","7","0","1","9","6","3","8"))

levels(Idents(Pyelonephritis_InfectionDay7))
```
...
Pyelonephritis_InfectionDay7 = alias_to_symbol_seurat(Pyelonephritis_InfectionDay7, "mouse")

Pyelonephritis_InfectionDay7@meta.data %>% head()
Pyelonephritis_InfectionDay7@meta.data$IntegrateAnnot %>% table() 

head(weighted_networks$gr) 
DimPlot(Pyelonephritis_InfectionDay7)

# If you use convert_to_alias before here, this one won't work
Pyelonephritis_InfectionDay7 = Seurat::PrepSCTFindMarkers(Pyelonephritis_InfectionDay7, assay = "SCT", verbose = FALSE)
...
lr_network %>% head()
Pyelonephritis_InfectionDay7 %>% subset(features = lr_network$ligand %>% intersect(rownames(Pyelonephritis_InfectionDay7)))
# calcualte the niches difference 
DE_sender = nichenetr::calculate_niche_de(seurat_obj  = Pyelonephritis_InfectionDay7 %>% subset(features = lr_network$ligand %>% intersect(rownames(Pyelonephritis_InfectionDay7))), niches = niches, type = "sender", assay_oi = "SCT") # only ligands important for sender cell types
DE_receiver = nichenetr::calculate_niche_de(seurat_obj  = Pyelonephritis_InfectionDay7 %>% subset(features = lr_network$receptor %>% unique()), niches = niches, type = "receiver", assay_oi = "SCT") # only receptors now, later on: DE analysis to find targets
...
DE_receiver_targets = nichenetr::calculate_niche_de_targets(seurat_obj = Pyelonephritis_InfectionDay7, niches = niches, lfc_cutoff = lfc_cutoff, expression_pct = expression_pct, assay_oi = "SCT") 
...
nichenet_output = nichenet_seuratobj_aggregate(
  seurat_obj = Pyelonephritis_InfectionDay7, 
  receiver = "PT", 
...
avg_expression_ligands = AverageExpression(Pyelonephritis_InfectionDay7, features = nichenet_output$top_ligands)
...
Pyelonephritis_InfectionDay7
  # there seems to be an error in the susetting, so we removed other igmages
  #names(Pyelonephritis_InfectionSubslides@images)
SelectedImageName <- names(Pyelonephritis_InfectionDay7@images)[5]
SelectedImageName
OtherImages <- Allimagmes[Allimagmes != SelectedImageName]
OtherImages
Pyelonephritis_InfectionDay7@images[OtherImages] = NULL

#nrow(Pyelonephritis_InfectionDay7@meta.data )
Pyelonephritis_InfectionDay7Dimplot <-SpatialDimPlot(Pyelonephritis_InfectionDay7, crop = T)

ggsave("Pyelonephritis_InfectionDay7Dimplot.pdf", plot =Pyelonephritis_InfectionDay7Dimplot, height = 6, width = 6 )

# the prediction of the object
Pyelonephritis_InfectionDay7[["predictions"]]
InfectedMat_D7<- t(Pyelonephritis_InfectionDay7[["predictions"]]@data)
...
SpatialD7MarkersDotPlot<- DotPlot(Pyelonephritis_InfectionDay7, features = Finalcelltypemarkers,cols = "Spectral", dot.scale = 4) +RotatedAxis() +theme(legend.text = element_text(size = 12), legend.title = element_text(size = 8))

ggsave("SpatialD7MarkersDotPlot.pdf", plot = SpatialD7MarkersDotPlot, height = 6, width = 14)
Pyelonephritis_InfectionDay7

for (i in Finalcelltypemarkers){
   CellTypeMarkerSpatialFeature<- SpatialFeaturePlot(Pyelonephritis_InfectionDay7, features = i, pt.size.factor = 1.6, crop = TRUE) 
   #&
       #   ggplot2::scale_fill_gradientn(colours = myPalette(4), limits=c(0,1))
...
#nrow(Pyelonephritis_InfectionDay7@meta.data )
SpatialDimPlot(Pyelonephritis_InfectionDay7)
...
coords <- GetTissueCoordinates(Pyelonephritis_InfectionDay7)
coords
Pyelonephritis_InfectionDay7@meta.data %>% head()
summary(coords)
Pyelonephritis_InfectionDay7@images$slice8

# we select the regions for the cluster
Pyelonephritis_InfectionDay7 <- subset (Pyelonephritis_InfectionDay7, slice8_imagerow > 140 | slice8_imagecol > 200, invert = TRUE)
Pyelonephritis_InfectionDay7 <- subset (Pyelonephritis_InfectionDay7, idents = c("N13","N14","N15","N16","N17"))
InfectedCorMedRegionD1Spatial <-SpatialDimPlot(Pyelonephritis_InfectionDay7,crop = TRUE, label = FALSE)
...
Pyelonephritis_InfectionDay7@meta.data[,c("orig.ident","Annotation")]
## merge is way too troublesome, therefore
FinalIntergratedTableAbcess<- merge(Pyelonephritis_InfectionDay7@meta.data[,c("orig.ident","Annotation")], t(Pyelonephritis_InfectionDay7[["predictions"]]@data), all=TRUE, by="row.names") 
?Heatmap
...
pal <- colorRampPalette(paletteMartin)(length(ct))
names(pal) <- ct
Pyelonephritis_InfectionDay7CellTypeMatrix <-t(Pyelonephritis_InfectionDay7[["predictions"]]@data)

Pyelonephritis_InfectionDay7CellTypeMatrixNoMax <- Pyelonephritis_InfectionDay7CellTypeMatrix[,!colnames(Pyelonephritis_InfectionDay7CellTypeMatrix) %in% c("max")]
Pyelonephritis_InfectionDay7CellTypeMatrixNoMax
# generate the scatter pie plot
colnames(Pyelonephritis_InfectionDay7CellTypeMatrixNoMax)
...
InfectedCorMedRegionD1SpatialPie<- plotSpatialScatterpie(
  x= Pyelonephritis_InfectionDay7,
  y= Pyelonephritis_InfectionDay7CellTypeMatrixNoMax,
  cell_types = colnames(Pyelonephritis_InfectionDay7CellTypeMatrixNoMax),
    img = TRUE,
    scatterpie_alpha = 1,
...
# correlation
InfectedCorMedRegionD1SpatialPieCorrelationMatrix <- plotCorrelationMatrix(Pyelonephritis_InfectionDay7CellTypeMatrixNoMax)
...
Pyelonephritis_InfectionDay7CellTypeMatrixNoMaxLarger<- Pyelonephritis_InfectionDay7CellTypeMatrixNoMax[,colSums(Pyelonephritis_InfectionDay7CellTypeMatrixNoMax[])>0]
InfectedPevRegionD1Interactions<- plotInteractions(Pyelonephritis_InfectionDay7CellTypeMatrixNoMaxLarger, "heatmap") + scale_fill_gradientn(colors = c("blue", "white", "red"))
?plotInteractions
...
Pyelonephritis_InfectionDay7 <- subset (Pyelonephritis_InfectionDay7, (slice8_imagerow >310 & slice8_imagerow <450 )  & (slice8_imagecol < 330 & slice8_imagecol> 220) )
Pyelonephritis_InfectionDay7 <- subset (Pyelonephritis_InfectionDay7, idents = c("N8","N10","N11","N12","N15"))
InfectedCorMedRegionD1Spatial <-SpatialDimPlot(Pyelonephritis_InfectionDay7,label = TRUE)
InfectedCorMedRegionD1Spatial
ggsave("Spatial_infected_day7_SpatialDimplot.pdf",
      plot = InfectedCorMedRegionD1Spatial,
      height = 3,
...
InfectedCorMedRegionD1Spatial
Pyelonephritis_InfectionDay7
DefaultAssay(Pyelonephritis_InfectionDay7) <- "integrated"
nbs_2.markers <- FindAllMarkers(Pyelonephritis_InfectionDay7,test.use ="MAST")
nbs_2.markers
??SubsetSTData
#nbs_2.markers$gene <- rownames(nbs_2.markers)

nbs_2.markers
#se.subset <- SubsetSTData(Pyelonephritis_InfectionDay7, expression = nbs_2 %in% c("N12", "N15"))
...
DotPlot(Pyelonephritis_InfectionDay7, features = sorted.marks$gene %>% unique()) +RotatedAxis()
#DoHeatmap(Pyelonephritis_InfectionDay7, features = sorted.marks$gene, group.colors = c("red", "lightgray"))
