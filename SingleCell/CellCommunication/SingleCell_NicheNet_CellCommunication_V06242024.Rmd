...
# with reference
allen_reference <- readRDS(paste0(Indir,"PIPSeq_PTICPCENDFIBUROIMMFinalAnn_Annotated_v061923.RDS"))
...
allen_reference_day7@meta.data<- allen_reference_day7@meta.data %>%   mutate(IntegrateAnnot = 
           ifelse(Raw_cell_type == "PT", Raw_cell_type,PTICPCENDFIBUROIMMFinalAnn)) 

allen_reference_day7@meta.data %>% head()
allen_reference_day7@meta.data <-
  allen_reference_day7@meta.data %>%mutate(
    FinalAnnotation2 =
      case_when(
        #
        IntegrateAnnot %in% c("Degenerated_URO") ~ "DegURO",
        # NK cell
        IntegrateAnnot %in% c("infPT") ~ "InfPT",
        # B
        IntegrateAnnot %in% c("PROF_EPI") ~ "ProfPT",
        #
        IntegrateAnnot %in% c("DCT-TAL") ~ "DCTTAL",
        .default = as.character(IntegrateAnnot)
      )
  )


#Idents(allen_reference_day7) <- "FinalAnnotation2"
...
allen_reference_day7 <- UpdateSeuratObject(allen_reference_day7)
allen_reference_day7[["DataSet"]]
# Convert gene names
allen_reference_day7 <- alias_to_symbol_seurat(allen_reference_day7, "mouse")
allen_reference_day7 = SetIdent(allen_reference_day7, value = allen_reference_day7[["FinalAnnotation2",drop=TRUE]])
class(Idents(allen_reference_day7))
#allen_reference_day7[["DataSet"]]
```
...
```{r}
# sender
sender_cluster <- c("FIB","MAC","END","NEU")
...
# define receive cluster
table(Idents(allen_reference_day7))
receiver_cluster <- c("PT")
...
#background_expressed_genes
## sender
sender_celltypes <- c("NEU","MAC","FIB","END")
...
list_expressed_genes_sender <-
    ...
    lapply(get_expressed_genes, allen_reference_day7, 0.10,assay_oi = "RNA")
...
```{r}
seurat_obj_receiver <-
    subset(allen_reference_day7, idents = receiver) %>%
    SetIdent(value = "DataSet")

seurat_obj_receiver@meta.data%>% head()
DE_table_receiver <-
...
Potential_ligands
Potential_ligandsDotPlot<-DotPlot(allen_reference_day7,
        features = rev(Potential_ligands),
        cols = "RdYlBu") +
    RotatedAxis() +coord_flip() 

Potential_ligandsDotPlot
```
...
allen_reference_day7%>% Idents() %>% unique()
```
