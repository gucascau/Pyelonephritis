---
title: "Single Cell cell-cell communication at day 0"
...
date: "2025-07-14"
Description: 
#   This script is to profile the cell-cell communications in the single cell resolution in the day 3
...
Scindir = "/Users/XXW004/Documents/Projects/RuizRosado/SpatialTranscriptomic/Results/Deconvolution_Six_V07022025/"
...
# The cell-cell communication output directory for day 0:
Outdir = "/Users/XXW004/Documents/Projects/RuizRosado/PIPseq/Results/CellCommunication_V07212025/OverallDay3//"
...
# we integrated single cell from epithelial, stroma and immune cells
SingleCellObj <- readRDS(file = paste0(Scindir,"PIPSeq_EpithelialStromaImmMergeObjs_v07022025.rds"))
...
SingleCellObj
DimPlot(SingleCellObj, reduction = "umap.harmony", split.by = "FinalCombinedDate", label = T)

```
# ############################## 
# We concentrate on control
# ############################## 
```{r}

# we  select the all dates
SingleCellObj@meta.data %>% head()
Pyelonephritis_InfectionDay3 <- 
    subset (SingleCellObj, subset= (FinalCombinedDate == "3DPI"))
Pyelonephritis_InfectionDay3
nrow(Pyelonephritis_InfectionDay3@meta.data)
Pyelonephritis_InfectionDay3 %>% tail()
### create a cellchat object


cellchat <- createCellChat(object = Pyelonephritis_InfectionDay3, group.by = "FinalCombinedAnnation", assay = "RNA")
...
saveRDS(cellchat, file = "SingleCell_Date3_Cellchat.rds")

# read the object of date 0
cellchat <- readRDS("SingleCell_Date3_Cellchat.rds")
# read the object
#cellchat<- readRDS("SingleCellDay0.rds")
...
pdf("SignalingSedingFromAllCellTypes_Day3.pdf", height = 6, width = 12)
...
nrow(mat)
pdf("SignalingSedingFromEachCelltype_Day3.pdf", height = 25, width = 25)
...
# 
pathways.show <- c("CXCL","CCL","TNF","TGFb","IGFBP","CDH5","EGF") 
pathways.show <- c("CXCL","CCL","IFN-II","CD45","CD52","CD86","ICAM") 
...
#cellchat@net$LRs
pdf("Pathwayshow_Day3.pdf", height = 6, width = 6)
netVisual_aggregate(cellchat, signaling = cellchat@netP$pathways,  vertex.receiver = vertex.receiver)
dev.off()
# Circle plot
pdf("Pathwayshow_Cxcl_CCL_Day3.pdf", height = 6, width = 6)
par(mfrow=c(1,1))
netVisual_aggregate(cellchat, signaling = pathways.show, layout = "circle")
dev.off()
# using the chord diagram
pdf("Pathwayshow_Cxcl_CCL_Day3_Chord.pdf", height = 5, width = 5)
netVisual_aggregate(cellchat, signaling = pathways.show, layout = "chord")
dev.off()

Test<-netAnalysis_contribution(cellchat, signaling = pathways.show,return.data=T)
 Test$LR.contribution
# Compute the contribution of each ligand-receptor pair to the overall signaling pathway and visualize cell-cell communication mediated by a single ligand-receptor pair
# generate the ligand receptor pairs
pdf("Pathwayshow_Cxcl_CCL_Day3_netAnalysis_contribution.pdf", height = 5, width = 4)
...
for (i in pairLR.CXCL$interaction_name){
  pdf(paste0(i,"PairedPathwayshow_Chord_Day3.pdf"), height = 6, width = 7)
  netVisual_individual(cellchat, signaling = pathways.show, pairLR.use = i, layout = "chord")
  dev.off()
}

LR.show <- "CXCL2_CXCR2" # show one ligand-receptor pair
LR.show
# Hierarchy plot
vertex.receiver = seq(1,4) # a numeric vector
...
pdf("PairedPathwayshow_CXCL1_CXCR2_Chord_Day3.pdf", height = 5, width = 5)
netVisual_individual(cellchat, signaling = pathways.show, pairLR.use = LR.show, layout = "chord")
dev.off()

pdf("PairedPathwayshow_CCL_CXCL_DotPlots_Day3.pdf", height = 5, width = 10)
...
saveRDS(cellchat, file = "cellchat_PN_D3_final.rds")
cellchat
```
...
Pyelonephritis_InfectionDay3[["predictions"]]
InfectedMat_CorMed<- t(Pyelonephritis_InfectionDay3[["predictions"]]@data)
...
#DefaultAssay(Pyelonephritis_InfectionDay3) <- "Spatial"
...
names(pal) <- ct
#pal
rownames(Pyelonephritis_InfectionDay3)
colnames(Pyelonephritis_InfectionDay3)
str(Pyelonephritis_InfectionDay3)
...
SpatialScatterPieCorMed <-plotSpatialScatterpie(
  x= Pyelonephritis_InfectionDay3,
...
Seurat::SpatialDimPlot(Pyelonephritis_InfectionDay3,pt.size.factor = 1500)
# Prepare input data for CelChat analysis
data.input = Seurat::GetAssayData(Pyelonephritis_InfectionDay3, slot = "data", assay = "SCT") # normalized data matrix
#> Warning: The `slot` argument of `GetAssayData()` is deprecated as of SeuratObject 5.0.0.
Seurat::Idents(Pyelonephritis_InfectionDay3)
...
meta = data.frame(labels = Seurat::Idents(Pyelonephritis_InfectionDay3), samples = "sample1", row.names = names(Seurat::Idents(Pyelonephritis_InfectionDay3))) # manually create a dataframe consisting of the cell labels
meta
...
spatial.locs = Seurat::GetTissueCoordinates(Pyelonephritis_InfectionDay3, scale = NULL, cols = c("imagerow", "imagecol")) 
...
RequestedClusters<- levels(Idents(Pyelonephritis_InfectionDay3))
RequestedClusters
...
DefaultAssay(Pyelonephritis_InfectionDay3) <-"SCT"
Finalcelltypemarkers<- c("Lrp2","Slc34a1","Slc13a3", # Proximal tubule cells (PT)
 ...
 for (i in Finalcelltypemarkers){
   CellTypeMarkerSpatialFeature<- SpatialFeaturePlot(Pyelonephritis_InfectionDay3, features = i, pt.size.factor = 6000, crop = TRUE)
...
? FindAllMarkers
Pyelonephritis_InfectionDay3 <- ScaleData(object = Pyelonephritis_InfectionDay3, features = rownames(Pyelonephritis_InfectionDay3))
# MAST was used to calculate the cluster
CorMed_nichesBiomarkers <-
  FindAllMarkers(
    object = Pyelonephritis_InfectionDay3,
...
CorMed_nichesBiomarkersTop5Heatmap<- DoHeatmap(Pyelonephritis_InfectionDay3, features = CorMed_nichesBiomarkersTop5 , slot = "scale.data") + scale_fill_gradientn(colors = c("blue", "white", "red")) + theme(axis.text.y = element_text(size = 10))
ggsave(
...
CorMedCorMedPTSubBiomarkersTop5DotPlot <-
  DotPlot(Pyelonephritis_InfectionDay3, features = CorMed_nichesBiomarkersTop5 %>% unique()) +
  RotatedAxis()
...
for (i in Top5PTMarker) {
  ...
  PTmarkerSpatialFeature <-
    SpatialFeaturePlot(Pyelonephritis_InfectionDay3, features = i, slot = "scale.data",pt.size.factor = 4)
...
Idents(Pyelonephritis_InfectionDay3)
for (n in Idents(Pyelonephritis_InfectionDay3)) {
...
CorMedCluster<- data.frame(Cell = names(Idents(Pyelonephritis_InfectionDay3)),   
                              CellType = as.character(Idents(Pyelonephritis_InfectionDay3)),
                              stringsAsFactors = FALSE)

?progeny
Pyelonephritis_InfectionDay3 <- progeny(Pyelonephritis_InfectionDay3, scale=FALSE, organism="Mouse", top=500, perm=1, return_assay = TRUE,assay_name = "SCT")
Pyelonephritis_InfectionDay3
# setup the default assay as progeny
DefaultAssay(object = Pyelonephritis_InfectionDay3) <- "progeny"

Pyelonephritis_InfectionDay3 <- Seurat::ScaleData(Pyelonephritis_InfectionDay3, assay = "progeny") 
## We can now directly apply Seurat functions in our Progeny scores. 
## For instance, we scale the pathway activity scores. 
progeny_scores_df <- 
  as.data.frame(t(GetAssayData(Pyelonephritis_InfectionDay3, slot = "scale.data", 
                               assay = "progeny"))) %>%
...
progeny_hmap
dev.off()
test<- FeaturePlot(Pyelonephritis_InfectionDay3, features = "NFkB")
...
for (i in ProgenyFeatures ){
  p2 <-(FeaturePlot(Pyelonephritis_InfectionDay3, features = i) & 
  scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red'))& 
  ggtitle(paste0(i," activity"))
  
  ggsave(paste0(i, "_CorMedInfected_cluster_featurePlot.pdf"), plot = p2, width = 6, height = 6)
  
  p3 <- (SpatialFeaturePlot(Pyelonephritis_InfectionDay3, features = i, pt.size.factor = 4) +ggtitle(paste0(i," activity")))
...
Pyelonephritis_InfectionDay3@assays$SCT@data %>% head()
mat <- as.matrix(Pyelonephritis_InfectionDay3@assays$SCT@data)
# Run ulm
...
Pyelonephritis_InfectionDay3[['tfsulm']] <- acts %>%
...
# Change assay
DefaultAssay(object = Pyelonephritis_InfectionDay3) <- "tfsulm"

# Scale the data
Pyelonephritis_InfectionDay3 <- ScaleData(Pyelonephritis_InfectionDay3)
Pyelonephritis_InfectionDay3@assays$tfsulm@data <- Pyelonephritis_InfectionDay3@assays$tfsulm@scale.data

# Explore the top 20 more varaible TFs
n_tfs <- 20

t(as.matrix(Pyelonephritis_InfectionDay3@assays$tfsulm@data))
# Extract activities from object as a long dataframe
df <- t(as.matrix(Pyelonephritis_InfectionDay3@assays$tfsulm@data)) %>%
  as.data.frame() %>%
  mutate(cluster = Idents(Pyelonephritis_InfectionDay3)) %>%
...
for (m in tfs){
  p2 <- (SpatialFeaturePlot(Pyelonephritis_InfectionDay3, features = m,pt.size.factor = 4) +
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
...
Pyelonephritis_InfectionDay3
DefaultAssay(object = Pyelonephritis_InfectionDay3) <- "SCT"

for (n in tfs){
  p3 <- (FeaturePlot(Pyelonephritis_InfectionDay3, features = n,slot= "scale.data") + 
  #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression')))
  
  p4 <- (SpatialFeaturePlot(Pyelonephritis_InfectionDay3, features = n, pt.size.factor = 4, slot= "scale.data") +
 # scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle(paste0(n, ' expression (time)')))
 
  ggsave(paste0("TFExpression_",n,"_Pyelonephritis_InfectionDay3_FeaturePlot.pdf"), plot = p3, width = 6, height = 6)
  ggsave(paste0("TFExpression_",n,"_Pyelonephritis_InfectionDay3_SpatilFeaturePlot.pdf"), plot = p4, width = 6, height = 6)
...
GeneOrderByPheatmap
?DotPlot
TFDotPlot <- DotPlot(Pyelonephritis_InfectionDay3, features = rev(GeneOrderByPheatmap)) +
...
```{r}
DefaultAssay(Pyelonephritis_InfectionDay3) <- "SCT"
Pyelonephritis_InfectionDay3@meta.data %>% head()
#Idents(Pyelonephritis_InfectionDay3) <- Pyelonephritis_InfectionDay3@meta.data$Sub2

# reset the identity, Combine the PT cells

Idents(Pyelonephritis_InfectionDay3)
Pyelonephritis_InfectionDay3@meta.data
# adjust the levels
# Idents(Pyelonephritis_InfectionDay3) <-
# ordered(factor(Idents(Pyelonephritis_InfectionDay3)), levels=c( "5_1","10_1","10_0","2_1","2_0","5_0","5_2","4","7","0","1","9","6","3","8"))

levels(Idents(Pyelonephritis_InfectionDay3))
```
...
Pyelonephritis_InfectionDay3 = alias_to_symbol_seurat(Pyelonephritis_InfectionDay3, "mouse")

Pyelonephritis_InfectionDay3@meta.data %>% head()
Pyelonephritis_InfectionDay3@meta.data$IntegrateAnnot %>% table() 

head(weighted_networks$gr) 
DimPlot(Pyelonephritis_InfectionDay3)

# If you use convert_to_alias before here, this one won't work
Pyelonephritis_InfectionDay3 = Seurat::PrepSCTFindMarkers(Pyelonephritis_InfectionDay3, assay = "SCT", verbose = FALSE)
...
lr_network %>% head()
Pyelonephritis_InfectionDay3 %>% subset(features = lr_network$ligand %>% intersect(rownames(Pyelonephritis_InfectionDay3)))
# calcualte the niches difference 
DE_sender = nichenetr::calculate_niche_de(seurat_obj  = Pyelonephritis_InfectionDay3 %>% subset(features = lr_network$ligand %>% intersect(rownames(Pyelonephritis_InfectionDay3))), niches = niches, type = "sender", assay_oi = "SCT") # only ligands important for sender cell types
DE_receiver = nichenetr::calculate_niche_de(seurat_obj  = Pyelonephritis_InfectionDay3 %>% subset(features = lr_network$receptor %>% unique()), niches = niches, type = "receiver", assay_oi = "SCT") # only receptors now, later on: DE analysis to find targets
...
DE_receiver_targets = nichenetr::calculate_niche_de_targets(seurat_obj = Pyelonephritis_InfectionDay3, niches = niches, lfc_cutoff = lfc_cutoff, expression_pct = expression_pct, assay_oi = "SCT") 
...
nichenet_output = nichenet_seuratobj_aggregate(
  seurat_obj = Pyelonephritis_InfectionDay3, 
  receiver = "PT", 
...
avg_expression_ligands = AverageExpression(Pyelonephritis_InfectionDay3, features = nichenet_output$top_ligands)
...
Pyelonephritis_InfectionDay3
  # there seems to be an error in the susetting, so we removed other igmages
  #names(Pyelonephritis_InfectionSubslides@images)
SelectedImageName <- names(Pyelonephritis_InfectionDay3@images)[5]
SelectedImageName
OtherImages <- Allimagmes[Allimagmes != SelectedImageName]
OtherImages
Pyelonephritis_InfectionDay3@images[OtherImages] = NULL

#nrow(Pyelonephritis_InfectionDay3@meta.data )
Pyelonephritis_InfectionDay3Dimplot <-SpatialDimPlot(Pyelonephritis_InfectionDay3, crop = T)

ggsave("Pyelonephritis_InfectionDay3Dimplot.pdf", plot =Pyelonephritis_InfectionDay3Dimplot, height = 6, width = 6 )

# the prediction of the object
Pyelonephritis_InfectionDay3[["predictions"]]
InfectedMat_D7<- t(Pyelonephritis_InfectionDay3[["predictions"]]@data)
...
SpatialD7MarkersDotPlot<- DotPlot(Pyelonephritis_InfectionDay3, features = Finalcelltypemarkers,cols = "Spectral", dot.scale = 4) +RotatedAxis() +theme(legend.text = element_text(size = 12), legend.title = element_text(size = 8))

ggsave("SpatialD7MarkersDotPlot.pdf", plot = SpatialD7MarkersDotPlot, height = 6, width = 14)
Pyelonephritis_InfectionDay3

for (i in Finalcelltypemarkers){
   CellTypeMarkerSpatialFeature<- SpatialFeaturePlot(Pyelonephritis_InfectionDay3, features = i, pt.size.factor = 1.6, crop = TRUE) 
   #&
       #   ggplot2::scale_fill_gradientn(colours = myPalette(4), limits=c(0,1))
...
#nrow(Pyelonephritis_InfectionDay3@meta.data )
SpatialDimPlot(Pyelonephritis_InfectionDay3)
...
coords <- GetTissueCoordinates(Pyelonephritis_InfectionDay3)
coords
Pyelonephritis_InfectionDay3@meta.data %>% head()
summary(coords)
Pyelonephritis_InfectionDay3@images$slice8

# we select the regions for the cluster
Pyelonephritis_InfectionDay3 <- subset (Pyelonephritis_InfectionDay3, slice8_imagerow > 140 | slice8_imagecol > 200, invert = TRUE)
Pyelonephritis_InfectionDay3 <- subset (Pyelonephritis_InfectionDay3, idents = c("N13","N14","N15","N16","N17"))
InfectedCorMedRegionD1Spatial <-SpatialDimPlot(Pyelonephritis_InfectionDay3,crop = TRUE, label = FALSE)
...
Pyelonephritis_InfectionDay3@meta.data[,c("orig.ident","Annotation")]
## merge is way too troublesome, therefore
FinalIntergratedTableAbcess<- merge(Pyelonephritis_InfectionDay3@meta.data[,c("orig.ident","Annotation")], t(Pyelonephritis_InfectionDay3[["predictions"]]@data), all=TRUE, by="row.names") 
?Heatmap
...
pal <- colorRampPalette(paletteMartin)(length(ct))
names(pal) <- ct
Pyelonephritis_InfectionDay3CellTypeMatrix <-t(Pyelonephritis_InfectionDay3[["predictions"]]@data)

Pyelonephritis_InfectionDay3CellTypeMatrixNoMax <- Pyelonephritis_InfectionDay3CellTypeMatrix[,!colnames(Pyelonephritis_InfectionDay3CellTypeMatrix) %in% c("max")]
Pyelonephritis_InfectionDay3CellTypeMatrixNoMax
# generate the scatter pie plot
colnames(Pyelonephritis_InfectionDay3CellTypeMatrixNoMax)
...
InfectedCorMedRegionD1SpatialPie<- plotSpatialScatterpie(
  x= Pyelonephritis_InfectionDay3,
  y= Pyelonephritis_InfectionDay3CellTypeMatrixNoMax,
  cell_types = colnames(Pyelonephritis_InfectionDay3CellTypeMatrixNoMax),
    img = TRUE,
    scatterpie_alpha = 1,
...
# correlation
InfectedCorMedRegionD1SpatialPieCorrelationMatrix <- plotCorrelationMatrix(Pyelonephritis_InfectionDay3CellTypeMatrixNoMax)
...
Pyelonephritis_InfectionDay3CellTypeMatrixNoMaxLarger<- Pyelonephritis_InfectionDay3CellTypeMatrixNoMax[,colSums(Pyelonephritis_InfectionDay3CellTypeMatrixNoMax[])>0]
InfectedPevRegionD1Interactions<- plotInteractions(Pyelonephritis_InfectionDay3CellTypeMatrixNoMaxLarger, "heatmap") + scale_fill_gradientn(colors = c("blue", "white", "red"))
?plotInteractions
...
Pyelonephritis_InfectionDay3 <- subset (Pyelonephritis_InfectionDay3, (slice8_imagerow >310 & slice8_imagerow <450 )  & (slice8_imagecol < 330 & slice8_imagecol> 220) )
Pyelonephritis_InfectionDay3 <- subset (Pyelonephritis_InfectionDay3, idents = c("N8","N10","N11","N12","N15"))
InfectedCorMedRegionD1Spatial <-SpatialDimPlot(Pyelonephritis_InfectionDay3,label = TRUE)
InfectedCorMedRegionD1Spatial
ggsave("Spatial_infected_day7_SpatialDimplot.pdf",
      plot = InfectedCorMedRegionD1Spatial,
      height = 3,
...
InfectedCorMedRegionD1Spatial
Pyelonephritis_InfectionDay3
DefaultAssay(Pyelonephritis_InfectionDay3) <- "integrated"
nbs_2.markers <- FindAllMarkers(Pyelonephritis_InfectionDay3,test.use ="MAST")
nbs_2.markers
??SubsetSTData
#nbs_2.markers$gene <- rownames(nbs_2.markers)

nbs_2.markers
#se.subset <- SubsetSTData(Pyelonephritis_InfectionDay3, expression = nbs_2 %in% c("N12", "N15"))
...
DotPlot(Pyelonephritis_InfectionDay3, features = sorted.marks$gene %>% unique()) +RotatedAxis()
#DoHeatmap(Pyelonephritis_InfectionDay3, features = sorted.marks$gene, group.colors = c("red", "lightgray"))
