---
title: "Pipseq Urothelium Subcluster"
author: "Xin Wang"
date: "2024-07-01"
description: Urothelium subclusters analyses:
  - Pathway activity inference by PROGENy model from URO subclusters
  - Transcription factor activity inference from URO subclusters
  - DEGs and Pathways
...
```{r}
#rm(list =ls())
setwd("/Users/XXW004/Documents/Projects/RuizRosado/PIPseq/Results/Evaluation_V022723/CellEvaluation_Subclustering_V0803/URO_Subclusters/")
Indir <-c("/Users/XXW004/Documents/Projects/RuizRosado/PIPseq/Results/Evaluation_V022723/CellEvaluation_Subclustering_V0803/")
Outdir <-c( "/Users/XXW004/Documents/Projects/RuizRosado/PIPseq/Results/Evaluation_V022723/CellEvaluation_Subclustering_V0803/URO_Subclusters/")
```
...
# generate the health cluster of URO cells
...
DimPlot(scrna)
# We only collect the URO cells
# CellType[CellType$cluster %in% c("1_0","1_1","0", "9","12","18", "19","3"),2] <- "URO" # "Proximal tubule"


# check the Urothelium markers
UrotheliumMarkers <- c("Krt5","Krt15","Krt14","Upk3bl","Krt6a","Cd44","Krt10","Upk2","Upk3a","Upk1a","Krt18","Krt7","Krt19","Krt20","Clu","Abhd2","Top2a","Mki67","Birc5")

DotPlot(scrna, features = UrotheliumMarkers) +RotatedAxis()
# check the URO markers: "Pdgfra","Pdgfrb","Dcn","Col1a1","Col1a2","Col6a3","Col5a1","Loxl1","Lum","Fbln1","Fbln2","Cd34","Tagln","Fbln5","Postn","C7","Cdh11","Cp","Negr1","Lama2","Pax7","Fgfr4"
...
# Select the Urothelium Cells 

UROcells0 <- subset(scrna, subset = (Raw_cell_type == "URO" ))
# For those cells, we only consider the Krt5, Upk2, Upk3a, 

UROcells <-subset(UROcells0, subset = Upk3a > 1 | Upk1a >1 |Upk1b >1 | Upk3b >1 |Krt5 >1 |Krt14 >1 )
...
UROcellsDim
ggsave(filename = "UROcellsDim.pdf", plot = UROcellsDim, width = 3.5, height = 3)
UROcellsDimSplit<- DimPlot(UROcells,label = T, split.by = "DataSet")
ggsave(filename = "UROcellsDimSplit.pdf", plot = UROcellsDimSplit, width = 8, height = 3)
UROcellsDimSplit
UROcells
...
#UROcellssub<- subset(UROcells, idents = c(2,3,4,5))
```

# For the markers
```{r}
# For Urothelium Markers:
# set up the directory
# create a 
#setwd(BioMarkerDir)

# check the biomarkers for Urothelium
Idents(UROcells) <- UROcells@meta.data$SCT_snn_res.0.2
# check the Urothelium markers
UrotheliumMarkers <- c("Krt5","Krt15","Krt14","Upk3bl","Krt6a","Cd44","Krt10","Upk2","Upk3a","Upk1a","Upk1b","Krt18","Krt7","Krt19","Krt20","Clu")
UROcellsDotPlot<- DotPlot(UROcells, features = UrotheliumMarkers, assay = "RNA") +RotatedAxis()
UROcellsDotPlot
# save the plot
ggsave(filename = "UROcellsDotPlot.pdf", plot = UROcellsDotPlot, height = 4, width = 6)


# for vlnPlot, we only plot the interesting genes
UrotheliumInterestMarkers <- c("Krt5","Upk3a","Upk2","Krt15","Krt14","Krt6a","Cd44","Upk1b","Krt18","Krt7","Krt19","Krt20")


UROcellsVlnPlot<- VlnPlot(UROcells, features = UrotheliumInterestMarkers) +RotatedAxis()
ggsave(filename = "UROcellsClusterVlnPlot.pdf", plot = UROcellsVlnPlot, height = 6, width = 8)

DoHeatmap(UROcells, features = UrotheliumMarkers)
Idents(UROcells)

# set the identity into DataSet
Idents(UROcells) <- UROcells@meta.data$DataSet
UROcellsDateDotPlot<-DotPlot(UROcells, features = UrotheliumMarkers, assay = "RNA") +RotatedAxis()
ggsave(filename = "UROcellsDateDotPlot.pdf", plot = UROcellsDateDotPlot, height = 4, width = 6.5)
UROcellsDateVlnPlot<- VlnPlot(UROcells, features = UrotheliumInterestMarkers) +RotatedAxis()
ggsave(filename = "UROcellsDateVlnPlot.pdf", plot = UROcellsDateVlnPlot, height = 6, width = 8)

# other cell type markers
# Cfh: Parietal epithelial cells
Finalcelltypemarkers<- c("Lrp2","Slc34a1","Slc13a3", # Proximal tubule cells (URO)
                         "Vcam1","Cfh","Havcr1",
                    ...
                    "Col3a1","Fbn1","Lum","Col1a1", # UROroblast
...
DotPlot(UROcells, features = Finalcelltypemarkers) +RotatedAxis()


#saveRDS(UROcells, file = "PIPSeq_Annotated_OnlyURO_v0823.rds")
#UROcells <- readRDS("PIPSeq_UROAnn_v061824.rds")
# UROroblast
...
DotPlot(UROcells, features = c("Col1a1","Col1a2","Col6a3","Col5a1","Pdgfra","Dcn","Loxl1","Lum","Fbln1","Fbln2","Col14a1","Agtr1a","Postn","Tagln","Myh11","Acta2","Myocd","Pecam1","Npr1","Emcn")) +RotatedAxis()
# UROroblast
DotPlot(UROcells, features = c("Pdgfra","Pdgfrb","Dcn","Col1a1","Col1a2","Col6a3","Col5a1","Loxl1","Lum","Fbln1","Fbln2","Cd34","Tagln","Fbln5","Postn","C7","Cdh11","Cp","Negr1","Lama2","Fgfr4"))+RotatedAxis()
DefaultAssay(UROcells) <-"RNA"
# myoblast
DotPlot(UROcells, features = c("Col14a1","Agtr1a","Pdgfra","Tgfb1","Trp53","Casp3"))+RotatedAxis()

# smc
DotPlot(UROcells, features = c("Piezo2","Postn","Tagln","Tgfb1","Myh11","Acta2"))+RotatedAxis()

# Endothelium
DotPlot(UROcells, features = c("Pecam1","Npr1","Emcn","Trpv4","Klf4","Kdr"))+RotatedAxis()

#DefaultAssay(UROcells) <-"RNA"
# Podotype
DotPlot(UROcells, features = c("Wt1","Foxc2","Synpo","Nphs1","Nphs2","Npr1","Lrp2","Slc34a1","Slc5a2","Il1b","Mmp7"))+RotatedAxis()
# Epithelial-mesenchymal transition (EMT) markers
DotPlot(UROcells, features = c("Cdh1","Cdh2","Vim","Snai1","Zbp1","Zeb2"))+RotatedAxis()

# Mesenchymal markers
DotPlot(UROcells, features = c("Acta2","Cdh2","Fbn1","Itgb1","Mmp2","Mmp3","Mmp9","S100a11","Snai1","Vim"))+RotatedAxis()

DimPlot(UROcells, split.by = "DataSet")
...
```{r}
UROcells@meta.data %>% head()

# add the annotation using case_when

UROcells@meta.data<- UROcells@meta.data %>%   mutate(UROAnnotation = 
           case_when(UROcells$SCT_snn_res.0.2 %in% c("0") ~ "URO", # Original URO
                     UROcells$SCT_snn_res.0.2 %in% c("1") ~ "aURO", # Adaptive URO
                     )) 

Idents(UROcells) <- UROcells@meta.data$UROAnnotation
table(UROcells@meta.data$UROAnnotation)
 
saveRDS(UROcells, file = "PIPSeq_UROAnn_v061824.rds")
UROcells<-readRDS("PIPSeq_UROAnn_v061824.rds")

# Final used Biomarkers

FinalMarkers <- c("Krt5","Upk3a","Upk2","Krt15","Krt14","Krt6a","Cd44","Upk1b","Krt18","Krt7","Krt19","Krt20")
# reorder the levels: 
Idents(UROcells)<-ordered(factor(UROcells@meta.data$UROENDIntegrateAnnot2),levels=c("URO","aURO"))

FinalMarkersDotPlot<-DotPlot(UROcells, features = FinalMarkers)+RotatedAxis()

ggsave("Urothelium_finalmarkers_DotPlot.pdf", plot = FinalMarkersDotPlot, height = 4, width = 8)
UROcellsDim<- DimPlot(UROcells,label = T)
UROcellsDim
ggsave(filename = "UROcellsDim_Ann.pdf", plot = UROcellsDim, width = 5, height = 4)

# we think the clueter 0 and cluster 1 are the doublet cells that experience with dying epithelial cells, so we removed the cluster 0 and cluster 1
#UROcellssub<- subset(UROcells, idents = c(2,3,4,5))


UROcellsDimSplit<- DimPlot(UROcells,label = F, split.by = "DataSet")
ggsave(filename = "UROcellsDimSplit_An.pdf", plot = UROcellsDimSplit, width = 8, height = 3)
...
UROmataAnnotation <- UROcells@meta.data %>% dplyr::select(SCT_snn_res.0.2, UROAnnotation)
UROmataAnnotation
table(UROmataAnnotation$UROAnnotation)
...
scrna@meta.data <- scrna@meta.data  %>% 
  left_join( UROmataAnnotation %>% rownames_to_column('rn'),by="rn") %>% 
# then add a new column that change the NA into Raw_cell_type
  # mutate(NewAnnoation = coalesce(PTIntegrateAnnot,Raw_cell_type))
  mutate(PTICPCENDUROFinalAnn = ifelse(is.na(UROENDIntegrateAnnot), PTICPCFinalAnn,UROENDIntegrateAnnot))
### join the meta data to meta annoation
levels(as.factor(scrna@meta.data$PTICPCENDUROFinalAnn))

# set up the row name and identity
row.names(scrna@meta.data) <- scrna@meta.data$rn
Idents(scrna) <- scrna@meta.data$PTICPCENDUROFinalAnn
...
#UUOProjectObject_F@meta.data %>% mutate(Symbol=rownames(UUOProjectObject_F@meta.data))%>% left_join(MetaAnnotation %>% mutate(Symbol=rownames(MetaAnnotation)), by = 'Symbol')
 ...
 saveRDS(scrna, file = "PIPSeq_PTICPCENDURO_Annotated_v061824.rds")
...
UROcells@meta.data$UROENDIntegrateAnnot

UROPropotionWithinTime <-table(UROcells@meta.data$DataSet,UROcells@meta.data$UROENDIntegrateAnnot) %>% as.data.frame() %>%
  group_by(Var1) %>%
...
ggsave("UROPropotionWithinTime.pdf", plot = UROPropotionWithinTime, width = 4, height = 3)
UROPropotionWithinTime
```
...
OtherMarkerGenesDotPlots<- DotPlot(UROcells, features =  Finalcelltypemarkers, col.min = -0.5) +RotatedAxis()
...
DotPlot(UROcells, features =  HealthMarkers, col.min = -0.5) +RotatedAxis()

CyclingMarker <- "Mki67"
DotPlot(UROcells, features =  CyclingMarker, col.min = -0.5) +RotatedAxis()

EarlyInjuryMarkers <- c("Cdh13", "Cdh6", "Havcr1")
DotPlot(UROcells, features =  EarlyInjuryMarkers, col.min = -0.5) +RotatedAxis()
LateInjuryMarkers <-c( "Ccl2", "Gm17268", "Cald1")
DotPlot(UROcells, features =  LateInjuryMarkers, col.min = -0.5) +RotatedAxis()

ReparingMarkers <- "Top2a"

DotPlot(UROcells, features =  ReparingMarkers, col.min = -0.5) +RotatedAxis()

FailedFepairMarkers <- c("Ypel2","Kcnip4")

DotPlot(UROcells, features =  FailedFepairMarkers, col.min = -0.5) +RotatedAxis()
# 
MaladUROiveMarkers <- c("Vcam1", "Havcr1",  "Icam1", "Prom1","Cxcl2","Ccl3","Myc")

DotPlot(UROcells, features =  MaladUROiveMarkers, col.min = -0.5) +RotatedAxis()
...
KidneyStatuesMarkers<- c( "Nrg1","Slc5a2","Slc34a1","Inmt","Apom", "Slc7a13","Slc22a6", "Prom1","Top2a","Cyp7b1","Mki67","Cdh13", "Cdh6", "Havcr1","Ccl2", "Gm17268", "Cald1","Vcam1", "Ypel2","Kcnip4","Hsp90aa1","Col27a1","Plin2")
 ...
 DotPlot(UROcells, features =  KidneyStatuesMarkers,col.min = -1) +RotatedAxis()
...
KidneyStatuesTypeMarkers<- c("Il1b", "Mlkl", "Gsdmd","Ripk3","Fn1", "Col1a1", "Col3a1", "Tgfb1","Vcam1", "Havcr1", "Icam1", "Prom1", "Spp1", "Cst3","Gdf15", "Clu", "Igfbp7", "Apoe", "Aldob", "B2m", "S100a6","S100a8","S100a9","Pdgfb","Bak1","Bad","Casp8","Ptgs2","Casp1","Hmox1")
 DotPlot(UROcells, features =  KidneyStatuesTypeMarkers) +RotatedAxis()
 ...
 KidneyStatuesTypeMarkers2<- c("Il1b", "Mlkl", "Gsdmd","Ripk3","Fn1", "Col1a1", "Col3a1", "Tgfb1","Vcam1", "Havcr1", "Icam1", "Prom1", "S100a6","S100a8","S100a9","Pdgfb","Bak1","Bad","Casp8","Ptgs2","Casp1","Hmox1","Nfkbia")
  DotPlot(UROcells, features =  KidneyStatuesTypeMarkers2) +RotatedAxis()

 #, cluster 5: Fn1, Havcr1, Cst3, Igfbp7, S100a8, Il1b, Slc7a13 : ProUROrotic URO
 # cluster 1: Spp1, Prom1, Kcnip4, Col27a1,Top2a,Nfkbia: Cycling URO
 # cluster 8: Cdh13, Hsp90aa1,Inmt, Mki67 : Proliferating URO
 
 # cluster 1: ASpp1, Prom1, Kcnipp4: Progenitor URO
...
CandidateMarkersDotPlot<-  DotPlot(UROcells, features =  CandidateMarkers, col.min = -1) +RotatedAxis()
CandidateMarkersDotPlot
ggsave("URO_CandidateMarkersDotPlot.pdf", plot = CandidateMarkersDotPlot, height = 6, width = 12)

CandidateMarkersDotPlot2<-  DotPlot(UROcells, features =  CandidateMarkers2, col.min = -1) +RotatedAxis()
CandidateMarkersDotPlot2
ggsave("URO_CandidateMarkersDotPlot_2.pdf", plot = CandidateMarkersDotPlot2, height = 6, width = 12)


for (i in CandidateMarkers ){
  p2 <-FeaturePlot(UROcells, features = i) & 
 #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')& 
  ggtitle(paste0(i,""))
  
  ggsave(paste0(i, "_URO_Marker_featurePlot.pdf"), plot = p2, width = 4, height = 3)
    p3 <-FeaturePlot(UROcells, features = i, split.by = "DataSet") & 
 #scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')& 
  ggtitle(paste0(i,""))
  
  ggsave(paste0(i, "_URO_Marker_featurePlot_split.pdf"), plot = p3, width = 12, height = 3)
}
```

# generate the Injury, adaptive, cytokine, signaling, growth factor, prolfieratio, panoptosis, proUROrotic markers
```{r}
...
StatusTypeMakers[["ProUROrotic"]] <- c("Mmp2","Mmp9","Tgfb1","Col4a1","Col1a1","Fn1","Col3a1")
...
StatusTypeMakers[["UROrosis"]] <- c("Fn1", "Col1a1", "Col3a1", "Tgfb1")
...
StatusTypeMakers[["UROroblast"]] <-c("Pdgfra","Pdgfrb","Dcn","Col1a1","Col1a2","Col6a3","Col5a1","Loxl1","Lum","Fbln1","Fbln2","Cd34","Tagln","Fbln5","Postn","C7","Cdh11","Cp","Negr1","Lama2","Pax7","Fgfr4")
...
for (i in 1:length(StatusTypeMakers)) {
  ...
  DPlots<- DotPlot(UROcells, features =StatusTypeMakers[[StatusTypeMakersType]] )+RotatedAxis()
  ...
  for (j in StatusTypeMakers[[StatusTypeMakersType]]) {
    ...
    FeaturePlots <-
      FeaturePlot(UROcells, features =  j, split.by = "DataSet",order =TRUE) + RotatedAxis()
    ggsave(
      paste0(
...
UROS1_S3Markers<- c("Lrp2","Slc5a12","Slc5a2","Slc4a4","Slc6a19","Slc7a7","Slc16a10","Slc16a14","Slc22a23","Slc22a29","Atp6v0a4","Atp6v1b2","Atp2a2","Adra1a","Slc7a13","Slc13a3","Slc34a1","Slc22a30","Slc5a10","Slc5a8","Slc22a6","Slc1a1","Cyp7b1","Slc7a12","Atp11a","Aqp7","Slc16a9","Slc5a4a","Slc9a8","Slc22a7","Slc39a8")
UROS1_S3DotPlot <- DotPlot(UROcells, features = UROS1_S3Markers) +RotatedAxis()
ggsave(paste0(Outdir,"UROS1_S3_DotPlot.pdf"), plot =UROS1_S3DotPlot ,height = 6, width = 12)

UROS1_S3_DotPlot_representMarkers<-DotPlot(UROcells, features = c("Lrp2","Hnf4a","Slc5a12","Slc5a2","Slc13a1","Slc1a1","Slc13a3","Cyp7b1","Slc22a7","Slc7a12","Slc16a9","Spag5","Mki67","Havcr1","Cdh13","Cdh6","Cald1","Pdgfd","Kcnip4","Vcam1","Ccl2","Gm17268")) +RotatedAxis() +coord_flip() +scale_x_discrete(limits=rev)
ggsave(paste0(Outdir,"UROS1_S3_DotPlot_representMarkers.pdf"), plot =UROS1_S3_DotPlot_representMarkers ,height = 6, width = 6)
...
ProUROrotic <- c("Mmp2","Mmp9","Tgfb1","Col4a1","Col1a1","Fn1","Col3a1")
...
# Final used biomarkers for URO cells: 
```{r}
# final biomarkers
...
levels(UROcells)<-rev(levels(UROcells))
...
UROCellTypeDotPlots <- DotPlot(UROcells, features = FinalBiomarkers) +
...
UROCellTypeDotPlots
ggsave(paste0(FinalBioMarkerDir,  "UROCellType_S1_S2_S3DotPlots.pdf"), plot =UROCellTypeDotPlots, height =3.5, width = 7.5)
...
UROAKIDotPlots <- DotPlot(UROcells, features =FinalAKIBiomarkers)+
guides(color = guide_colorbar(title = 'AverExp'),
         size = guide_legend(title = "PercExp")) + RotatedAxis() 
...
UROAKIDotPlots
ggsave(paste0(FinalBioMarkerDir,  "UROAKI_Biomarker_DotPlots.pdf"), plot =UROAKIDotPlots, height =3.5, width = 7)
...
UROcellcyle <-  CellCycleScoring(UROcells, s.features = mmus_s, g2m.features = mmus_g2m, set.ident = TRUE)

# "#999999", "#E69F00", "#56B4E9"
UROcellcyleDimplotTimepoint<- DimPlot(UROcellcyle, split.by = "DataSet",cols = c("#1a80bb","#b3b3b2","#ea801c"))
ggsave("UROcellcyleDimplotTimepoint.pdf", plot = UROcellcyleDimplotTimepoint, height = 3, width = 10)

UROcellcyleDimplotTimepoint
UROcellcyleDimplotAll<- DimPlot(UROcellcyle, cols = c("#1a80bb","#b3b3b2","#ea801c"))
ggsave("UROcellcyleDimplotAll.pdf", plot = UROcellcyleDimplotAll, height = 3, width = 4)
UROcellcyleDimplotAll
# measure the cycling score for each cluster
# Firstly, create a new column to combine Dataset and cluster
UROcellcyle@meta.data %>% head()

UROCellCylcePropotionWithinClusters<-table(UROcellcyle@meta.data$Phase,UROcellcyle@meta.data$SCT_snn_res.0.1)%>% as.data.frame() %>%
  group_by(Var2) %>%
...
UROCellCylcePropotionWithinClusters
ggsave("UROCellCylcePropotionWithinClusters.pdf", plot = UROCellCylcePropotionWithinClusters, width = 4, height = 3)
table(UROcellcyle@meta.data$Phase,UROcellcyle@meta.data$SCT_snn_res.0.5,UROcells@meta.data$DataSet)

UROCellCylcePropotionWithinTimepoint <-table(UROcellcyle@meta.data$Phase,UROcells@meta.data$DataSet) %>% as.data.frame() %>%
  group_by(Var2) %>%
  mutate(pct= prop.table(Freq) )  %>% ggplot(aes(x=Var2, y=pct, fill=Var1)) +geom_bar(position="stack", stat="identity") + theme_classic() +scale_y_continuous(labels = scales::percent) +scale_fill_manual(values=c("#b3b3b2","#1a80bb","#ea801c"))
ggsave("UROCellCylceWithinTime.pdf", plot = UROCellCylcePropotionWithinTimepoint, width = 4, height = 3)
...
Cyclin.genes<- rownames(UROcells)[grep("^Ccn",rownames(UROcells))]

UROCellCylceGeneDotPlot<-DotPlot(UROcells, features =Cyclin.genes ) +RotatedAxis()
ggsave("UROCellCylceGeneDotPlot.pdf", plot = UROCellCylceGeneDotPlot, width = 9, height = 3)
Cyclin.genes
for (i in Cyclin.genes){
  #i=c("Ccnh")
  CyclinFeaturePlot<- FeaturePlot(UROcells, features = i, split.by = "DataSet", order = T)
...
UROCluster<- data.frame(Cell = names(Idents(UROcells)),   
                              CellType = as.character(Idents(UROcells)),
                              stringsAsFactors = FALSE)
UROCluster

UROcells <- progeny(UROcells, scale=FALSE, organism="Mouse", top=500, perm=1, return_assay = TRUE)
UROcells
# setup the default assay as progeny
DefaultAssay(object = UROcells) <- "progeny"

UROcells <- Seurat::ScaleData(UROcells, assay = "progeny") 
## We can now directly apply Seurat functions in our Progeny scores. 
## For instance, we scale the pathway activity scores. 
progeny_scores_df <- 
  as.data.frame(t(GetAssayData(UROcells, slot = "scale.data", 
                               assay = "progeny"))) %>%
...
progeny_scores_df <- inner_join(progeny_scores_df, UROCluster)
...
# generate the heatmap for URO
pdf(file="URO_Progeny_ForHeatmap.pdf",height = 4, width = 4)
...
dev.off()
progeny_hmap
test<- FeaturePlot(UROcells, features = "NFkB")
...
for (i in ProgenyFeatures ){
  p1 <-(FeaturePlot(UROcells, features = i) &
  scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) &
  ggtitle(paste0(i," activity"))
   ggsave(paste0(i, "_URO_VariousCluster_featurePlot.pdf"), plot = p1, width = 5, height = 5)
  p2 <-(FeaturePlot(UROcells, features = i,split.by  = "DataSet") &
  scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) &
  ggtitle(paste0(i," activity"))
  
  ggsave(paste0(i, "_URO_cluster_featurePlot.pdf"), plot = p2, width = 10, height = 5)
}
...
dev.off()
i="EGFR"
FeaturePlot(UROcells, features = "EGFR")

p2 <- FeaturePlot(UROcells, features = i,  pt.size = 1) &
  scale_color_gradientn(colors = myColor, limits = c(-75, 50))  &
  ggtitle(paste0(i," activity"))
ggsave(paste0(i, "_URO_cluster_featurePlot_ClusterCombined.pdf"), plot = p2, width = 4, height = 3)

i="Hypoxia"
FeaturePlot(UROcells, features = "Hypoxia")

p2 <- FeaturePlot(UROcells, features = i,  pt.size = 1) &
  scale_color_gradientn(colors = myColor, limits = c(-50, 250))  &
  ggtitle(paste0(i," activity"))
ggsave(paste0(i, "_URO_cluster_featurePlot_ClusterCombined.pdf"), plot = p2, width = 4, height = 3)

i="NFkB"
FeaturePlot(UROcells, features = "NFkB")

p2 <- FeaturePlot(UROcells, features = i,  pt.size = 1) &
  scale_color_gradientn(colors = myColor, limits = c(-50, 600))  &
  ggtitle(paste0(i," activity"))
ggsave(paste0(i, "_URO_cluster_featurePlot_ClusterCombined.pdf"), plot = p2, width = 4, height = 3)

i="JAK-STAT"
FeaturePlot(UROcells, features = "JAK-STAT")

p2 <- FeaturePlot(UROcells, features = i,  pt.size = 1) &
  scale_color_gradientn(colors = myColor, limits = c(0, 400))  &
  ggtitle(paste0(i," activity"))
ggsave(paste0(i, "_URO_cluster_featurePlot_ClusterCombined.pdf"), plot = p2, width = 12, height = 4)

i="TGFb"
FeaturePlot(UROcells, features =i)
...
p2 <- FeaturePlot(UROcells, features = i,  pt.size = 1) &
  scale_color_gradientn(colors = myColor, limits = c(0, 700))  &
  ggtitle(paste0(i," activity"))
ggsave(paste0(i, "_URO_cluster_featurePlot_ClusterCombined.pdf"), plot = p2, width = 4, height = 3)
...
p2 <- FeaturePlot(UROcells, features = i,  pt.size = 1) &
  scale_color_gradientn(colors = myColor, limits = c(-75, 60))  &
  ggtitle(paste0(i," activity"))
ggsave(paste0(i, "_URO_cluster_featurePlot_ClusterCombined.pdf"), plot = p2, width = 4, height = 3)
...
# 
# for (i in ProgenyFeatures ){
#   p2 <-FeaturePlotSingle(UROcells, feature= i, metadata_column = "DataSet", pt.size = 0.05, order =TRUE) &
#   ggtitle(paste0(i," activity"))
#   
#   ggsave(paste0(i, "_URO_cluster_featurePlot_Custom.pdf"), plot = p2, width = 10, height = 5)
# }
# 
# ProgenyFeatures
# p_list<- FeaturePlotSingle(UROcells, feature= "EGFR", metadata_column = "DataSet", pt.size = 0.05, order =TRUE)
```
...
for (i in ProgenyFeatures ){
  ...
  ggsave(paste0(i, "_URO_cluster_featurePlot_Date.pdf"), plot = p2, width = 10, height = 5)
}
...
UROcells@meta.data<- UROcells@meta.data %>% mutate(IntegratedClusterDate = paste(UROcells@meta.data$DataSet,UROcells@meta.data$SCT_snn_res.0.1, sep = "_"))
levels(as.factor(UROcells@meta.data$IntegratedClusterDate))  
...
Idents(UROcells)<-ordered(factor(UROcells@meta.data$IntegratedClusterDate),levels=c("0DPI_0","0DPI_1","0DPI_2","0DPI_3","0DPI_4","0DPI_5","0DPI_6","3DPI_0","3DPI_1","3DPI_2","3DPI_3","3DPI_4","3DPI_5","3DPI_6","7DPI_0","7DPI_1","7DPI_2","7DPI_3","7DPI_4","7DPI_5","7DPI_6","28DPI_0","28DPI_1","28DPI_2","28DPI_3","28DPI_4","28DPI_5","28DPI_6"))
...
p2 <- FeaturePlot(UROcells, features = i, split.by = "DataSet", pt.size = 1) &
  scale_color_gradientn(colors = myColor, limits = c(-100, 40))  &
  ggtitle(paste0(i," activity"))
ggsave(paste0(i, "_URO_cluster_featurePlot_DateClusterCombined.pdf"), plot = p2, width = 12, height = 4)
...
#   ggtitle(paste0(i," activity"))
#   
#   ggsave(paste0(i, "_URO_cluster_featurePlot_DateClusterCombined.pdf"), plot = p2, width = 10, height = 5)
# }
```
...
row.names(scrna@meta.data) <- scrna@meta.data$rn
 ...
 saveRDS(scrna, file = "PIPSeq_AnnotatedWithUro_v061324.rds")
 
 #scrna <- readRDS("PIPSeq_AnnotatedWithENDURO_v061324.rds")
