...
Idents(PTcells) <- PTcells @meta.data$SCT_snn_res.0.2
Idents(PTcells)
```
...
write.csv(PTsubclusterBiomarkers, "PTsubclusterBiomarkers_Mast.csv")
```
...
PTsubclusterBiomarkersTop3 <-
  as.data.frame(PTsubclusterBiomarkers) %>% filter (avg_log2FC > 0.25) %>% group_by(cluster) %>% slice_max(order_by = avg_log2FC, n = 3) %>% pull (gene)

PTsubclusterBiomarkersTop3 %>% unique()
DoHeatmap(PTcells, features = PTsubclusterBiomarkersTop3 , slot = "scale.data") + scale_fill_gradientn(colors = c("blue", "white", "red")) + theme(axis.text.y = element_text(size = 10))
ggsave(
  "PTsubclusterBiomarkersTop3.pdf",
  plot = PTsubclusterBiomarkersTop3,
  height = 6,
  width = 6
)
...
PTSubBiomarkersTop3DotPlot <-
  DotPlot(PTcells, features = PTsubclusterBiomarkersTop3 %>% unique()) +
  RotatedAxis()
...
for (n in Idents(PTcells)) {
  ...
  PTClusterSpecifcUpgene[[n]] <-
    PTsubclusterBiomarkers %>% filter(cluster %in% n &
...
for (m in PTclusters) {
  ...
  PTClusterSpecifcUpgene[[m]] <-
    as.data.frame(PTsubclusterBiomarkers) %>% filter(cluster == m &                                                     avg_log2FC >= 0.25) %>% pull(gene)
