...
#ICPCcells <- subset(scrna, subset = (Atp6v1g3 > 0 | Atp6v1b1 >0) & (Slc26a4 >0 |Hmx2 >0 | Kit>0 | Slc4a1 >0 |Aqp6 >0 )  )
...
ggsave(filename = "ICPCcellsDim.pdf", plot = ICPCcellsDim, width = 8, height = 6)

ICPCcellsDimSplit<-DimPlot(ICPCcells, split.by = "DataSet",label = T)
ICPCcellsDimSplit 
ggsave(filename = "ICPCcellsDimSplit.pdf", plot = ICPCcellsDimSplit, width = 24, height = 6)
...
CollectingDuctMarkersDotPlot <- DotPlot(ICPCcells, features = ICPCmarkers)

ggsave(filename = "CollectingDuctMarkersDotPlot.pdf", plot = CollectingDuctMarkersDotPlot, height = 5, width = 12)
...
ICPCcells@meta.data <- ICPCcells@meta.data %>%   mutate(ICPCAnnot = 
           case_when(SCT_snn_res.0.1 %in% c("0") ~ "PC", # PC markers: Aqp2, Hsd11b2, scnn1g
                     SCT_snn_res.0.1 %in% c("1") ~ "CD", # Seems to be an combined CDs that affected by infection
                     SCT_snn_res.0.1 %in% c("2") ~ "ICA", # Intercalted Cell A markers: Slc4a1, Aqp6
                     SCT_snn_res.0.1 %in% c("3") ~ "ICB", # Intercalated Cell B markers: Slc26A4, SpinkB
                 )) 

ICPCcells@meta.data %>% head()

Idents(ICPCcells) <- ICPCcells@meta.data$ICPCAnnot
Idents(ICPCcells)

# Reorder the levels:
Idents(ICPCcells)<-ordered(factor(Idents(ICPCcells)),levels=c("ICA","ICB","PC","CD"))

FinalMarkersWithAnn <- c("Atp6v1g3","Aqp6","Slc4a1","Slc26a7","Kit","Slc26a4","Spink8","Hmx2","Aqp2","Hsd11b2","Scnn1g")

ICPGenesDotPlotsWithAnn<- DotPlot(ICPCcells, features =  FinalMarkersWithAnn, col.min = -1) +RotatedAxis()
ICPGenesDotPlotsWithAnn
ggsave(filename = "ICPGenesDotPlotsWithAnn.pdf", plot = ICPGenesDotPlotsWithAnn, width = 8, height = 4)

ICPCcellsDimWithAnno<- DimPlot(ICPCcells,label = T)
ICPCcellsDimWithAnno
ggsave(filename = "ICPCcellsDimWithAnno.pdf", plot = ICPCcellsDimWithAnno, width = 5, height =4)

ICPCcells@meta.data %>% head()
ICPCcellsDimWithAnnoGroup<- DimPlot(ICPCcells,label = T, split.by = "DataSet")
ICPCcellsDimWithAnnoGroup
ggsave(filename = "ICPCcellsDimWithAnnoGroup.pdf", plot = ICPCcellsDimWithAnnoGroup, width = 12, height =4)
...
# 
  ...
  ggsave(paste0(Outdir,"/Biomarkers/", i, "_Feature_GeneMarkers_Annoatation.pdf"), plot =FeaturePlots, height =4, width = 14)
}

# heatmap for all the cells

ICPCcellsMarkerHeatmapWithAnno<- DoHeatmap(ICPCcells, features = FinalMarkersWithAnn)
ggsave(filename = "ICPCcellsMarkerHeatmapWithAnno.pdf", plot = ICPCcellsMarkerHeatmapWithAnno, width = 5, height =6)
...
```{r}
setwd(BioMarkerDir)
AllClustermarkers <- FindAllMarkers(ICPCcells,
...
ClusterSpecificGenes[["PC"]] <- AllClustermarkers %>% filter(cluster == "PC")%>%filter(p_val_adj <=0.05) %>% pull(gene)
...
# check the top 5 detected biomarkers in each cluster
ICPCTop5List<- AllClustermarkers %>% group_by(cluster) %>% slice_max(avg_log2FC, n = 5) %>% pull(gene)
ICPCTop5MarkersDotPlot <- DotPlot(ICPCcells, features =  ICPCTop5List) +RotatedAxis()
ggsave(filename = "ICPCTop5MarkersDotPlot.pdf", plot = ICPCTop5MarkersDotPlot, width = 10, height =6)
...
# 
ClusterSpecificGenesCompareCluster_Dotplot<- dotplot(ClusterSpecificGenesCompareCluster, showCategory = 10,font.size = 12,label_format = 80,by = "geneRatio")

ggsave(filename = "ClusterSpecificGenesCompareCluster_Dotplot.pdf", plot = ClusterSpecificGenesCompareCluster_Dotplot, width = 8, height = 9)

# write the cluster into the GO terms
write.csv(file = "ClusterSpecificGenesCompareClusterGO.csv", ClusterSpecificGenesCompareCluster)
...
UniqueGroupPathway<- ClusterSpecificGenesCompareClusterWithUniq %>% filter(is_duplicated == FALSE) %>% arrange() %>% group_by(Cluster) %>% slice_min( n=8) %>% pull(Description)

ClusterSpecificGenesCompareClusterWithUniq %>% filter(is_duplicated == FALSE) %>% group_by(Cluster) 
...
ggsave(filename = "UniqueGroupPathwayCompareCluster_Dotplot.pdf", plot = UniqueGroupPathwayCompareCluster_Dotplot, width = 10, height = 15)
...
# 
  FeaturePlots<-FeaturePlot(ICPCcells, features =  i, split.by = "DataSet" ) +RotatedAxis()
  ggsave(paste0(Outdir,"/Biomarkers/", i, "_RegulationofPhagocytosisMarkers.pdf"), plot =FeaturePlots, height =4, width = 14)
}


TopListCDMarkers <- AllClustermarkers %>% filter(cluster == "CD") %>% head(n=10) %>% pull(gene)

CDMarkerGenesDotPlot<- DotPlot(ICPCcells, features =TopListCDMarkers) +RotatedAxis()

ggsave(filename = "CDMarkerGenesDotPlot.pdf", plot = CDMarkerGenesDotPlot, width = 8, height = 4)

# 
```


# Then we concentrated on the 7 days infection compare to control

```{r}
ICPCcells@meta.data %>% head()

ICPCcells@meta.data$Celltype.Stim <- paste(ICPCcells@meta.data$ICPCAnnot,ICPCcells@meta.data$DataSet, sep = "_")
# set up the identity
Idents(ICPCcells) <- "Celltype.Stim"
Idents(ICPCcells)
# determine the DEGs from different cell types

levels(Idents(ICPCcells))

# comparison between 0DPI and 7DPI for ICA, ICB, PC, and CD
ICA_DEGs_D7vsD0 <- FindMarkers(ICPCcells, ident.1 = "ICA_7DPI", ident.2 = "ICA_0DPI",verbose = FALSE,min.pct = 0.25,test.use= "MAST") 
nrow(ICA_DEGs_D7vsD0)

ICB_DEGs_D7vsD0 <- FindMarkers(ICPCcells, ident.1 = "ICB_7DPI", ident.2 = "ICB_0DPI",verbose = FALSE,min.pct = 0.25,test.use= "MAST") 


PC_DEGs_D7vsD0 <- FindMarkers(ICPCcells, ident.1 = "PC_7DPI", ident.2 = "PC_0DPI",verbose = FALSE,min.pct = 0.25,test.use= "MAST") 

CD_DEGs_D7vsD0 <- FindMarkers(ICPCcells, ident.1 = "CD_7DPI", ident.2 = "CD_0DPI",verbose = FALSE,min.pct = 0.25,test.use= "MAST") 

# Identify all the DEGs from the collect duct
ICPCcells @meta.data %>% head()
Idents(ICPCcells) <- "DataSet"

CollectingDuctAllDEGs <- FindMarkers(ICPCcells, ident.1 = "7DPI", ident.2 = "0DPI",verbose = FALSE,min.pct = 0.25,test.use= "MAST") 
...
table(ICPCcells@meta.data$DataSet,Idents(ICPCcells)) %>% as.data.frame()
...
Idents(ICPCcells)<-ordered(factor(Idents(ICPCcells)),levels=c("4","5","0","6","1","2","3"))
...
MYEGenesDotPlots<- DotPlot(ICPCcells, features =  FinalMarkers, col.min = -1) +RotatedAxis()
MYEGenesDotPlots
...
#                     "Havcr1")
# 
# MarkerGenesDotPlots<- DotPlot(ICPCcells, features =  Finalcelltypemarkers, col.min = -0.5) +RotatedAxis()
# MarkerGenesDotPlots
...
DoHeatmap(ICPCcells, features = FinalMarkers)
...
for (i in 1:length(ImmuneDiffType)) {
  ...
  for (j in ImmuneDiffType[[ImmuneCellType]]) {
    ...
    FeaturePlots <-
      FeaturePlot(ICPCcells, features =  j, split.by = "DataSet") + RotatedAxis()
    ggsave(
      paste0(
        ImmuneCellTypeDir,
        ImmuneCellType,
...
levels(ICPCcells@meta.data$SCT_snn_res.0.3)
ICPCcells@meta.data <- ICPCcells@meta.data %>%   mutate(ImmuneIntegrateAnnot = 
...
Idents(ICPCcells) <- ICPCcells@meta.data$ImmuneIntegrateAnnot
Idents(ICPCcells)
