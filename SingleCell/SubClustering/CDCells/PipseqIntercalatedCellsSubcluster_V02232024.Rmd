---
title: "Intercalated Cells Sucluter Analyses"
author: "Xin Wang"
date: "2024-02-23"
description: Intercalated subclusters analyses:
  - Recluster of the Intercalated cells
  - Annotation of subclusters using the candidate markers
  - Psudotime of Intercalated
  - Basic statistics of Intercalated
...
Outdir =c("/Users/XXW004/Documents/Projects/RuizRosado/PIPseq/Results/Evaluation_V022723/CellEvaluation_Subclustering_V0803/IC_Subclusters/")
...
ICcells <- subset(scrna, subset = Raw_cell_type == "IC")

#ICcells <- subset(scrna, subset = (Atp6v1g3 > 0 | Atp6v1b1 >0) & (Slc26a4 >0 |Hmx2 >0 | Kit>0 | Slc4a1 >0 |Aqp6 >0 )  )
# To concentrate on the 

nrow(ICcells@meta.data)
# We then recluster the Immune cells
# recluster the cells
ICcells <- SCTransform(ICcells, verbose = TRUE)
ICcells <- RunPCA(ICcells,npcs = 8, verbose = TRUE)

# Run harmony for the Immune cells
ICcells <- RunHarmony(ICcells, 
				group.by.vars = c("DataSet"), 
				reduction = "pca", assay.use = "SCT", reduction.save = "harmony")
ICcells <- RunUMAP(ICcells, dims = 1:8, verbose = TRUE, reduction = "harmony")
ICcells <- FindNeighbors(ICcells, dims = 1:8,  verbose = TRUE, reduction = "harmony")

ICcells<- FindClusters(ICcells,  verbose = TRUE, resolution = 0.1)

ICcellsDim<- DimPlot(ICcells,label = T)

ggsave(filename = "ICcellsDim.pdf", plot = ICcellsDim, width = 8, height = 6)

ICcellsDimSplit<-DimPlot(ICcells, split.by = "DataSet",label = T)
ICcellsDimSplit 
ggsave(filename = "ICcellsDimSplit.pdf", plot = ICcellsDimSplit, width = 24, height = 6)

```

# check the ICA markers and ICB markers
```{r}
ICmarkers <- c("Atp6v1g3","Atp6v1g1","Rcan2","Slc26a4","Hmx2","Spink8","Slc26a7","Aqp6","Slc4a1","Kit")
ICmarkerFinal<- c("Atp6v1g3","Slc26a4","Aqp6")
DotPlot(ICcells, features = ICmarkerFinal)
```
...
Idents(ICcells)<-ordered(factor(Idents(ICcells)),levels=c("4","5","0","6","1","2","3"))
...
MYEGenesDotPlots<- DotPlot(ICcells, features =  FinalMarkers, col.min = -1) +RotatedAxis()
MYEGenesDotPlots
...
#                     "Havcr1")
# 
# MarkerGenesDotPlots<- DotPlot(ICcells, features =  Finalcelltypemarkers, col.min = -0.5) +RotatedAxis()
# MarkerGenesDotPlots
...
DoHeatmap(ICcells, features = FinalMarkers)
...
for (i in 1:length(ImmuneDiffType)) {
  ...
  for (j in ImmuneDiffType[[ImmuneCellType]]) {
    ...
    FeaturePlots <-
      FeaturePlot(ICcells, features =  j, split.by = "DataSet") + RotatedAxis()
    ggsave(
      paste0(
        ImmuneCellTypeDir,
        ImmuneCellType,
...
levels(ICcells@meta.data$SCT_snn_res.0.3)
ICcells@meta.data <- ICcells@meta.data %>%   mutate(ImmuneIntegrateAnnot = 
...
Idents(ICcells) <- ICcells@meta.data$ImmuneIntegrateAnnot
Idents(ICcells)

# Reorder the levels:
Idents(ICcells)<-ordered(factor(Idents(ICcells)),levels=c("NK","B","T","NEU","RecMAC","M1-M2","ResIMM"))
...
MYEGenesDotPlotsWithAnn<- DotPlot(ICcells, features =  FinalMarkersWithAnn, col.min = -1) +RotatedAxis()
MYEGenesDotPlotsWithAnn
ggsave(filename = "MYEGenesDotPlots_WithAnn.pdf", plot = MYEGenesDotPlotsWithAnn, width = 12, height = 5)

ICcellsDimWithAnno<- DimPlot(ICcells,label = T)
ICcellsDimWithAnno
ggsave(filename = "ICcellsDimWithAnno.pdf", plot = ICcellsDimWithAnno, width = 6, height =4)
...
ImmuneSubAnnotation <- ICcells@meta.data %>% dplyr::select(ImmuneIntegrateAnnot)
...
# add the frequency
```{r}
table(ICcells@meta.data$DataSet)/table(scrna@meta.data$DataSet)

ICcellsPropotionWithinTime <- table(ICcells@meta.data$DataSet,Idents(ICcells)) %>% as.data.frame() %>%
  group_by(Var1) %>%
...
ggsave("ICcellsPropotionWithinTime.pdf", plot = ICcellsPropotionWithinTime, width = 4, height = 3)
